...
优先级：**flag > `./WorkGround2.toml` > 用户配置文件 > 内置默认值**。从
**WorkGround2 v1.8.1** 开始，用户配置位于 macOS/Linux 的
`~/.WorkGround2/config.toml`，Windows 为 `%AppData%\WorkGround2\config.toml`；迁移和相关数据路径见
[配置路径](./CONFIG_PATHS.zh-CN.md)。标注为“仅用户/全局”的字段（包括 agent 轮数上限）不会被 `./WorkGround2.toml` 覆盖。
Provider 通过 `api_key_env` 命名密钥，真实密钥值保存在 CLI 与桌面端共用的
WorkGround2 全局 `<WorkGround2 home>/.env`。项目 `.env`、home `.env`、继承的 shell 环境变量、旧 credentials 和系统 keyring 都不再作为 provider key 的运行时 fallback；旧凭据只作为迁移来源读取。项目 `.env` 仍会作为当前 workspace 范围内的 MCP/plugin 非 provider `${VAR}` 展开来源，但不会导入 provider key 或 WorkGround2 控制变量。全局 `config.toml` 和 `.env` 的完整结构见
[配置路径](./CONFIG_PATHS.zh-CN.md)。
...
# language    = "zh"               # 界面语言；为空则按 $LANG / $WorkGround2_LANG 自动检测
...
# password_hash = ""           # 用 WorkGround2 serve --hash-password --password '...' 生成
...
多数日常设置应写在 `config.toml` 或前文提到的 WorkGround2 全局 `.env` 中。下面这些变量是进程级高级开关；
需要在启动 WorkGround2 之前设置。项目 `.env` 不是 WorkGround2 控制变量的运行时来源。

`WorkGround2_MEMORY_COMPILER_LLM_CLASSIFICATION=true` 会为 Memory v5 启用可选的 LLM 任务/聊天分类器。
默认关闭，此时 WorkGround2 使用本地 heuristic classifier，不会产生额外 provider 调用。开启后，分类缓存未命中时，
WorkGround2 可能先通过已配置 provider 发送一个很小的分类请求，再决定用户输入是任务还是普通对话；这会增加少量延迟、
...
`WorkGround2 serve` 会用同一个本地 WorkGround2 引擎启动浏览器 UI。适合不安装桌面端但想用可视化界面、
...
**API 地址** 填写服务端点。默认模式下，WorkGround2 会预览并把聊天请求发送到：
...
开启 **完整 URL**。开启后 WorkGround2 会直接使用该地址，不再追加 `/chat/completions`。
输入框下方的预览就是最终请求地址。
...
桌面端快捷键在 **设置 → 快捷键** 中管理。选择一行后按下新的组合键，WorkGround2 会为桌面端保存该绑定。
...
| 终端原生选择 | 复制 transcript 文本 | WorkGround2 默认不启用鼠标报告，因此终端自己的选择/复制仍可使用。 |
...
`WorkGround2` 会在 writer 调用前征求同意（普通工具为 `1` 本次 · `2` 本会话允许此范围 · `3` 总是允许此范围（保存） · `4` 拒绝；Bash 可额外选择命令前缀授权）；
其中 Bash 默认按具体命令记，也可按安全推导出的命令前缀记（如 `Bash(go test:*)`）；文件编辑类工具的本会话授权按编辑能力记，持久授权则写入 `Edit(<path>)` 文件路径规则；
`WorkGround2 run` 保持自主运行但仍然遵守 `deny`。
...
WorkGround2 是一个 MCP 客户端。`[[plugins]]` 的 `type` 选择传输：`stdio`（默认）启动本地子进
...
`mcpServers.<server>.trusted_read_only_tools`，普通 WorkGround2 plugin 会写入用户级 WorkGround2 config。
...
各自暴露的内容。`make build` 还会产出 `bin/WorkGround2-plugin-example`——一个可直接运行的
...
**已有 Claude Code 的 `.mcp.json`？** 直接放到项目根目录，WorkGround2 会原样读取——其
`mcpServers` 规范（`command`/`args`/`env`、`type`/`url`/`headers`、`${VAR}` 展开）
与 `[[plugins]]` 字段一一对应。两处来源会合并加载；同名时以 `WorkGround2.toml` 为准。
...
**从 `0.x` 升级？** 旧的 `~/.WorkGround2/config.json` 仍会被读取（读其 `mcpServers`、并遵从
`mcpDisabled`），作为最低优先级来源——所以 MCP 服务器照常可用；方便时再把它们挪进
`WorkGround2.toml` 的 `[[plugins]]` 或 `.mcp.json`。

## 斜杠命令

交互式 `WorkGround2` 会话里，内置命令（`/compact`、`/new`、`/clear`、`/rewind`、`/tree`、`/branch`、`/switch`、`/todo`、`/model`、`/mcp`、`/skills`、`/hooks`、`/memory`、`/memory-v5`、`/goal`、`/output-style`、`/sandbox`、`/language`、`/auto-plan`、`/reasoning-language`、`/help`）在本地执行——`/help` 可列出全部。
...
是放在 `.WorkGround2/commands/`（项目）或 `~/.WorkGround2/commands/`（用户）下的 Markdown 文件——
`review.md` 即 `/review`，子目录构成命名空间（`git/commit.md` → `/git:commit`）。文件正文
是 prompt 模板，调用即作为一轮对话发出。

`/memory` 会同时列出记忆文档（`WorkGround2.md` / `AGENTS.md`）和已保存的 auto-memory 条目。
...
Memory v5 在 CLI/TUI、`WorkGround2 serve` 和桌面端默认开启，因为这些入口共用同一套本地
controller。它会把本地、按项目隔离的执行轨迹和编译器状态写在 WorkGround2 home 下，并且只有
...
`WorkGround2 config memory-v5 off|observe|compact|on|status`。桌面端还可以在设置 → 通用 → Memory v5 中控制。
...
CLI/TUI 和 `WorkGround2 serve` 使用同一个 user/global 配置。项目内的 `WorkGround2.toml` 不能覆盖
这个 user/global 设置。CLI 命令会更新底层配置；高级用户也可以手动编辑 WorkGround2 home 下的
用户配置：
...
`WorkGround2 run --metrics <path>` 时，JSON 还会输出内容无关的 `memory_compiler_*` 汇总字段，
...
Goal 是长期目标的统一运行机制。普通 `/goal` 继续走轻量 Goal：WorkGround2 会持续推进，直到
...
`.WorkGround2/autoresearch/<task-id>/` 任务目录。高级用户可以用
...
它会创建或复用项目级 `.WorkGround2/autoresearch/<task-id>/` 目录。新任务默认使用
...
完成。动态运行态只写进 `.WorkGround2/autoresearch/...`，不写入 `WorkGround2.md`、`AGENTS.md`、
project memory、tool schema 或 cache-stable system prompt。公开发布、破坏性操作、凭证、
付款和外部通知仍然遵守正常的 approval、privacy 与 cache gate。

## @ 引用

在消息里写 `@` 引用，WorkGround2 会在发送前解析成带标签的上下文块：`@path/to/file`（或
...
`WorkGround2 setup` 刻意保持首次体验极简：选 provider → 输入 key（所选 provider 的所有
SKU 都会启用）。若要让两个模型协同（执行器 + 规划器，各自独立、缓存稳定的
session），向导后手动在 `WorkGround2.toml` 加一行即可：
...
Planner 会看到已加载的 `WorkGround2.md` / `AGENTS.md` 记忆，并拿到一小组只读研究工具，
因此可以先检查相关文件再把计划交给执行器。写入类和流程类工具仍只给执行器使用。
`max_steps` 限制执行器；`planner_max_steps` 只限制规划器，两者都可设为 `0` 表示不限。

轮数上限请放在用户级配置。项目 `./WorkGround2.toml` 不会覆盖 `max_steps` 或
`planner_max_steps`。
...
`image_generation` 启动后绝不自动重试——返回明确错误以避免重复产出。图片成功优先
来自真实 `draw_image` 工具结果，并通过任务状态、配置输出目录、文件存在、非空、MIME
和图片解码校验；若 `draw_image` 结果不存在，回退到 CLI provider 通过请求作用域收集器
报告的 Codex 副作用制品（`$CODEX_HOME/generated_images/<thread_id>/`），执行同等校验。
文字声称、伪路径和越界路径都会失败。

能力不会从模型品牌自动推断，但有两个受控例外：Google Gemini chat 模型
（`gemini-*`）从模型名推断 `web_search`，当前 Gemini 3 图片模型也识别搜索 grounding；
Google 图片模型（`imagen-*`、`nano-banana*`、已知 Gemini Image 模型家族）推断 `image_generation`。
所有 Gemini 模型推断 `vision`。显式 `capabilities`（含空数组）始终优先并关闭推断。
其他 Provider 通常需要显式声明：
...
执行与 `draw_image` 结果同等的校验：绝对路径、目录边界、非空普通文件、image MIME、
可解码。
...
的任务会自动进入 plan mode：WorkGround2 先只读生成计划，待用户批准后才
编辑文件或执行有副作用的命令。`auto_plan_classifier` 可以指定便宜的 provider，例如
`deepseek-flash`；它只在边界输入上调用，分类失败会回退到启发式规则。也可以用
在 `WorkGround2` 会话里用 `/auto-plan off|on` 修改用户级设置，或在 shell/脚本里用
`WorkGround2 config auto-plan off|on`。Auto-plan 只认用户级设置；项目
`WorkGround2.toml` 里的 `agent.auto_plan` 会被忽略。可见思考语言也采用类似形态：
会话里用 `/reasoning-language auto|zh|en`，shell/脚本里用
`WorkGround2 config reasoning-language auto|zh|en`。Memory v5 使用 `/memory-v5 off|observe|compact|on|status`
或 `WorkGround2 config memory-v5 off|observe|compact|on|status`，并且只认用户级设置。只有明确想为

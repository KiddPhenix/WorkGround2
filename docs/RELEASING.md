...
How WorkGround2 ships, who can ship what, and the canary-before-stable flow.
...
|---|---|---|
| npm | `latest` (0.x), `next` (1.x) | `canary` (`npm i WorkGround2@canary`) |
...
| **Ship `next` / stable** | **KiddPhenix only** | stable publish jobs gate on the `release` environment — KiddPhenix must approve before anything goes public |

So a maintainer can dispatch a canary anytime, but a stable release — even one a
maintainer starts by pushing a tag — pauses in the Actions UI until **KiddPhenix approves**
the `release` environment deployment.

> Repo settings backing this: Environments → `release` has KiddPhenix as a required
> reviewer; `canary` has none. (Optional hardening: a tag ruleset restricting
> `v*`/`npm-v*`/`desktop-v*` creation to KiddPhenix, so maintainers can't even start a
> stable release.)
...
3. **Test** — testers install `WorkGround2@canary` (CLI) or grab the desktop canary
...
5. **Ship stable** when the canary is clean — push the three tags:
   ...
   ```
   Each stable run **waits for KiddPhenix to approve the `release` environment** before publishing.
6. **Promote to default install** (optional, when 1.x should become the bare `npm i` target):
   ```sh
   npm dist-tag add WorkGround2@1.4.0 latest
   ```

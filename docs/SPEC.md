# WorkGround2 Engineering Spec

> WorkGround2 is a coding agent: a thin harness driving multiple models, with **all
...
```
WorkGround2/
├── go.mod / go.sum          # module WorkGround2; require BurntSushi/toml
├── Makefile                 # build / cross / vet / fmt / test
├── README.md / README.zh-CN.md
├── WorkGround2.example.toml         # sample config
├── docs/SPEC.md             # this file
├── cmd/WorkGround2/main.go          # entry; blank-imports built-in providers/tools
├── cmd/WorkGround2-plugin-example/  # reference MCP stdio plugin (a runnable example)
└── internal/
    ...
    ├── command/             # custom slash commands loaded from .WorkGround2/commands/*.md
...
- `cmd/WorkGround2-plugin-example` is a runnable reference stdio server (`echo`,
...
- The dropped originals are archived under the user config dir
  (`WorkGround2/archive/<timestamp>.jsonl`; see §5 for its per-OS location), one
...
- **Resolving `Ask`.** The interactive front-end (the chat TUI) prompts the user
  ...
  non-interactive run
  (`WorkGround2 run`, a sub-agent, anything with no TTY / no approver) cannot prompt, so
...
- **Collaboration mode is separate from tool approval.** The desktop composer
  ...
  `.WorkGround2/autoresearch/YYYYMMDD-HHMMSS-slug/` and keeps dynamic run state out
  of `WorkGround2.md`, `AGENTS.md`, project memory, tool schemas, and the
...
Out of the box (`mode = "ask"`, no rules) `WorkGround2 run` behaves exactly as before
...
- **Custom commands** are Markdown files under `.WorkGround2/commands/` (project) and
  the user config dir, e.g. `~/.WorkGround2/commands/` on macOS/Linux; the project dir overrides the user dir on a
...
- Frontmatter is an optional `---`-fenced block of simple `key: value` lines;
  `description` and `argument-hint` are recognised (no YAML dependency — WorkGround2
...
Resolution order: **flag > project `./WorkGround2.toml` > the user config file
> built-in defaults**. Starting with **WorkGround2 v1.8.1**, the user config lives
at `~/.WorkGround2/config.toml` on macOS/Linux and
...
without importing provider keys or WorkGround2 control variables. Step-limit
preferences belong in the user config.
Project `WorkGround2.toml` does not override `agent.max_steps` or
...
# language    = "zh"                # ui language tag; empty = auto-detect from $LANG / $WorkGround2_LANG
...
[agent]
system_prompt = "You are WorkGround2, a coding agent..."  # or system_prompt_file = "..."
...
temperature       = 0.0
memory_compiler = { enabled = true, verbosity = "observe" }   # user/global only; observe|compact; CLI: WorkGround2 config memory-v5 off|observe|compact|on|status
...
`WorkGround2 setup` writes this default config so the CLI is usable out of the box.
...
`[serve]` controls the HTTP browser frontend used by `WorkGround2 serve`. The
...
`[[plugins]]`; on a name collision `WorkGround2.toml` wins (it is the more explicit,
WorkGround2-specific source). This lets a server already configured for Claude work in
WorkGround2 unchanged.
...
`multi_edit`, `move_file`) to `workspace_root` (default cwd), the WorkGround2 user
...
- Build: `CGO_ENABLED=0 go build -ldflags "-s -w -X main.version=$(VERSION)" -o WorkGround2 ./cmd/WorkGround2`
...
- Sandbox Phase 1: an OS-level jail for `bash` so commands — not just the
  ...
  command unconfined via the permission gate (in `WorkGround2 run`, the command just
...
- "Always allow" persistence writing learned rules back to project config; a
  per-session permission override flag for `WorkGround2 run`.

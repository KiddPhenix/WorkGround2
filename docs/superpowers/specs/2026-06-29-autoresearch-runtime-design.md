...
WorkGround2 already has Goal mode and AutoResearch instructions. When a goal looks
long-running, `activeGoalBlock` injects an AutoResearch protocol that asks the
model to create `.WorkGround2/autoresearch/<task-id>/` and maintain files such as
...
- No network, publish, payment, credential, or destructive-operation bypass.
  Existing WorkGround2 gates still apply.
...
```text
.WorkGround2/autoresearch/<task-id>/
  state/
    task_spec.json
    progress.json
...
2. If AutoResearch is on, the controller creates a task unless the goal contains
   an explicit `.WorkGround2/autoresearch/<task-id>/` path.
...
- Explicit: a goal or prompt includes `.WorkGround2/autoresearch/<task-id>/`.
...
`AutoResearchOpenTask` opens `.WorkGround2/autoresearch/<task-id>/` in the
...
- Resume: starts or continues `/goal --research .WorkGround2/autoresearch/<task-id>/`
...
- explicit `.WorkGround2/autoresearch/<task-id>/` resumes existing state
...
No existing `.WorkGround2/autoresearch` task should be deleted or rewritten without

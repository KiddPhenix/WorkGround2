...
`.WorkGround2/autoresearch/<task-id>/`, injects a runtime summary into active goal
...
| State files are host-owned JSON/JSONL under `.WorkGround2/autoresearch/<task-id>/` | `task_spec.json`, `progress.json`, `directions_tried.json`, `findings.jsonl`, `iteration_log.jsonl`, `heartbeat.jsonl`; store tests and `./dev` smoke task directory | Verified |
...
| Explicit `.WorkGround2/autoresearch/<task-id>/` resumes existing task | `ResumeFromGoalText`; `TestResumeFromGoalTextLoadsExplicitTaskPath` | Verified |
...
```bash
GOCACHE=/private/tmp/WorkGround2-go-build-cache go test ./internal/autoresearch ./internal/control -count=1
GOCACHE=/private/tmp/WorkGround2-go-build-cache go test . -run 'TestAutoResearch|TestMetaReportsGoalStatus' -count=1
GOCACHE=/private/tmp/WorkGround2-go-build-cache go test . -count=1
...
- External browser loaded the page with title `WorkGround2`.
...
- Host state files were created under `.WorkGround2/autoresearch/...`.

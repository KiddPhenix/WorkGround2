...
Inside `WorkGround2`, use the slash command:
...
`WorkGround2.toml`; use the shell command with `--local` for that.

Headless runs also use the same setting:

```bash
WorkGround2 run "explain this module"
```
...
```text
./WorkGround2.toml > user config.toml > built-in defaults
```
...
When set to `zh` or `en`, WorkGround2 adds a small transient

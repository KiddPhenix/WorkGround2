...
`agent.reasoning_language` 控制模型服务暴露“可见思考过程”时，WorkGround2 希望它优先使用哪种语言。
...
桌面端设置会写入用户级默认值。项目仍然可以通过 `./WorkGround2.toml` 覆盖。
...
```bash
WorkGround2 config reasoning-language auto
WorkGround2 config reasoning-language zh
WorkGround2 config reasoning-language en
```

默认写入用户配置。要写入当前项目的覆盖配置：

```bash
WorkGround2 config reasoning-language --local zh
```

在 `WorkGround2` 内可以用斜杠命令：
...
斜杠命令会写入用户级设置，并立即更新当前 chat controller，后续 turn 生效。它不会改写当前项目的 `WorkGround2.toml`；如果要写项目级覆盖，请使用带 `--local` 的 shell 命令。

单次 headless 运行也会读取同一设置：

```bash
WorkGround2 run "解释这个模块"
```
...
```text
./WorkGround2.toml > 用户 config.toml > 内置默认值
```
...
当设置为 `zh` 或 `en` 时，WorkGround2 会把一个很小的 `<reasoning-language>` 临时 block 放进本次 user turn。它不会改变：

...
Resolution order: **flag > `./WorkGround2.toml` > the user config file >
built-in defaults**. Starting with **WorkGround2 v1.8.1**, the user config lives at
`~/.WorkGround2/config.toml` on macOS/Linux and
`%AppData%\WorkGround2\config.toml` on Windows; see
...
WorkGround2's global `<WorkGround2 home>/.env`, shared by CLI and desktop. Project
...
provider keys or WorkGround2 control variables. See
...
# language    = "zh"               # ui language; empty = auto-detect from $LANG / $WorkGround2_LANG
...
# password_hash = ""           # bcrypt hash generated with WorkGround2 serve --hash-password --password '...'
# behind_proxy = false         # true only behind a trusted reverse proxy

[[plugins]]
name    = "example"
command = "WorkGround2-plugin-example"
...
Most day-to-day settings belong in `config.toml` or the global WorkGround2 `.env`
described above. The variables below are process-level advanced switches; set
them before launching WorkGround2. Project `.env` files are not a runtime source for
WorkGround2 control variables.

`WorkGround2_MEMORY_COMPILER_LLM_CLASSIFICATION=true` enables the optional LLM
task/chat classifier for Memory v5. By default it is disabled, and WorkGround2 uses
...
```bash
WorkGround2_MEMORY_COMPILER_LLM_CLASSIFICATION=true WorkGround2
```

For development runs, prefix the command that starts the process, for example:

```bash
WorkGround2_MEMORY_COMPILER_LLM_CLASSIFICATION=true wails dev -forcebuild
```
...
`WorkGround2 serve` starts the same local engine behind a browser UI. Use it when
you want a desktop-style surface without installing the desktop app, when running
WorkGround2 on a remote development box through a tunnel, or when you want a
...
WorkGround2 serve --addr 0.0.0.0:8787 --auth token
WorkGround2 serve --auth password --password 'temporary-password'
```
...
```bash
WorkGround2 serve --hash-password --password 'strong-password'

# ~/.WorkGround2/config.toml
[serve]
...
chat path. In this mode WorkGround2 previews and sends chat requests to:
...
example `https://gateway.example.com/v1/chat/completions`. WorkGround2 then sends
...
`hookSpecificOutput.additionalContext`; WorkGround2 injects that text once into the
...
new key combination, and WorkGround2 saves it for the desktop app. Conflicting
...
| `/mouse` | Toggles in-app mouse capture | Off hands the mouse back to your terminal, restoring its native click-drag selection and right-click context menu, at the cost of in-app drag-select, the transcript scrollbar, and wheel-scroll. Set `WorkGround2_DISABLE_MOUSE=1` to start every session with it off. |
...
`WorkGround2` can grant Bash as an exact command or as a conservative command
...
`WorkGround2 run` stays autonomous but still honours `deny`.
...
WorkGround2 is an MCP client. A `[[plugins]]` entry's `type` selects the transport:
...
`mcpServers.<server>.trusted_read_only_tools`, while ordinary WorkGround2 plugins
are updated in the user's WorkGround2 config. Trust only side-effect-free readers;
...
exposes. `make build` also produces `bin/WorkGround2-plugin-example` — a runnable
...
**Already have an `.mcp.json`?** Drop it in the project root and WorkGround2
reads it as-is — the `mcpServers` spec (`command`/`args`/`env`, `type`/`url`/
`headers`, `${VAR}` expansion) maps field-for-field onto `[[plugins]]`. Both
sources are merged; on a name collision `WorkGround2.toml` wins.
...
**Upgrading from `0.x`?** Your old `~/.WorkGround2/config.json` is still read for its
`mcpServers` (honouring `mcpDisabled`) as a lowest-priority source, so MCP servers
keep working — move them into `WorkGround2.toml`'s `[[plugins]]` or a `.mcp.json` when
convenient.

## Slash commands

In an interactive `WorkGround2` session, built-in commands (`/compact`, `/new`, `/clear`, `/rewind`,
...
another branch. **Custom commands** are Markdown files under `.WorkGround2/commands/`
(project) or `~/.WorkGround2/commands/` (user) — `review.md` becomes
...
`/memory` lists both memory documents (`WorkGround2.md` / `AGENTS.md`) and saved
...
Memory v5 is enabled by default across the CLI/TUI, `WorkGround2 serve`, and the
desktop app because they all share the same local controller. It records local,
project-scoped execution traces and compiler state under WorkGround2 home, then
...
interactive session, or with `WorkGround2 config memory-v5 off|observe|compact|on|status`
from a shell/script.
...
CLI/TUI and `WorkGround2 serve` use the same user/global config. Project
`WorkGround2.toml` files cannot override this user/global setting. The CLI command
...
aggregate metrics upload pipeline. When `WorkGround2 run --metrics <path>` is used,
...
objectives stay lightweight: WorkGround2 keeps working until the goal is complete,
...
`.WorkGround2/autoresearch/<task-id>/` directory. Advanced users can force it with
...
`.WorkGround2/autoresearch/<task-id>/` directory. For new tasks, the default id
shape is `YYYYMMDD-HHMMSS-slug`, such as `20260618-224530-cache-audit`; WorkGround2
...
broad requirement. Dynamic run state stays in `.WorkGround2/autoresearch/...`, not
in `WorkGround2.md`, `AGENTS.md`, project memory, tool schemas, or the cache-stable
...
Embed `@` references in a message and WorkGround2 resolves them before sending, as
...
`WorkGround2 setup` keeps first-run minimal: pick provider → keys (every SKU of a
...
The planner sees loaded `WorkGround2.md` / `AGENTS.md` memory and a small read-only
...
Keep step-limit preferences in the user config. Project `./WorkGround2.toml` files
...
automatically: WorkGround2 first drafts a read-only plan, then waits for approval
...
`/auto-plan off|on` inside `WorkGround2` to change the user-level setting, or
`WorkGround2 config auto-plan off|on` from a shell/script. Auto-plan is user-level
only; `agent.auto_plan` in a project `WorkGround2.toml` is ignored. The visible
reasoning language uses a similar shape: `/reasoning-language auto|zh|en` in the
session, or `WorkGround2 config reasoning-language auto|zh|en` in a shell/script.
Memory v5 uses `/memory-v5 off|observe|compact|on|status` or
`WorkGround2 config memory-v5 off|observe|compact|on|status` and is user-level only. Pass `--local`

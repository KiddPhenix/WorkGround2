...
for (const t of TARGETS) {
  const name = `@WorkGround2/cli-${t.node}`;
  const dir = join(STAGE, `cli-${t.node}`);
  const exe = t.goos === "windows" ? "WorkGround2.exe" : "WorkGround2";
    ...
    `${JSON.stringify(
      {
        name,
        version,
        description: `WorkGround2 prebuilt binary for ${t.node}.`,
        ...
        repository: {
          type: "git",
          url: "git+https://github.com/KiddPhenix/WorkGround2.git",
        },
      },
      null,
      2,
...
const mainDir = join(STAGE, "WorkGround2");
mkdirSync(mainDir, { recursive: true });
cpSync(join(HERE, "WorkGround2", "bin"), join(mainDir, "bin"), { recursive: true });
cpSync(join(ROOT, "README.md"), join(mainDir, "README.md"));

const mainPkg = JSON.parse(
  readFileSync(join(HERE, "WorkGround2", "package.json"), "utf8"),
);
...
// stable to default with a manual `npm dist-tag add WorkGround2@<ver> latest`.
...
console.log(`publish WorkGround2@${version} (${distTag})`);

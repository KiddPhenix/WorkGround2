...
- Summary: 长期助手聚合（使命、责任图 Plan、Routine、记忆、Run、Attention、渠道与改进提案）与本机后台调度；Run 使用冻结的用户意图生成可读 Session 标题并自愈旧内部提示标题，从助手打开的 Session 保留精确路径与助手身份以提供返回入口；Run 成功后经 `<assistant-progress>` 幂等推进责任、证据、机会与类型化配置提案。提案经用户接受后用目标基线 revision 原子应用，用户已修改的目标不会被覆盖，权限不可通过提案扩大。Assistant 可经二次确认删除，聚合原子移入本地回收区且请求重放安全，既有 Session 保留；管理页关闭按钮避开 Windows 原生标题栏控制区。浏览器点击/输入按当前 page revision 的真实元素语义分类，普通交互随 `network`，发帖/回复/提交随 `publish` 三态，密码、私有字段、删除与付费继续保持敏感边界；Desktop 与 daemon 共用同一权限映射。阶段 6 新增角色化调度：Dispatcher 先持久化直接输入并分类（task/question/feedback/improvement/correction/control）生成一级回复与 0..N 具名 Runner Job（稳定 request ID 幂等重放、改参冲突、模型不可用/JSON 不合法显式可重试不丢输入）；Job 冻结分类/目标/权限/Workspace/ContextPack revision，单助手最多并行 3 个且带 lease/fence/取消/失败/重试/重启恢复；全部 Job 终态后 Reflector 恰好一次生成有界 ContextPack（按归属过滤、限条数/UTF-8 字节）；Ideator 在新增 5 个成功 task Dispatch 或距上次 7 天时低频触发（另有手动入口），产出 IdeaProposal 只允许 pending→accepted/rejected/superseded 的 revision/CAS，接受只写 strategy 记忆或创建责任候选，绝不改 Mission/Policy/Workspace/凭据/发布或执行外部动作。三个角色都经真实模型（`RoleModel` 接口，Desktop/daemon 用 `boot.Build`+Controller+冻结 deny policy 无工具副作用调用），受限于严格 JSON 协议与 DisallowUnknownFields 校验，拒绝任何权限/Workspace/外部动作字段；反思与周期脑洞失败持久为可观察、可重试、有界退避状态。Desktop UI 时间线展示分类、Dispatcher 回复、Runner 状态、反思摘要与脑洞提案，提供手动脑洞、接受/拒绝、失败重试与分类重试；迟到 snapshot 不覆盖新 revision。
- Keywords: AssistantWorkspace, AssistantRuntime, AssistantSessionTarget, SessionReturn, AssistantDelete, ChangeProposal, ResolveProposal, BrowserPermissionSubject, publish policy, local trash, readable title, Responsibility, Plan, CompleteRunWithProgress, Dispatch, RunnerJob, ContextPack, IdeaProposal, Dispatcher, Reflector, Ideator, RoleModel
- Source: verified-by-search
- Updated: 2026-08-27

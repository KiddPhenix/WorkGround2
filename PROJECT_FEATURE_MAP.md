...
- Summary: `work.enabled` 缺省为 true，仅显式 false 关闭；Stage 6 生产入口按 per-tab 配置稳定占位，WorkCapable 仅控制可点击性且失败可原位重试，迟到 ACK 保持 tab/generation 隔离；Cornerstone Drawer 在失败/空态/Pin 表单中均可返回或 Esc 关闭并恢复焦点。Stage 7 归档历史/复用仍为 todo、未实现。
- Keywords: work.enabled, WorkEnabled, WorkCapable, WorkPage, WorkView, CornerstoneDrawer, drawer return, focus restore, Stage7 todo

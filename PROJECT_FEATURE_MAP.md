...
- Summary: `work.enabled` 缺省为 true，仅显式 false 关闭；Stage 6 生产入口、空态创建、SessionSurface 和迟到 ACK 防护保持不变。WorkView 共享输出把必需集合规范为数组，Wails port 解码 RawMessage 字节 payload 后仍由 Store 严格校验 workID/revision/generation；订阅失败可单 generation 重试恢复且不泄漏 listener。Stage 7 未启动。
- Keywords: work.enabled, WorkCapable, WorkPage, WorkView, RecoverWorkView, WatchWork, RawMessage, authoritative retry snapshot, retry subscription, Stage7 todo

// Client for the WorkGround2-accounts API (id.WorkGround2.io). Cookie-based session,
// so every call sends credentials; the API base is build-time configurable.
const API = (import.meta.env.PUBLIC_ACCOUNTS_API || "https://id.WorkGround2.io").replace(/\/$/, "");
...
// under WorkGround2.io, so a subdomain (e.g. crash.WorkGround2.io) can return here
...
function safeNext(next) {
  ...
  try {
    ...
    if (u.protocol === "https:" && (u.host === "WorkGround2.io" || u.host.endsWith(".WorkGround2.io"))) return next;
  } catch {}
  return null;
}

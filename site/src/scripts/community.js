// WorkGround2 Community client. Renders the forum from the forum.WorkGround2.io API and
// gates posting on the shared id.WorkGround2.io session (cookie sent cross-subdomain).
// Bilingual like the rest of the site: static labels via .l-en/.l-zh spans that the
// shared `WorkGround2-lang` choice toggles; plain-text strings pick the current lang.
const FORUM = (import.meta.env.PUBLIC_FORUM_API || "https://forum.WorkGround2.io").replace(/\/$/, "");
const ACCOUNTS = (import.meta.env.PUBLIC_ACCOUNTS_API || "https://id.WorkGround2.io").replace(/\/$/, "");
...
function setLang(l) {
  ...
  try { localStorage.setItem("WorkGround2-lang", l); } catch {}
  applyLangText();
}
function initLang() {
  let saved = "";
  try { saved = localStorage.getItem("WorkGround2-lang") || ""; } catch {}
...
const CATDESC = {
  ...
  show: ["Built something with WorkGround2? Show the community.", "用 WorkGround2 做了东西？来给社区看看。"],
...
async function renderThread() {
  ...
  document.title = `${topic.title} — ${t("WorkGround2 Community", "WorkGround2 社区")}`;
  ...
  if (!account) {
    zone.innerHTML = `<div class="composer"><div class="gate"><p>${L("Sign in with your WorkGround2 account to reply.", "用你的 WorkGround2 账号登录后回复。")}</p><a class="btn btn-primary" href="${loginUrl()}">${L("Sign in", "登录")}</a></div></div>`;
    return;
  }
  zone.innerHTML = `

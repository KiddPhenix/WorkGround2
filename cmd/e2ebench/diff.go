...
func resetTree(repo string) {
	...
	_ = exec.Command("git", "-C", repo, "clean", "-fd", "-e", "WorkGround2.toml").Run()
}
...
func renderDiff(r diffReport) string {
	...
	fmt.Fprintf(&b, "## 🤖 WorkGround2 e2e — diff test-gen\n\n")
...
// repoRelFromModulePath turns "workground2/internal/agent/foo.go" into

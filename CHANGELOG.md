...
All notable changes to the Go line (WorkGround2 1.0+) are recorded here. The legacy
`0.x` TypeScript history lives on the [`v1`](https://github.com/KiddPhenix/WorkGround2/tree/v1)
branch.
...
- Agent runtime defaults now leave both executor and dedicated planner tool-call
  ...
  come from the user/global config only; project `WorkGround2.toml` does not
  override them.
...
- **Go kernel**: a single static binary (CGO-free), cross-compiled for
  ...
  the native binary), Homebrew (`KiddPhenix/WorkGround2` tap), and release archives;
...
- **Code intelligence via CodeGraph**: a tree-sitter symbol/call graph
  ...
  or API cost. Fetched into a local cache on first use (or `WorkGround2 codegraph
...
- **Memory**: `WorkGround2.md` hierarchy + auto-memory, folded into the cache-stable
  prefix.
- **ACP** (`WorkGround2 acp`) and an HTTP/SSE server frontend; desktop app (Wails).
...
[1.0.0]: https://github.com/KiddPhenix/WorkGround2/releases/tag/v1.0.0

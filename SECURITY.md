...
WorkGround2 security fixes are prioritized for the currently developed Go rewrite
...
- Affected WorkGround2 version, commit, operating system, and installation method.
...
WorkGround2 is a local coding agent. Many features intentionally operate on the
...
WorkGround2 supports `@path` references so users can include local files and images
...
- A local user intentionally asks WorkGround2 to read, edit, or send their own

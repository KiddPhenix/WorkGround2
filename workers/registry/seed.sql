...
--   wrangler d1 execute WorkGround2-registry --local --file=seed.sql
DELETE FROM events;
DELETE FROM stars;
...
VALUES
  ('skill','KiddPhenix','pr-review','KiddPhenix/pr-review','Adversarial multi-pass PR review with inline findings','https://github.com/KiddPhenix/WorkGround2-skills/pr-review','skill','https://github.com/KiddPhenix/WorkGround2-skills','review,git,quality','1.2.0',428,96,1,1,datetime('now','-40 days'),datetime('now','-3 days')),
  ('skill','KiddPhenix','go-test-gen','KiddPhenix/go-test-gen','Generate table-driven Go tests for the current package','https://github.com/KiddPhenix/WorkGround2-skills/go-test-gen','skill','https://github.com/KiddPhenix/WorkGround2-skills','go,testing','0.4.1',213,44,1,1,datetime('now','-22 days'),datetime('now','-6 days')),
...
INSERT INTO events (type, package_id, actor_handle, summary, created_at) VALUES
  ...
  ((SELECT id FROM packages WHERE slug='KiddPhenix/pr-review'),'star','sivancola','starred KiddPhenix/pr-review',datetime('now','-3 hours')),

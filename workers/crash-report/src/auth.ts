// Dashboard authorization. Identity comes from id.WorkGround2.io (the shared
...
// The id.WorkGround2.io session cookie is scoped to `.WorkGround2.io`, so the browser
...
function idOrigin(env: Env): string {
  return (env.ID_ORIGIN ?? "https://id.WorkGround2.io").replace(/\/$/, "");
}

function appOrigin(env: Env): string {
  return (env.APP_ORIGIN ?? "https://WorkGround2.io").replace(/\/$/, "");
}
...
// Ends the shared id.WorkGround2.io session (best-effort) and returns a Set-Cookie
...
export async function sharedLogout(request: Request, env: Env): Promise<string> {
  ...
  return `${SHARED_COOKIE}=; HttpOnly; Secure; SameSite=Lax; Path=/; Domain=.WorkGround2.io; Max-Age=0`;
}

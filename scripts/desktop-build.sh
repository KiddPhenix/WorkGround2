...
#   macOS:   WorkGround2-darwin-<arch>.zip                  (ditto archive; updater channel)
#            WorkGround2-darwin-universal.dmg               (drag-to-install; human download)
#   Windows: WorkGround2-windows-<arch>-installer.exe       (NSIS per-user installer; updater channel)
#            WorkGround2-windows-<arch>.zip                 (portable human download)
#   Linux:   WorkGround2-linux-<arch>.tar.gz                (bare binary; updater channel)
#            WorkGround2-linux-<arch>.deb                   (Debian/Ubuntu package; human download)
#
...
APPNAME="WorkGround2"            # wails.json productName -> WorkGround2.app
BINNAME="WorkGround2"            # wails.json outputfilename -> Linux binary name
...
case "$os" in
darwin)
	# Wails names the bundle after outputfilename (WorkGround2.app). Copy it into a
	# disposable staging directory before signing and packaging.
	staging=$(mktemp -d)

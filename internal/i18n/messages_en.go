...
var English = Messages{
	...
	SetKeyHint:      "Set your API key, then run `WorkGround2`.",
	ConfigLabel:     "config",
	ModelsLabel:     "models",
	...
	StepScaffold:    "scaffold WorkGround2.toml",
	StepSetKey:      "set API key",

	InitHint:       "Project memory (AGENTS.md) is generated in-session: run `WorkGround2`, then `/init` — the model analyzes the codebase and writes it. For configuration, use `WorkGround2 setup`.",
	StepSetKeyHint: "run `WorkGround2 setup`, or export DEEPSEEK_API_KEY=…",
	StepChatDesc:   "interactive session",
	StepRunDesc:    "one-shot task",
	HelpFooter:     "WorkGround2 help · all commands",

	ChatTip:           "Context is kept across turns. Type 'exit' or Ctrl-D to quit.",
	TurnCancelled:     "cancelled — back to prompt",
	NoSessionToResume: "no saved session to resume — start a new one with `WorkGround2`",
	...
	OutputStyleHint:    "set agent.output_style in WorkGround2.toml to apply one (takes effect next session)",
	ThemeHeader:        "themes:",
	...
	SlashMCPNone:                 "no MCP servers configured — add a [[plugins]] entry in WorkGround2.toml",
	...
	ArgLanguageAuto:     "auto-detect from WorkGround2_LANG / locale",
	...
	ListHooksNone:       "hooks: none active — configure in .workground2/settings.json (project, after trust) or ~/.workground2/settings.json (global)",
	ListMcpHeader:       "mcp servers",
	ListMcpNone:         "mcp: no servers connected — add one in WorkGround2.toml ([[plugins]]) or a project .mcp.json",

	MemoryNone:             "memory: none — add with “/remember <note>” or create WorkGround2.md in the project root",
	...
	MissingKeyIntro:       "WorkGround2.toml is ready — just an API key away.",
	WroteFileFmt:          "Wrote %s",
	SetupComplete:         "Setup complete.",
	SetupCancelled:        "setup cancelled.",
	TryHintFmt:            "Try: %s",
	NextHint:              "Next: set your API key (run `WorkGround2 setup` or export DEEPSEEK_API_KEY=...), then run `WorkGround2 run \"your task\"`.",
	...
	SkipStaleCustomEntryFmt:    "skipping stale %q entry from WorkGround2.toml (pointing at %s) — please remove it from [[providers]]",
	...
	UsageRunHint:              "usage: WorkGround2 run [--model NAME] <task>",
	ErrorPrefix:               "error:",
	...
	ProviderErrAuth:                "Authentication failed (HTTP 401): your API key is missing or unset. Add it to .env or run `WorkGround2 setup`.",
	ProviderErrAuthRejected:        "Authentication failed (HTTP 401): the server rejected your API key. It may be wrong or expired, or the provider hit a transient auth/quota issue — retried with backoff and still failed. Try again shortly, or check the key in .env / run `WorkGround2 setup`.",
	...
	UsageBody: `WorkGround2 — a config- and plugin-driven coding agent (multi-model)

Usage:
  WorkGround2 [--model NAME] [-c|--continue] [--resume] [--yolo] [--dir PATH]   interactive session (multi-turn; -c resumes the latest, --resume picks one)
  WorkGround2 run  [--model NAME] [--max-steps N] [-c|--continue] [--resume PATH] <task>   run one task and exit
  WorkGround2 review [--base BRANCH] [--commit SHA] [--model NAME]  AI-powered code review on local diffs
  WorkGround2 serve [--model NAME] [--addr HOST:PORT] [--auth none|token|password] [--token STR] [--password STR] [--hash-password]  serve over HTTP+SSE (with optional auth)
  WorkGround2 acp [--model NAME]                           serve Agent Client Protocol over stdio (also: WorkGround2 --acp)
  WorkGround2 setup [path]                                 interactive config wizard; writes WorkGround2.toml (+ .env)
  WorkGround2 config auto-plan [off|on]                    configure automatic plan mode
  WorkGround2 config memory-v5 [off|observe|compact|on|status]  configure Memory v5
  WorkGround2 config reasoning-language [auto|zh|en]        configure visible reasoning language
  WorkGround2 mcp <add|remove|list|import>                 manage MCP servers in WorkGround2.toml
...
Examples:
  WorkGround2
  WorkGround2 --continue
  WorkGround2 run "implement the TODOs in main.go"
  WorkGround2 run --model mimo-pro "add unit tests for this function"
  echo "explain this code" | WorkGround2 run

Configuration:
  Resolution: flag > ./WorkGround2.toml > ~/.workground2/config.toml > built-in defaults
  Secrets come from the environment via api_key_env (e.g. DEEPSEEK_API_KEY).
  Run 'WorkGround2 setup' to scaffold a config; see docs/SPEC.md.
`,
}

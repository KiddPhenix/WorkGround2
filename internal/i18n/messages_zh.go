...
var Chinese = Messages{
	...
	SetKeyHint:      "设置好 API key 后运行 `WorkGround2`。",
	ConfigLabel:     "配置",
	ModelsLabel:     "模型",
	...
	Ready:           "已就绪",
	GetStarted:      "开始使用",
	StepScaffold:    "生成 WorkGround2.toml",
	StepSetKey:      "设置 API key",

	InitHint:       "项目记忆（AGENTS.md）在会话内由模型生成：运行 `WorkGround2`，然后 `/init` —— 模型会分析代码库并写入。配置请用 `WorkGround2 setup`。",
	StepSetKeyHint: "运行 `WorkGround2 setup`，或 export DEEPSEEK_API_KEY=…",
	StepChatDesc:   "交互式会话",
	StepRunDesc:    "执行单次任务",
	HelpFooter:     "WorkGround2 help · 查看全部命令",

	ChatTip:           "对话上下文将跨轮保留。输入 'exit' 或按 Ctrl-D 退出。",
	TurnCancelled:     "已取消 — 回到提示符",
	NoSessionToResume: "没有可恢复的会话 — 用 `WorkGround2` 开一个新的",
	...
	OutputStyleHint:    "在 WorkGround2.toml 设置 agent.output_style 即可启用（下次会话生效）",
	ThemeHeader:        "主题：",
	...
	CornerstoneCleanupPreserved: "本次不会清理 %d 个 Work 基石",
	...
	SlashMCPNone:                 "没有配置 MCP 服务器 — 在 WorkGround2.toml 加一个 [[plugins]] 条目",
	...
	CmdRebuildVocab:     "扫描 Workspace 并重建项目词表",
	...
	ArgEffortMax:        "最高推理",
	ArgThemeCurrent:     "当前",
	ArgLanguageAuto:     "从 WorkGround2_LANG / 系统 locale 自动检测",
	...
	ListHooksNone:       "无生效 hooks — 在 .workground2/settings.json（项目，需信任后）或 ~/.workground2/settings.json（全局）配置",
	ListMcpHeader:       "MCP 服务器",
	ListMcpNone:         "未连接 MCP 服务器 — 在 WorkGround2.toml（[[plugins]]）或项目 .mcp.json 中添加",

	MemoryNone:             "还没有加载任何记忆 — 输入 “/remember 内容” 可快速记录，也可以在项目根目录创建 WorkGround2.md",
	...
	MissingKeyIntro:       "WorkGround2.toml 已配置好 — 只差一个 API key 就可以开始。",
	WroteFileFmt:          "已写入 %s",
	SetupComplete:         "设置完成。",
	SetupCancelled:        "设置已取消。",
	TryHintFmt:            "试试: %s",
	NextHint:              "下一步：设置 API key（运行 `WorkGround2 setup` 或 export DEEPSEEK_API_KEY=...），然后运行 `WorkGround2 run \"你的任务\"`。",
	...
	SkipStaleCustomEntryFmt:    "跳过 WorkGround2.toml 里的旧 %q 条目（指向 %s）— 请手动从 [[providers]] 里删除",
	...
	UsageRunHint:              "用法：WorkGround2 run [--model NAME] <task>",
	ErrorPrefix:               "错误：",
	...
	ProviderErrAuth:                "认证失败 (HTTP 401)：未读到 API key（缺失或未设置）。请在 .env 中配置密钥，或运行 `WorkGround2 setup`。",
	ProviderErrAuthRejected:        "认证失败 (HTTP 401)：服务端拒绝了你的 API key。可能是 key 错误或已过期，也可能是服务端出现瞬时鉴权/额度问题——已退避重试仍失败。请稍后再试，或检查 .env 中的密钥 / 运行 `WorkGround2 setup`。",
	...
	UsageBody: `WorkGround2 — 由配置和插件驱动的 coding agent（多模型）

用法：
  WorkGround2 [--model NAME] [-c|--continue] [--resume] [--yolo] [--dir PATH]   交互式会话（多轮；-c 恢复最近一次，--resume 选择一个）
  WorkGround2 run  [--model NAME] [--max-steps N] [-c|--continue] [--resume PATH] <task>   执行单次任务后退出
  WorkGround2 review [--base BRANCH] [--commit SHA] [--model NAME]  AI 代码审查（基于本地 diff）
  WorkGround2 serve [--model NAME] [--addr HOST:PORT] [--auth none|token|password] [--token STR] [--password STR] [--hash-password]  通过 HTTP+SSE 提供服务（支持可选认证）
  WorkGround2 acp [--model NAME]                           通过 stdio 提供 Agent Client Protocol（也可用：WorkGround2 --acp）
  WorkGround2 setup [path]                                 交互式配置向导；生成 WorkGround2.toml（及 .env）
  WorkGround2 config auto-plan [off|on]                    配置自动计划模式
  WorkGround2 config memory-v5 [off|observe|compact|on|status]  配置 Memory v5
  WorkGround2 config reasoning-language [auto|zh|en]        配置可见思考语言
  WorkGround2 mcp <add|remove|list|import>                 管理 WorkGround2.toml 里的 MCP 服务器
...
示例：
  ...
  WorkGround2 run "把 main.go 里的 TODO 实现掉"
  WorkGround2 run --model mimo-pro "给这个函数补单元测试"
  echo "解释这段代码" | WorkGround2 run

配置：
  优先级：flag > ./WorkGround2.toml > ~/.workground2/config.toml > 内置默认值
  密钥通过 api_key_env 从环境变量注入（如 DEEPSEEK_API_KEY）。
  运行 'WorkGround2 setup' 生成配置；详见 docs/SPEC.md。
`,
}

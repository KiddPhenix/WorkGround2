...
type Messages struct {
	...
	StepScaffold    string // step 1 desc — WorkGround2 setup
	StepSetKey      string // step 2 command label

	// `WorkGround2 init` — points to the in-session /init skill + setup
	InitHint       string
	StepSetKeyHint string // step 2 desc — env var hint
	StepChatDesc   string // interactive session step desc
	StepRunDesc    string // WorkGround2 run step desc
	HelpFooter     string // dim footer linking to WorkGround2 help
	...
	CmdRebuildVocab     string // /rebuild_vocabulary
	...
	WroteFileFmt          string // "Wrote %s" — used for WorkGround2.toml and .env both
	...
	SkipStaleCustomEntryFmt    string // "skipping stale %q entry from WorkGround2.toml (pointing at %s) — please remove it"
	...
	UsageRunHint              string // "usage: WorkGround2 run [--model NAME] <task>"
	...
	// `WorkGround2 upgrade` / `WorkGround2 update` — self-update
...
//
// Priority: override > WorkGround2_LANG > LC_ALL > LC_MESSAGES > LANG > "en".
...
func envCandidates() []string {
	keys := []string{"WorkGround2_LANG", "LC_ALL", "LC_MESSAGES", "LANG"}

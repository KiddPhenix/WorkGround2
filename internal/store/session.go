// Package store is the single authority for WorkGround2's on-disk persistence
...
// ~/.WorkGround2 tree) and the desktop root unification land in later slices.
package store
...
// SessionPinnedMemo is the per-session pinned-memory sidecar
// (<id>.pinned-memo.json).
func SessionPinnedMemo(sessionPath string) string {
	if sessionPath == "" {
		return ""
	}
	return sessionStem(sessionPath) + ".pinned-memo.json"
}
...
// SessionTaskMemory is the recoverable task briefing sidecar
// (<id>.task-memory.json).
func SessionTaskMemory(sessionPath string) string {
	if sessionPath == "" {
		return ""
	}
...
// transcript: branch meta, pinned memos, goal state, event/index logs, and
// diagnostic logs.
// Directory artifacts (checkpoints, jobs) and ephemeral lock/lease files have
// their own lifecycles and are intentionally not listed.

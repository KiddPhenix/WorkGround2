...
func TestToWirePhaseSource(t *testing.T) {
	w := ToWire(event.Event{Kind: event.Phase, Text: "model · planning", Source: event.UsageSourcePlanner})
	if w.Kind != "phase" || w.Source != event.UsageSourcePlanner {
		t.Fatalf("phase wire = %+v", w)
	}
}

func TestToWireTaskMemory(t *testing.T) {
	w := ToWire(event.Event{Kind: event.TaskMemoryUpdated, TaskMemory: event.TaskMemory{
		Goal: "ship", Current: "执行中", GoalSource: "explicit_goal", Revision: 7, UpdatedAt: 99,
	}})
	if w.Kind != "task_memory_updated" || w.TaskMemory == nil || w.TaskMemory.Revision != 7 {
		t.Fatalf("task memory wire = %+v", w)
	}
}
...
func TestToWireMessageMemoryCitations(t *testing.T) {
	...
	if got.Source != "MEMORY.md" || got.LineStart != 116 || got.LineEnd != 123 || got.Note != "WorkGround2 workflow" {

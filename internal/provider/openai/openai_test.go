...
func TestStreamSendsCustomHeaders(t *testing.T) {
	srv := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		...
		if r.Header.Get("HTTP-Referer") != "https://app.example" || r.Header.Get("X-Title") != "WorkGround2" {
...
// TestBuildRequestReasoningContentEmptyForDeepSeekToolCalls proves requirement 2:
// a DeepSeek thinking-mode tool_calls turn must include the reasoning_content key
// even when the value is empty. Sending the empty string is recovery-safe and
// DeepSeek accepts it; omitting the key entirely would 400 on a cache-miss replay.
func TestBuildRequestReasoningContentEmptyForDeepSeekToolCalls(t *testing.T) {
	msgs := []provider.Message{
		{Role: provider.RoleUser, Content: "list files"},
		{
			Role:             provider.RoleAssistant,
			ReasoningContent: "", // no reasoning received — recovery case
			ToolCalls:        []provider.ToolCall{{ID: "c1", Name: "bash", Arguments: `{}`}},
		},
		{Role: provider.RoleTool, Content: "ok", ToolCallID: "c1", Name: "bash"},
	}

	// DeepSeek must force-include reasoning_content even when empty.
	deepseek, _ := json.Marshal((&client{model: "deepseek-v4", deepseek: true}).buildRequest(provider.Request{Messages: msgs}).Messages)
	if !strings.Contains(string(deepseek), `"reasoning_content":""`) {
		t.Errorf("DeepSeek tool_calls turn must include reasoning_content even when empty: %s", deepseek)
	}

	// Non-DeepSeek must omit empty reasoning_content (requirement 3).
	other, _ := json.Marshal((&client{model: "mimo-v2"}).buildRequest(provider.Request{Messages: msgs}).Messages)
	if strings.Contains(string(other), "reasoning_content") {
		t.Errorf("non-DeepSeek must not include empty reasoning_content: %s", other)
	}

	// Plain assistant turn (no tool_calls) must omit reasoning_content even on DeepSeek (requirement 4).
	plainMsgs := []provider.Message{
		{Role: provider.RoleUser, Content: "hello"},
		{Role: provider.RoleAssistant, Content: "hi", ReasoningContent: ""},
	}
	plain, _ := json.Marshal((&client{model: "deepseek-v4", deepseek: true}).buildRequest(provider.Request{Messages: plainMsgs}).Messages)
	if strings.Contains(string(plain), "reasoning_content") {
		t.Errorf("plain assistant turn must not include reasoning_content: %s", plain)
	}
}

// TestStreamResponseAcceptsReasoningFieldAlias proves requirement 1: the
// streamResponse delta deserialises both "reasoning_content" (canonical) and
// "reasoning" (alias used by some OpenAI-compatible gateways).
func TestStreamResponseAcceptsReasoningFieldAlias(t *testing.T) {
	// Check struct tags directly.
	var sr streamResponse
	// JSON with "reasoning" field should populate Reasoning.
	if err := json.Unmarshal([]byte(`{"choices":[{"delta":{"reasoning":"hello world"}}]}`), &sr); err != nil {
		t.Fatalf("unmarshal: %v", err)
	}
	if got := sr.Choices[0].Delta.Reasoning; got != "hello world" {
		t.Errorf("Reasoning = %q, want hello world", got)
	}
	if got := sr.Choices[0].Delta.ReasoningContent; got != "" {
		t.Errorf("ReasoningContent = %q, want empty when only reasoning is set", got)
	}

	// JSON with "reasoning_content" should populate ReasoningContent.
	if err := json.Unmarshal([]byte(`{"choices":[{"delta":{"reasoning_content":"think deep"}}]}`), &sr); err != nil {
		t.Fatalf("unmarshal: %v", err)
	}
	if got := sr.Choices[0].Delta.ReasoningContent; got != "think deep" {
		t.Errorf("ReasoningContent = %q, want think deep", got)
	}
}

...
// RequiresReasoningReplay implements provider.ReasoningReplayProvider. It
// returns true only when the resolved protocol is DeepSeek thinking mode,
// which requires reasoning_content replayed on tool-call turns.
func (c *client) RequiresReasoningReplay() bool { return c.deepseek }
...
func (c *client) buildRequest(req provider.Request) chatRequest {
	...
	for i, m := range src {
		...
		// Using *string: nil omits the key (safe for non-DeepSeek / plain turns),
		// non-nil always renders the key even when empty (DeepSeek recovery-safe).
...
func (c *client) readStream(ctx context.Context, resp *http.Response, out chan<- provider.Chunk) (emitted bool, _ error) {
	...
	for scanner.Scan() {
		...
		if rc != "" {
			emitted = true
			if !sendChunk(ctx, out, provider.Chunk{Type: provider.ChunkReasoning, Text: rc}) {
				return emitted, ctx.Err()
			}
		}
		if delta.Content != "" {

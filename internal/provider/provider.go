...
// ReasoningReplayProvider is an optional Provider capability. When true, the
// provider requires reasoning_content to be replayed on multi-turn tool-call
// history so the model's thinking-mode chain-of-thought stays intact across
// turns (e.g. DeepSeek's "reasoning_content must be passed back" contract).
// The agent reads this via type assertion to decide whether to preserve
// original pre-transformation reasoning for replay and to warn on missing
// reasoning in tool-call turns. Providers that never require reasoning replay
// (including Anthropic — it uses signed thinking blocks instead) simply don't
// implement this interface; the agent treats missing as false.
type ReasoningReplayProvider interface {
...
func (e *AuthError) Error() string {
	...
	return fmt.Sprintf("authentication failed for provider %q (HTTP %d): %s is invalid or expired — update it (in .env or your environment) and retry, or run `WorkGround2 setup`",

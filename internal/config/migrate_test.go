...
func legacyHome(t *testing.T) (src, dest, home string) {
	...
	return filepath.Join(home, ".WorkGround2", "config.json"), userConfigPath(), home
}
...
func TestMigrateMCPToUserConfigOnUpgradeCollectsKnownSources(t *testing.T) {
	...
	writeLegacy(t, filepath.Join(filepath.Dir(dest), "WorkGround2.toml"), `
[[plugins]]
name = "legacy-toml"
...
`), 0o644); err != nil {
	...
	if err := os.WriteFile(filepath.Join(projectTOML, "WorkGround2.toml"), []byte(`
[[plugins]]
name = "project-toml"
...
`), 0o644); err != nil {
	...
	if err := os.WriteFile(filepath.Join(lateProject, "WorkGround2.toml"), []byte(`
[[plugins]]
name = "late"
command = "late-bin"
`), 0o644); err != nil {
...
func TestMigrateImportsLegacyV1TOMLBeforeJSON(t *testing.T) {
	srcJSON, dest, _ := legacyHome(t)
	legacyTOML := filepath.Join(filepath.Dir(dest), "WorkGround2.toml")
...
func TestMigrateImportsLegacyV1HomeTOMLBeforeJSON(t *testing.T) {
	...
	legacyTOML := filepath.Join(home, ".WorkGround2", "WorkGround2.toml")
	if err := os.MkdirAll(filepath.Dir(legacyTOML), 0o755); err != nil {
		t.Fatal(err)
	}
	if err := os.WriteFile(legacyTOML, []byte(`
...
func TestMigrateImportsLegacyXDGConfigToPrimaryConfig(t *testing.T) {
	...
	legacy := filepath.Join(home, ".config", "WorkGround2", "config.toml")
...
func TestMigrateSupportData(t *testing.T) {
	if runtime.GOOS == "windows" {
		t.Skip("skipping since legacyOSSupportDir equals current WorkGround2HomeDir on Windows")
	}
	home := t.TempDir()
	t.Setenv("HOME", home)
	t.Setenv("WorkGround2_CREDENTIALS_STORE", "file")

...
// canonical .WorkGround2 project dir is highest priority (last, since command.Load
...
func TestCommandDirsIncludeConventions(t *testing.T) {
	...
	for _, want := range []string{
		...
		filepath.Join(".WorkGround2", "commands"),
	} {
		if !strings.Contains(joined, want) {
			t.Errorf("CommandDirs missing %q\ngot:\n%s", want, joined)
		}
	}
	// The project's .workground2/commands must be the highest-priority (last) entry.
	if last := dirs[len(dirs)-1]; last != filepath.Join(".WorkGround2", "commands") {
		t.Errorf("project .workground2/commands should be highest priority (last), got %q", last)
	}
}

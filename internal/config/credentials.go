...
const (
	...
	credentialsKeyringService = "WorkGround2"
	credentialClearedPrefix   = "# WorkGround2-cleared "
)
...
// ResolveGlobalFirst resolves key from WorkGround2's global .env only. Repeated
...
func credentialsStoreMode() string {
	if mode := strings.TrimSpace(os.Getenv("WorkGround2_CREDENTIALS_STORE")); mode != "" {
...
func loadCredentialStoreForRoot(root string) {
	...
	if p := UserCredentialsPath(); p != "" {
		loadDotEnvFileAs(p, CredentialSource{Kind: CredentialSourceCredentials, Path: p, Label: "WorkGround2 credentials (.env)"})
	}
}

// StoreCredentialLines stores KEY=value assignments in WorkGround2's global .env
...
func pinCredentialAssignments(assignments map[string]string) {
	for key, value := range assignments {
		...
		recordCredentialSource(key, value, CredentialSource{Kind: CredentialSourceCredentials, Path: UserCredentialsPath(), Label: "WorkGround2 credentials (.env)"})
	}
}
...
func credentialSourceLabel(source CredentialSource) string {
	...
	case CredentialSourceCredentials:
		return "WorkGround2 credentials"
	case CredentialSourceHomeEnv:
		return "home .env"
	case CredentialSourceLegacy:
		return "legacy WorkGround2 credentials"
...
func storedCredentialValue(key string) (string, CredentialSource, bool) {
	if p := UserCredentialsPath(); p != "" {
		if value, ok := envFileValue(p, key); ok && value != "" {
			return value, CredentialSource{Kind: CredentialSourceCredentials, Path: p, Label: "WorkGround2 credentials (.env)"}, true
		}
	}
	return "", CredentialSource{}, false
}

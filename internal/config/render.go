...
// RenderTOML renders the config as annotated TOML in the `WorkGround2 setup` house style:
...
// WorkGround2.toml stays focused on project behavior and intentionally excludes
...
func RenderTOMLForScope(c *Config, scope RenderScope) string {
	...
	b.WriteString("# WorkGround2 configuration.\n")
	fmt.Fprintf(&b, "# Resolution order: flag > ./WorkGround2.toml > %s > built-in defaults.\n", userConfigDisplayPath())
	b.WriteString("# Fields marked user/global only are not overridden by ./WorkGround2.toml.\n")
	b.WriteString("# Secrets are named via api_key_env and stored in WorkGround2's global .env; never put keys here.\n\n")

	fmt.Fprintf(&b, "config_version = %d   # schema marker for diagnostics; old versions may ignore it\n", configVersion(c))
	fmt.Fprintf(&b, "default_model = %q\n", c.DefaultModel)
	if c.Language != "" {
		fmt.Fprintf(&b, "language      = %q   # ui/model language; empty = auto-detect from $LANG / $WorkGround2_LANG\n", c.Language)
	} else {
		b.WriteString("# language      = \"zh\"   # ui/model language; empty = auto-detect from $LANG / $WorkGround2_LANG\n")
	}
	if scope != RenderScopeProject {
		fmt.Fprintf(&b, "credentials_store = %q   # legacy compatibility; provider keys are saved in WorkGround2's global .env\n", normalizeCredentialsStore(c.CredentialsStore))
	}
	b.WriteString("\n")

	if shouldRenderUI(c, defaults, scope) {
		b.WriteString("[ui]\n")
		fmt.Fprintf(&b, "theme = %q   # auto|dark|light; CLI colors only; WorkGround2_THEME can override per run\n", c.UITheme())
		if style := c.UIThemeStyle(); style != "" {
			fmt.Fprintf(&b, "theme_style = %q   # CLI accent palette; WorkGround2_THEME_STYLE can override per run\n", style)
		} else {
			b.WriteString("# theme_style = \"graphite\"   # graphite|aurora|slate|carbon|nocturne|amber|iris and legacy aliases\n")
		}
	...
	if scope != RenderScopeProject {
		...
		if style := c.DesktopThemeStyle(); style != "" {
			...
			b.WriteString("# theme_style = \"iris\"   # graphite|aurora|slate|carbon|nocturne|amber|iris and legacy aliases\n")
		}
		fmt.Fprintf(&b, "close_behavior = %q   # desktop: quit|background when the window close button is clicked\n", c.DesktopCloseBehavior())
		fmt.Fprintf(&b, "composer_submit_key = %q   # desktop composer send shortcut: enter|ctrl_enter\n", c.DesktopComposerSubmitKey())
	...
	if shouldRenderNetwork(c, defaults, scope) {
		...
		if c.Network.Proxy.Password != "" {
			...
			b.WriteString("# password = \"${WorkGround2_PROXY_PASSWORD}\"   # optional; supports ${VAR} expansion\n")
		}
		b.WriteString("\n")
	}
	...
	if shouldRenderProviders(c, defaults, scope) {
		for _, p := range c.Providers {
			...
			fmt.Fprintf(&b, "kind        = %q\n", p.Kind)
			if p.BaseURL != "" || !strings.EqualFold(strings.TrimSpace(p.Kind), "cli") {
			...
			if p.Command != "" {
				fmt.Fprintf(&b, "command     = %q   # local CLI executable for kind=\"cli\"\n", p.Command)
			}
			if len(p.Args) > 0 {
				fmt.Fprintf(&b, "args        = %s   # one argument per array item; no shell splitting\n", renderStringArray(p.Args))
			}
			if p.Protocol != "" {
				fmt.Fprintf(&b, "protocol    = %q   # cli stdout protocol: text|json|jsonl\n", p.Protocol)
			}
			if p.TimeoutSeconds > 0 {
				fmt.Fprintf(&b, "timeout_seconds = %d   # local CLI request timeout\n", p.TimeoutSeconds)
			}
			if len(p.Models) > 0 {
			...
			if p.APIKeyEnv != "" || !strings.EqualFold(strings.TrimSpace(p.Kind), "cli") {
	...
	if len(c.Plugins) == 0 {
		...
		b.WriteString("# command = \"WorkGround2-plugin-example\"\n")
...
// active TOML array; an empty one renders as a commented example so `WorkGround2 setup`

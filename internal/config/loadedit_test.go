...
`
	...
	cfg := LoadForEdit(path)
	if cfg.DefaultModel != "local/中文模型" || len(cfg.Providers) != 1 || cfg.Providers[0].Model != "中文模型" {
		t.Fatalf("decoded config = default %q providers %+v", cfg.DefaultModel, cfg.Providers)
	}
}
...
func TestLoadForEditIgnoresProjectDotEnvForProviderCredentials(t *testing.T) {
	...
	launch := t.TempDir()
	home := t.TempDir()
	path := filepath.Join(project, "WorkGround2.toml")

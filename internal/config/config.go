// Package config loads WorkGround2's runtime configuration from TOML. Resolution order:
// flag > project ./WorkGround2.toml > user config.toml (in the OS user-config dir) > built-in defaults.
...
// Config is WorkGround2's runtime configuration.
type Config struct {
	ConfigVersion    int                 `toml:"config_version"`
	DefaultModel     string              `toml:"default_model"`
	Language         string              `toml:"language"` // ui/model language tag (e.g. "zh"); empty = auto-detect from $LANG / $WorkGround2_LANG
...
func normalizeThemeStyle(style string) string {
	...
	case "graphite", "aurora", "slate", "carbon", "nocturne", "amber", "iris", "ember", "midnight", "sandstone", "porcelain", "linen", "glacier":
...
// NormalizeComposerSubmitKey returns the canonical desktop composer submit
// shortcut. Missing and unknown values preserve the historical Enter-to-send
// behavior.
func NormalizeComposerSubmitKey(mode string) string {
	switch strings.ToLower(strings.TrimSpace(mode)) {
...
// DesktopComposerSubmitKey normalizes the desktop composer submit shortcut.
func (c *Config) DesktopComposerSubmitKey() string {
	if c == nil {
		return "enter"
	}
	return NormalizeComposerSubmitKey(c.Desktop.ComposerSubmitKey)
}
...
type ServeConfig struct {
	...
	// Generate one with: WorkGround2 serve --hash-password --password '...'
...
// the project roots (.workground2/.agents/.agent/.claude under the workspace) and
...
type ProviderEntry struct {
	...
	// V4 Pro currently exposes high|max.
...
// WorkGround2.toml's [[plugins]] or a project-root .mcp.json (see loadMCPJSON).
type PluginEntry struct {
	...
	// TrustedReadOnlyTools names raw MCP tool names that WorkGround2 should treat as
...
func Default() *Config {
	return &Config{
		...
		// Mode "ask" with no rules keeps `WorkGround2 run` autonomous (no TTY → ask
		// resolves to allow) while `WorkGround2` prompts before writers. Users add
		...
		Bot: BotConfig{
			...
			Control:            BotControlConfig{Addr: "127.0.0.1:37913", TokenEnv: "WORKGROUND2_BOT_CONTROL_TOKEN"},
...
// not call this; loaded provider entries still resolve only from WorkGround2's
// global .env.
...
func (e *ProviderEntry) RequiresAPIKey() bool {
	...
	if strings.EqualFold(strings.TrimSpace(e.Kind), "cli") {
		return false
	}
...
func (c *Config) Validate(model string) error {
	...
	if strings.EqualFold(strings.TrimSpace(e.Kind), "cli") {
		if strings.TrimSpace(e.Command) == "" {
			return fmt.Errorf("provider %q: command is required", model)
		}
	} else if e.BaseURL == "" {

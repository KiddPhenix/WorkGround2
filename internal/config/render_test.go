...
func expectedDefaultWorkGround2Home(home string) string {
...
func TestUserConfigDisplayPathCollapsesHome(t *testing.T) {
	...
	if !strings.HasSuffix(strings.ToLower(got), "workground2/config.toml") {
		t.Fatalf("display path = %q, want workground2/config.toml suffix", got)
	}
...
func TestUserConfigPathUsesWorkGround2Home(t *testing.T) {
	home := isolateUserConfigHome(t)
	want := filepath.Join(expectedDefaultWorkGround2Home(home), "config.toml")
	if got := UserConfigPath(); filepath.Clean(got) != filepath.Clean(want) {
		t.Fatalf("UserConfigPath() = %q, want %q", got, want)
	}
}

func TestUserConfigPathHonorsWorkGround2Home(t *testing.T) {
	home := isolateUserConfigHome(t)
	custom := filepath.Join(home, "custom-home")
	t.Setenv("WorkGround2_HOME", custom)
...
func TestLoadForRootUsesWindowsHomeFallbackWhenConfigDirUnavailable(t *testing.T) {
	...
	t.Setenv("WorkGround2_HOME", "")

	configPath := filepath.Join(home, "AppData", "Roaming", "WorkGround2", "config.toml")
...
func TestRenderTOMLRoundTrips(t *testing.T) {
	...
	orig.Network = NetworkConfig{
		...
		Proxy: NetworkProxyConfig{
			...
			Port:     7890,
			Username: "user",
			Password: "${WorkGround2_PROXY_PASSWORD}",
		},
	}
	...
	orig.Plugins = []PluginEntry{
		{Name: "example", Command: "WorkGround2-plugin-example"},
	...
	if len(got.Bot.Connections) != 1 || got.Bot.Connections[0].Model != "deepseek-pro" || got.Bot.Connections[0].WorkspaceRoot != "/tmp/WorkGround2-bot" {
...
func TestScopedRenderSeparatesUserAndProjectConfig(t *testing.T) {
	...
	for _, want := range []string{"config_version = 3", "[desktop]", `theme = "dark"`, `close_behavior = "background"`, `composer_submit_key = "enter"`, `status_bar_style = "text"`, `default_tool_approval_mode = "auto"`, `check_updates = false`, "[notifications]", "[tools.shell]"} {
...
func TestRenderTOMLRoundTripsProviderHeadersAndModelOverrides(t *testing.T) {
	...
	rendered := RenderTOML(orig)
	if !strings.Contains(rendered, `headers     = { HTTP-Referer = "https://app.example", X-Title = "WorkGround2" }`) {
	...
	if p.Headers["HTTP-Referer"] != "https://app.example" || p.Headers["X-Title"] != "WorkGround2" {
...
func TestRenderTOMLPreservesDesktopDisplayMode(t *testing.T) {
	...
	if err := c.SetDesktopComposerSubmitKey("ctrl_enter"); err != nil {
	...
	if !strings.Contains(rendered, `composer_submit_key = "ctrl_enter"`) {
		t.Fatalf("rendered user config missing composer_submit_key:\n%s", rendered)
	}
	var got Config
	if _, err := toml.Decode(rendered, &got); err != nil {
		t.Fatalf("rendered TOML does not parse: %v\n---\n%s", err, rendered)
	...
	if got.DesktopComposerSubmitKey() != "ctrl_enter" {
		t.Fatalf("composer_submit_key after round trip = %q, want ctrl_enter", got.DesktopComposerSubmitKey())
	}
}
...
func TestRenderTOMLDefaultStepsDoNotOverrideGlobalConfig(t *testing.T) {
	...
	projectPath := filepath.Join(projectDir, "WorkGround2.toml")

...
func TestSetDesktopComposerSubmitKey(t *testing.T) {
	c := Default()
	if got := c.DesktopComposerSubmitKey(); got != "enter" {
		t.Fatalf("default DesktopComposerSubmitKey = %q, want enter", got)
	}
	if err := c.SetDesktopComposerSubmitKey("ctrl+enter"); err != nil {
		t.Fatalf("SetDesktopComposerSubmitKey(ctrl+enter): %v", err)
	}
	if got := c.DesktopComposerSubmitKey(); got != "ctrl_enter" {
		t.Fatalf("DesktopComposerSubmitKey = %q, want ctrl_enter", got)
	}
	if err := c.SetDesktopComposerSubmitKey("maybe"); err == nil {
		t.Fatal("expected error for invalid composer submit key")
	}
}
...
func TestPluginMutators(t *testing.T) {
	...
	if err := c.UpsertPlugin(PluginEntry{Name: "ex", Command: "WorkGround2-plugin-example"}); err != nil {
...
func TestSaveToRoundTrips(t *testing.T) {
	...
	path := filepath.Join(t.TempDir(), "nested", "WorkGround2.toml")
...
func TestSaveToScopesUserAndProjectFiles(t *testing.T) {
	...
	projectPath := filepath.Join(t.TempDir(), "WorkGround2.toml")

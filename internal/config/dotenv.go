...
// loadDotEnv loads WorkGround2's global .env for provider credentials. The
...
// expansion, then loads WorkGround2's global .env for provider credentials.
...
// other and project files cannot redirect WorkGround2's own config/credential
// paths.
...
func isProjectDotEnvControlKey(key string) bool {
	...
	if strings.HasPrefix(upper, "WORKGROUND2_") {
		return true
	}
	switch upper {

...
// separate from the `WorkGround2 setup` wizard (cli) so a GUI can apply one setting at a
...
// SetLanguage pins the CLI UI/model language; empty/auto clears the override so runtime detection falls back to WorkGround2_LANG / locale.
...
func (c *Config) SetDesktopAppearance(theme, style string) error {
	...
	if normalized == "" {
		return fmt.Errorf("desktop theme style %q: must be graphite|aurora|slate|carbon|nocturne|amber|iris", style)
	}
...
func (c *Config) SetDesktopSessionBackground(next DesktopSessionBackgroundConfig) error {
	...
	if mode == "" {
		if strings.TrimSpace(next.Mode) != "" {
			return fmt.Errorf("desktop Session background mode %q: must be pattern|solid|waves|aurora|nebula|starfield|blackhole|moonclouds|biolume|dunes|custom", next.Mode)
		}
...
// SetDesktopComposerSubmitKey sets the composer send shortcut. UI-only.
func (c *Config) SetDesktopComposerSubmitKey(mode string) error {
	switch strings.ToLower(strings.TrimSpace(mode)) {
	case "", "enter":
		c.Desktop.ComposerSubmitKey = "enter"
	case "ctrl_enter", "ctrl-enter", "ctrl+enter", "control_enter", "control-enter", "control+enter":
		c.Desktop.ComposerSubmitKey = "ctrl_enter"
		return nil
	default:
		return fmt.Errorf("composer submit key %q: must be enter|ctrl_enter", mode)
	}
	return nil
}
...
func validateProvider(e ProviderEntry) error {
	...
	case strings.EqualFold(strings.TrimSpace(e.Kind), "cli") && strings.TrimSpace(e.Command) == "":
		return fmt.Errorf("provider %q: command is required", e.Name)
	case !strings.EqualFold(strings.TrimSpace(e.Kind), "cli") && strings.TrimSpace(e.BaseURL) == "":
...
// .mcp.json-only server would otherwise be serialized into WorkGround2.toml or the
...
func pluginTOMLSourcePathForRoot(root, name string) string {
	projectTOML := "WorkGround2.toml"
	if resolved := resolveRoot(root); resolved != "." {
		projectTOML = filepath.Join(resolved, "WorkGround2.toml")
	}
...
// half-written WorkGround2.toml that fails to parse on next load. Parent directories
// are created as needed.
//
// For project configs (./WorkGround2.toml) the write is incremental: only sections
...
func SaveMinimalProjectReasoningLanguage(path, lang string) (string, error) {
	...
	body := fmt.Sprintf(`# WorkGround2 project configuration.
...
func replaceTOMLSection(body, sectionName, newContent string) string {
	...
	for _, span := range spans {
		...
		// Field-level merge for regular sections: update only the keys present in
		// newContent while preserving all other keys, comments, and formatting.
		// Wholesale section replacement would drop fields that are set in the
		// project WorkGround2.toml but at default in the runtime (merged) config.
		headerLen := len(span.text)
		existingContent := body[span.start+headerLen : end]
		merged := mergeSectionFields(existingContent, newContent)
		return body[:span.start] + span.text + merged + body[end:]
	}
	return strings.TrimRight(body, "\n") + "\n\n" + newContent
}

// mergeSectionFields merges the key=value pairs from delta into existing section
// content (the text between the [section] header and the next section). Keys
// present in delta replace their counterpart in existing; keys not found are
// appended at the end. Comments, blank lines, and formatting in existing are
// preserved. This prevents the parent call (saveProjectIncremental) from dropping
// project-level fields that happen to be at default in the runtime merged config.
func mergeSectionFields(existingContent, delta string) string {
	if existingContent == "" {
		return delta
	}
	if delta == "" {
	...
	// Parse delta into ordered key→full-text entries, supporting multi-line
	// values (e.g. system_prompt = """...").
	...
	for _, line := range strings.Split(delta, "\n") {
		if k, ok := tomlTopLevelKey(line); ok && k != "" {
			entries = append(entries, deltaEntry{key: k, text: ""})
		}
		if len(entries) > 0 {
			entries[len(entries)-1].text += line + "\n"
		}
	}
	if len(entries) == 0 {
		return existingContent
	...
	// Build a set of delta keys for fast lookup.
	keysInDelta := make(map[string]int, len(entries))
	for i, e := range entries {
		keysInDelta[e.key] = i
	}

	// Scan existingContent line-by-line and record spans for each key that
	// matches a delta key. A span covers the key line through the line before
	// the next key (or end). Process from last to first so replacement
	// positions stay valid as the result length changes.
	type keySpan struct{ start, end int }
	var spans []keySpan
	existingLines := strings.Split(existingContent, "\n")
	for i := 0; i < len(existingLines); i++ {
		line := strings.TrimSpace(existingLines[i])
		if k, ok := tomlTopLevelKey(line); ok && k != "" {
			if _, inDelta := keysInDelta[k]; inDelta {
				start := i
				i++
				for i < len(existingLines) {
					if _, ok := tomlTopLevelKey(strings.TrimSpace(existingLines[i])); ok {
						break
					}
					i++
				}
				spans = append(spans, keySpan{start: start, end: i})
				i-- // loop increment will advance past the non-key line
			}
		}
	}

	// Apply replacements bottom-up to keep line indices valid.
	result := make([]string, len(existingLines))
	copy(result, existingLines)
	for s := len(spans) - 1; s >= 0; s-- {
		ks := spans[s]
		keyName := ""
		if k, ok := tomlTopLevelKey(strings.TrimSpace(existingLines[ks.start])); ok {
			keyName = k
		}
		entryIdx := keysInDelta[keyName]
		newLines := strings.Split(entries[entryIdx].text, "\n")
		if len(newLines) > 0 && newLines[len(newLines)-1] == "" {
			newLines = newLines[:len(newLines)-1]
		}
		result = append(result[:ks.start], append(newLines, result[ks.end:]...)...)
		delete(keysInDelta, keyName)
	}

	// Append any delta keys that were not found in existing content.
	if len(keysInDelta) > 0 {
		var b strings.Builder
		b.WriteString(strings.Join(result, "\n"))
		if len(result) > 0 && result[len(result)-1] != "" {
			b.WriteString("\n")
		}
		for _, e := range entries {
			if _, found := keysInDelta[e.key]; found {
...
// (SourcePath), or to ./WorkGround2.toml when none exists yet — the conventional
// project-local target a fresh GUI session would create.
func (c *Config) Save() error {
	path := SourcePath()
	if path == "" {
		path = "WorkGround2.toml"
	}
	return c.SaveTo(path)
}

// SaveForRoot saves root's project config when it exists, falling back to the
// user's global config when root has no WorkGround2.toml. Existing project files

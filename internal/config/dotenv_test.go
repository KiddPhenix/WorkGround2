...
// TestLoadDotEnvReadsGlobalCredentials proves `WorkGround2 setup`'s target — the
// WorkGround2-owned credentials file under WorkGround2 home — is loaded from any
...
func TestLoadDotEnvDecodesGB18030Credentials(t *testing.T) {
	...
	t.Setenv("WorkGround2_HOME", filepath.Join(home, "workground2-home"))
	t.Setenv("WorkGround2_CREDENTIALS_STORE", "file")
	t.Setenv("PINNED_CN", "")
...
func TestLoadForRootFiltersProjectDotEnvControlVars(t *testing.T) {
	...
	t.Setenv("XDG_CONFIG_HOME", filepath.Join(home, ".config"))
	t.Setenv("AppData", filepath.Join(home, "AppData"))
	t.Setenv("WorkGround2_STATE_HOME", "")
	os.Unsetenv("WorkGround2_STATE_HOME")

	wantCred := UserCredentialsPath()
	if wantCred == "" {
		t.Skip("user credentials path unavailable")
	}
	if err := os.WriteFile(filepath.Join(project, ".env"), []byte("WorkGround2_STATE_HOME="+redirect+"\nPLUGIN_TOKEN=project-token\n"), 0o600); err != nil {
		t.Fatal(err)
	}
	if err := os.WriteFile(filepath.Join(project, ".mcp.json"), []byte(`{
  "mcpServers": {
    "svc": {
      ...
      "headers": {
        ...
        "State": "${WorkGround2_STATE_HOME:-default-state}"
      }
    }
  }
}`), 0o644); err != nil {
	...
	if got := os.Getenv("WorkGround2_STATE_HOME"); got != "" {
...
func TestCredentialResolverCachesGlobalFirstLookups(t *testing.T) {
	...
	stubStoredCredentialValueForTest(t, func(got string) (string, CredentialSource, bool) {
		...
		return "from_credentials", CredentialSource{Kind: CredentialSourceCredentials, Label: "WorkGround2 credentials"}, true
	})
...
func TestResolveCredentialSourceShowsCredentialsShadowingProjectEnv(t *testing.T) {
	...
	if !got.Set || got.Source.Kind != CredentialSourceCredentials {
		t.Fatalf("source = %+v set=%v, want WorkGround2 credentials", got.Source, got.Set)
	}
...
func TestProjectConfigCannotOverrideCredentialStoreMode(t *testing.T) {
	...
	if err := os.WriteFile(filepath.Join(project, "WorkGround2.toml"), []byte(`credentials_store = "keyring"`), 0o644); err != nil {
		t.Fatal(err)
	}
...
// TestLoadDotEnvGlobalCredentialsOverrideEnv confirms WorkGround2-owned global

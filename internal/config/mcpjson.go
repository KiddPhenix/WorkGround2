...
// mcpJSONFile is the project-root file Claude Code calls .mcp.json. WorkGround2 reads
...
// stdio server; type/url/headers describe a remote one. WorkGround2 also accepts
...
// legacyConfigPath is the v0.x (TypeScript line) config file, ~/.workground2/config.json.
func legacyConfigPath() string {
	home, err := os.UserHomeDir()
	if err != nil {
		return ""
	}
	return filepath.Join(home, ".WorkGround2", "config.json")
}

// loadLegacyMCP reads the v0.x ~/.workground2/config.json and returns its enabled
...
// already declare. WorkGround2.toml's [[plugins]] win on a name collision: it is the
// WorkGround2-specific, more explicit of the two, so it overrides the shared,

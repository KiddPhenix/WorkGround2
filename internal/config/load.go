...
// the v0.x ~/.workground2/config.json's mcpServers. Provider api_key_env values
// resolve from WorkGround2's global .env, not from project .env files.
...
// each project's WorkGround2.toml + .mcp.json are resolved independently without
// changing the process cwd, while provider keys stay rooted in WorkGround2 home.
func LoadForRoot(root string) (*Config, error) {
	...
	// WorkGround2.toml wins on a name collision (see mergeMCPJSON).
	mcpFile := mcpJSONFile
	if root != "." {
	...
	// Lowest priority: the v0.x ~/.workground2/config.json's mcpServers, so upgrading
...
func officialProviderKind(p *ProviderEntry) string {
	...
	if strings.EqualFold(u.Hostname(), "generativelanguage.googleapis.com") {
		return "google"
	}
	if strings.EqualFold(u.Hostname(), "api.groq.com") {
		return "groq"
	}
	return ""
}
...
// LoadForEdit returns a config to seed the `WorkGround2 setup` wizard when reconfiguring:
...
// of resetting to defaults. WorkGround2's global .env is loaded so api_key_env
...
func LoadForEdit(path string) *Config {
	...
	// The file exists but failed to parse — backing up before we fall back to
	// defaults so the user's settings are never silently lost.
	backupPath := backupConfigFile(path)
	slog.Error("config: load for edit failed, using defaults — original saved to backup",
		"path", path, "backup", backupPath, "err", err)
...
func LoadForEditWithoutCredentials(path string) *Config {
	...
	backupPath := backupConfigFile(path)
	slog.Error("config: load for edit (no credentials) failed, using defaults — original saved to backup",
		"path", path, "backup", backupPath, "err", err)
	cfg = Default()
	normalizeConfigForEdit(cfg)
	return cfg
...
// backupConfigFile copies path to path.bak when path exists and is readable.
// Returns the backup path on success, or empty string when backing up wasn't
// possible (file missing, unreadable, or copy failed).
func backupConfigFile(path string) string {
	if path == "" {
		return ""
	}
	...
	if err := os.WriteFile(backupPath, data, 0o600); err != nil {
		slog.Warn("config: failed to write backup", "path", backupPath, "err", err)
		return ""
	}
	return backupPath
}
...
// MiMo providers needed by legacy refs that live outside WorkGround2.toml, such as
...
func providerEntryMatchesCanonicalOfficialAccess(p *ProviderEntry, canonical string) bool {
	...
	case "openai":
		return officialProviderKind(p) == "openai"
	case "anthropic":
		return officialProviderKind(p) == "anthropic"
	case "google":
		return officialProviderKind(p) == "google"
	case "groq":
		return officialProviderKind(p) == "groq"
	default:
		return false
	}
}
...
func ensureOfficialProviders(c *Config) {
	ensureDeepSeekOfficialProviderLegacy(c)
}

func ensureDeepSeekOfficialProviderLegacy(c *Config) {

...
func TestRunSkillInline(t *testing.T) {
	home := t.TempDir()
	writeSkill(t, home, ".workground2/skills/note.md", "---\ndescription: take a note\n---\nDo the thing.")
...
func TestReadOnlySkillSubagentResolvesProfile(t *testing.T) {
	home := t.TempDir()
	writeSkill(t, home, ".workground2/skills/deep.md", "---\ndescription: deep\nrunAs: subagent\nmodel: deepseek-pro\neffort: max\n---\nbody")
	tl := NewReadOnlySkillTool(New(Options{HomeDir: home, DisableBuiltins: true}), nil)
...
func TestInstallSkill(t *testing.T) {
	...
	wantPath := filepath.Join(home, ".WorkGround2", "skills", "deploy", SkillFile)
	...
	if _, err := os.Stat(filepath.Join(home, ".WorkGround2", "skills", "deploy.md")); !errors.Is(err, os.ErrNotExist) {
...
func TestReadSkillBlocksProtectedRawBody(t *testing.T) {
	home := t.TempDir()
	writeSkill(t, home, ".workground2/skills/secret.md", "---\ndescription: protected\nprotected: true\nantiLeak: true\n---\nDo not show this body.")
	tl := NewReadSkillTool(New(Options{HomeDir: home, DisableBuiltins: true}))

	out, err := tl.Execute(context.Background(), json.RawMessage(`{"name":"secret"}`))
	if err != nil {
		t.Fatalf("execute: %v", err)
	}
	if strings.Contains(out, "Do not show this body.") {
		t.Fatalf("read_skill leaked protected body:\n%s", out)
	}
	if !strings.Contains(out, "raw source/body access is blocked") {
		t.Fatalf("read_skill should explain protected blocking, got:\n%s", out)
	}
}

func TestRunSkillProtectedInlineCarriesPolicyAndSentinel(t *testing.T) {
	home := t.TempDir()
	writeSkill(t, home, ".workground2/skills/secret.md", "---\ndescription: protected\nprotected: true\nantiLeak: true\n---\nFollow protected steps.")
	tl := NewRunSkillTool(New(Options{HomeDir: home, DisableBuiltins: true}), nil)

	out, err := tl.Execute(context.Background(), json.RawMessage(`{"name":"secret","arguments":"do it"}`))
	if err != nil {
		t.Fatalf("execute: %v", err)
	}
	for _, want := range []string{"<protected-skill-pin", "<protected-skill-policy>", "Follow protected steps.", "Arguments: do it"} {
		if !strings.Contains(out, want) {
			t.Fatalf("protected inline output missing %q:\n%s", want, out)
		}
	}
	if strings.Contains(out, "<skill-pin") {
		t.Fatalf("protected skills should not use regular skill-pin:\n%s", out)
	}
}

func TestProtectedSkillRedactionAndFingerprint(t *testing.T) {
	body := "alpha beta gamma " + strings.Repeat("protected source line with enough unique material ", 8)
	sk := Skill{Name: "secret", Description: "protected", Body: body, Protected: true, AntiLeak: true}
	rendered := Render(sk, "")

	redacted := RedactProtectedContent(rendered)
	if strings.Contains(redacted, "protected source line") {
		t.Fatalf("redaction leaked body:\n%s", redacted)
	}
	if !strings.Contains(redacted, ProtectedSkillRedaction) || !strings.Contains(redacted, "secret") {
		t.Fatalf("redaction missing placeholder/name:\n%s", redacted)
	}
	if !ProtectedOutputLeaks(rendered, "Here is the source:\n"+body) {
		t.Fatal("fingerprint should block a large protected body copy")
	}
	if ProtectedOutputLeaks(rendered, "I used the protected skill and completed the task.") {
		t.Fatal("fingerprint should allow ordinary completion text")
	}
	if !ProtectedOutputLeaks(rendered, `api_key = "abcdefghijklmnopqrstuvwxyz123456"`) {
		t.Fatal("credential-like output should be blocked in protected context")
	}
}

...
func TestBuiltinReviewAndSecurityReviewAreReadOnly(t *testing.T) {
	...
	for _, name := range []string{"review", "security-review"} {
		...
		if sk.RunAs != RunSubagent {
			t.Errorf("builtin %q should be RunSubagent, got %s", name, sk.RunAs)
		}
		if !sk.ReadOnly {
			t.Errorf("builtin %q should be ReadOnly=true", name)
		}
	}
}

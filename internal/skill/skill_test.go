...
func TestListPrecedenceProjectOverGlobal(t *testing.T) {
	home := t.TempDir()
	proj := t.TempDir()
	writeSkill(t, proj, ".workground2/skills/greet.md", "---\nname: greet\ndescription: project greet\n---\nproject body")
	writeSkill(t, home, ".workground2/skills/greet.md", "---\ndescription: global greet\n---\nglobal body")
	writeSkill(t, home, ".workground2/skills/onlyglobal.md", "---\ndescription: only global\n---\nbody")
...
func TestListDecodesGB18030SkillFile(t *testing.T) {
	home := t.TempDir()
	root := t.TempDir()
	path := filepath.Join(root, "cn", SkillFile)
...
func TestFlatAndDirLayout(t *testing.T) {
	home := t.TempDir()
	writeSkill(t, home, ".workground2/skills/flat.md", "---\ndescription: flat\n---\nflat body")
	writeSkill(t, home, ".workground2/skills/dir/SKILL.md", "---\ndescription: dir\n---\ndir body")
...
func TestNestedSkillsDiscoveredByDefault(t *testing.T) {
	home := t.TempDir()
	writeSkill(t, home, ".workground2/skills/superpower/skill-a.md", "---\ndescription: nested flat\n---\nflat body")
	writeSkill(t, home, ".workground2/skills/superpower/tool-a/SKILL.md", "---\ndescription: nested dir\n---\ndir body")
	writeSkill(t, home, ".workground2/skills/superpower/references/notes.md", "---\ndescription: not a skill\n---\nnotes")
...
func TestNestedSkillsRequireDescription(t *testing.T) {
	home := t.TempDir()
	writeSkill(t, home, ".workground2/skills/root.md", "---\n---\nroot body")
	writeSkill(t, home, ".workground2/skills/superpower/draft.md", "---\n---\ndraft body")
	writeSkill(t, home, ".workground2/skills/superpower/tool/SKILL.md", "---\n---\ntool body")
...
func TestNestedDirectorySkillStopsTraversal(t *testing.T) {
	home := t.TempDir()
	writeSkill(t, home, ".workground2/skills/pack/SKILL.md", "---\ndescription: pack\n---\npack body")
	writeSkill(t, home, ".workground2/skills/pack/child.md", "---\ndescription: child\n---\nchild body")
...
func TestWorkGround2HomeDirOverridesGlobalWorkGround2Skills(t *testing.T) {
	home := t.TempDir()
	WorkGround2Home := filepath.Join(t.TempDir(), "rx-home")
	writeSkill(t, home, ".workground2/skills/old.md", "---\ndescription: old\n---\nold")
	writeSkill(t, home, ".workground2/skills/current.md", "---\ndescription: old current\n---\nold current")
	currentPath := writeSkill(t, WorkGround2Home, "skills/current.md", "---\ndescription: current\n---\ncurrent")

	st := New(Options{HomeDir: home, WorkGround2HomeDir: WorkGround2Home, DisableBuiltins: true})
	list := st.List()
	current, ok := find(list, "current")
	if !ok {
		t.Fatal("WorkGround2 home skill should be discovered")
	}
	if current.Path != currentPath {
		t.Fatalf("current skill path = %q, want WorkGround2 home path %q", current.Path, currentPath)
	}
	if _, ok := find(list, "old"); !ok {
		t.Fatal("legacy ~/.WorkGround2 skill should remain discoverable")
	}
	...
	want := filepath.Join(WorkGround2Home, SkillsDirname, "created", SkillFile)
	if path != want {
...
func TestExcludedPathsHideConventionRoots(t *testing.T) {
	home := t.TempDir()
	writeSkill(t, home, ".workground2/skills/keep.md", "---\ndescription: keep\n---\nb")
...
func TestFrontmatterFields(t *testing.T) {
	home := t.TempDir()
	writeSkill(t, home, ".workground2/skills/sub.md",
		"---\ndescription: a sub\nrunAs: subagent\nallowed-tools: read_file, grep\nmodel: deepseek-pro\n---\nbody")
	writeSkill(t, home, ".workground2/skills/fork.md", "---\ndescription: f\ncontext: fork\n---\nbody")
	writeSkill(t, home, ".workground2/skills/plain.md", "---\ndescription: p\n---\nbody")
...
func TestReferencesInlined(t *testing.T) {
	home := t.TempDir()
	writeSkill(t, home, ".workground2/skills/withrefs/SKILL.md", "---\ndescription: r\n---\nmain body")
	writeSkill(t, home, ".workground2/skills/withrefs/references/b.md", "second ref")
	writeSkill(t, home, ".workground2/skills/withrefs/references/a.md", "first ref")
...
func TestScriptsAppended(t *testing.T) {
	...
	writeSkill(t, home, ".workground2/skills/withscripts/SKILL.md", "---\ndescription: r\n---\nmain body")
	writeScript(t, home, ".workground2/skills/withscripts/scripts/lint.py", "#!/usr/bin/env python3\nprint('ok')")
	writeScript(t, home, ".workground2/skills/withscripts/scripts/deploy.sh", "#!/usr/bin/env bash\necho ok")
...
func TestNoScriptsWhenDirAbsent(t *testing.T) {
	home := t.TempDir()
	writeSkill(t, home, ".workground2/skills/noscripts/SKILL.md", "---\ndescription: r\n---\nmain body")
	st := New(Options{HomeDir: home, DisableBuiltins: true})
...
func TestScriptsFilteredByExt(t *testing.T) {
	home := t.TempDir()
	writeSkill(t, home, ".workground2/skills/scriptscheck/SKILL.md", "---\ndescription: t\n---\nbody")
	writeScript(t, home, ".workground2/skills/scriptscheck/scripts/lint.py", "#!/usr/bin/env python3\nprint('ok')\n")
	writeScript(t, home, ".workground2/skills/scriptscheck/scripts/.hidden.py", "")
	writeScript(t, home, ".workground2/skills/scriptscheck/scripts/readme.md", "# readme")
	writeScript(t, home, ".workground2/skills/scriptscheck/scripts/deploy", "#!/bin/sh\necho ok")
	writeScript(t, home, ".workground2/skills/scriptscheck/scripts/legacy.p", "print 'ok'\n")
	writeScript(t, home, ".workground2/skills/scriptscheck/scripts/.gitkeep", "")
...
func TestBuiltinsPresentAndOverridable(t *testing.T) {
	...
	home := t.TempDir()
	writeSkill(t, home, ".workground2/skills/explore.md", "---\ndescription: mine\nrunAs: inline\n---\nbody")
...
func TestDisabledSkillsAreFilteredFromListAndRead(t *testing.T) {
	home := t.TempDir()
	writeSkill(t, home, ".workground2/skills/active.md", "---\ndescription: active\n---\nbody")
	writeSkill(t, home, ".workground2/skills/hidden.md", "---\ndescription: hidden\n---\nbody")
...
func TestInvalidNamesSkipped(t *testing.T) {
	home := t.TempDir()
	writeSkill(t, home, ".workground2/skills/bad name.md", "---\ndescription: x\n---\nb") // space → invalid
	st := New(Options{HomeDir: home, DisableBuiltins: true})
	if len(st.List()) != 0 {
...
func TestSymlinkedDirAndFile(t *testing.T) {
	...
	skillsRoot := filepath.Join(home, ".WorkGround2", "skills")
...
func TestCreateRefusesOverwrite(t *testing.T) {
	...
	if !strings.HasSuffix(path, filepath.Join(".WorkGround2", "skills", "mine", SkillFile)) {
...
func TestParseMarkdownContentReadOnlyFrontmatter(t *testing.T) {
	tests := []struct {
		...
		},
		{
			...
			wantRO:   false,
			wantName: "normal-skill",
		},
		{
			name:     "yes variant",
			content:  "---\nname: yes-skill\ndescription: yes variant\nread-only: yes\n---\nbody",
			wantRO:   true,
			wantName: "yes-skill",
		},
		{
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			sk, ok := ParseMarkdownContent(tt.content, "(test)", "stem", ScopeBuiltin, false)
			if !ok {
				t.Fatal("ParseMarkdownContent returned false")
			}
			if sk.ReadOnly != tt.wantRO {
				t.Errorf("ReadOnly = %v, want %v", sk.ReadOnly, tt.wantRO)
			}

...
func (t *readSkillTool) Execute(_ context.Context, args json.RawMessage) (string, error) {
	...
	if sk.IsProtected() {
		return ProtectedSkillNotice(sk), nil
	}
...
func (t *installSkillTool) Description() string {
	scope := "'global' (only option — no project workspace) writes to the WorkGround2 home skills directory."
	if t.store.HasProjectScope() {
		scope = "'project' (default) writes to <repo>/.workground2/skills/ (this workspace only); 'global' writes to the WorkGround2 home skills directory (every project)."
	}
...
func Render(sk Skill, args string) string {
	...
	if policy := protectedSkillPolicy(sk); policy != "" {

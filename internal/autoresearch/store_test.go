...
func TestCreateTaskCreatesHostOwnedLayoutAndInitialState(t *testing.T) {
	...
	wantRoot := filepath.Join(root, ".WorkGround2", "autoresearch", task.ID)
...
func TestLoadTaskRejectsSymlinkTaskDirectory(t *testing.T) {
	...
	if err := os.MkdirAll(filepath.Join(root, ".WorkGround2", "autoresearch"), 0o755); err != nil {
		t.Fatalf("create autoresearch root: %v", err)
	}
	if err := os.Symlink(outside, filepath.Join(root, ".WorkGround2", "autoresearch", taskID)); err != nil {
...
func TestResumeFromGoalTextLoadsExplicitTaskPath(t *testing.T) {
	...
	resumed, ok, err := store.ResumeFromGoalText("继续 .workground2/autoresearch/" + task.ID + "/ 这个任务")
	if err != nil {

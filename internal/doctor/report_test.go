...
func TestCollectReportRedactsSecrets(t *testing.T) {
	t.Setenv("WorkGround2_TEST_SECRET", "sk-live-secret")
	...
	cfg.Providers = []config.ProviderEntry{{
		...
		Model:     "model-a",
		APIKeyEnv: "WorkGround2_TEST_SECRET",
	}}
...
func TestCollectReportDoesNotRequireAPIKey(t *testing.T) {
	t.Setenv("WorkGround2_HOME", filepath.Join(t.TempDir(), "WorkGround2"))
	...
	if !strings.Contains(text, "WorkGround2 1.2.3 doctor") {
...
func TestRenderTextSurfacesWarningsUpTop(t *testing.T) {
	text := RenderText(Report{Warnings: []string{"config WorkGround2.toml: parse boom"}})

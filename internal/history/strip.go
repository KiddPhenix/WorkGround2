...
var reComposeBlock = regexp.MustCompile(`(?s)^\s*<(?:memory-update|pinned-memory|background-jobs|active-goal|hook-context)(?:\s+[^>]*)?>.*?</(?:memory-update|pinned-memory|background-jobs|active-goal|hook-context)>\s*\n`)

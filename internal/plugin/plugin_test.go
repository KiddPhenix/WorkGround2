...
func TestStartAvailableKeepsGoodServers(t *testing.T) {
	...
	bad := Spec{Name: "bad", Command: "WorkGround2-missing-mcp-binary"}
...
func TestStdioCommandNotFoundSuggestsPATHFix(t *testing.T) {
	...
	host, _ := StartAvailable(ctx, []Spec{{Name: "missing", Command: "WorkGround2-missing-mcp-binary"}})
	defer host.Close()
	...
	for _, want := range []string{
		`command "WorkGround2-missing-mcp-binary" not found on PATH`,

// Package plugin is WorkGround2's MCP client. It connects to external MCP servers and
...
// protocolVersion is the MCP revision WorkGround2 advertises during initialize.
...
type Spec struct {
	...
	// WorkGround2's built-in defaultCallTimeout.
	...
	// inherit WorkGround2's cwd (the default for user-configured plugins). It exists
...
// messages (notifications, requests like roots/list) are ignored — WorkGround2 is a
...
// CallMethod sends an arbitrary MCP JSON-RPC request to a named server and
// returns the raw result.  This is the escape hatch for custom/non-standard MCP
// methods such as panel/query and panel/action that are not part of the core
// MCP specification.
func (h *Host) CallMethod(ctx context.Context, server, method string, params any) (json.RawMessage, error) {
	c := h.client(server)
	if c == nil {
		return nil, fmt.Errorf("plugin: server %q is not connected", server)
	}
...
func (c *Client) initializeSession(ctx context.Context, recordCapabilities bool) error {
	res, err := c.call(ctx, "initialize", map[string]any{
		...
		"clientInfo":      map[string]any{"name": "WorkGround2", "version": "dev"},
	})
	if err != nil {
		return err
	}
...
type mcpTool struct {
	...
	// plugin that declares a tool read-only opts it into WorkGround2's parallel-dispatch

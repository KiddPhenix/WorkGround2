...
// buildExamplePlugin compiles cmd/workground2-plugin-example into a temp binary and
...
func buildExamplePlugin(t *testing.T) string {
	t.Helper()
	bin := filepath.Join(t.TempDir(), "WorkGround2-plugin-example")
	if runtime.GOOS == "windows" {
		bin += ".exe"
	}
	out, err := exec.Command("go", "build", "-o", bin, "workground2/cmd/workground2-plugin-example").CombinedOutput()
	if err != nil {

// Package memory implements WorkGround2's persistent memory. It mirrors Claude
// Code's two-layer model while honoring WorkGround2's cache-first architecture:
//
//   - Hierarchical doc memory: WorkGround2.md / AGENTS.md files discovered from the
...
const (
	ScopeUser     Scope = "user"     // ~/.workground2/WorkGround2.md
	ScopeAncestor Scope = "ancestor" // a WorkGround2.md above the project root
	ScopeProject  Scope = "project"  // ./WorkGround2.md (committed, shared)
	ScopeLocal    Scope = "local"    // ./WorkGround2.local.md (personal, git-ignored)
)

// docNames are the recognized memory filenames at each level, in load order.
// WorkGround2.md is ours; AGENTS.md and CLAUDE.md are the cross-tool conventions.
...
var docNames = []string{"WorkGround2.md", "AGENTS.md", "CLAUDE.md"}

// localNames are the personal, git-ignored overrides, highest precedence.
var localNames = []string{"WorkGround2.local.md", "AGENTS.local.md", "CLAUDE.local.md"}

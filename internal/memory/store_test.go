...
用户希望默认使用中文。`
	...
	memories := s.List()
	if len(memories) != 1 || memories[0].Title != "中文偏好" || memories[0].Description != "使用中文回答" || !strings.Contains(memories[0].Body, "默认使用中文") {
		t.Fatalf("decoded memories = %+v", memories)
	}
}
...
func TestStoreForSlug(t *testing.T) {
	s := StoreFor("/home/me/.WorkGround2", "/Users/me/proj")

...
func TestDocPathPrefersExisting(t *testing.T) {
	proj := t.TempDir()
	// An existing WorkGround2.md should keep receiving notes (no split to AGENTS.md).
	if err := os.WriteFile(filepath.Join(proj, "WorkGround2.md"), []byte("x"), 0o644); err != nil {
		t.Fatal(err)
	}
	set := Load(Options{CWD: proj})
	if got := set.DocPath(ScopeProject); filepath.Base(got) != "WorkGround2.md" {
		t.Errorf("should append to the existing WorkGround2.md, got %s", got)
	}

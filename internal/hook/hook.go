...
// (.workground2/settings.json, only when the project is trusted) and a global
// (~/.workground2/settings.json) file. A hook's exit
...
const (
	SettingsDirname  = ".WorkGround2"
	SettingsFilename = "settings.json"
)

// GlobalSettingsPath is <WorkGround2 home>/settings.json (homeDir overrides ~ for
// tests and legacy callers).
func GlobalSettingsPath(homeDir string) string {
	return filepath.Join(WorkGround2Home(homeDir), SettingsFilename)
}

// ProjectSettingsPath is <root>/.workground2/settings.json.
...
func Load(opts LoadOptions) []ResolvedHook {
	...
	appendPluginHooks(&out, WorkGround2Home(opts.HomeDir), opts.ProjectRoot)
...
func appendPluginHooks(out *[]ResolvedHook, WorkGround2HomeDir, projectRoot string) {
	if strings.TrimSpace(WorkGround2HomeDir) == "" {
		return
	}
	installed, _ := pluginpkg.LoadInstalled(WorkGround2HomeDir)
	for _, item := range installed {
		...
		for _, eventName := range events {
			...
			for _, h := range pkg.Manifest.Hooks[eventName] {
				...
				env := cloneEnv(h.Env)
				env["WorkGround2_PLUGIN_ROOT"] = pkg.Root
				env["WorkGround2_PLUGIN_NAME"] = item.Installed.Name
				env["WorkGround2_HOME"] = WorkGround2HomeDir
				env["WorkGround2_WORKSPACE_ROOT"] = projectRoot
				if item.Installed.Version != "" {
					env["WorkGround2_PLUGIN_VERSION"] = item.Installed.Version
				}
...
func WorkGround2Home(override string) string {
	if override != "" {
		return filepath.Join(override, SettingsDirname)
	}
	if dir := config.WorkGround2HomeDir(); dir != "" {
		return dir
	}
...
func legacyGlobalSettingsPath(homeDir string) string {
	dir := legacyWorkGround2Home(homeDir)
	if dir == "" {
		return ""
	}
...
func legacyWorkGround2Home(override string) string {
	if override != "" {
		return ""
	}
	...
	if sameCleanPath(legacy, WorkGround2Home("")) {
		return ""
	}
	return legacy
}

...
// Trust gates project hooks. A project's .workground2/settings.json can run
...
// root. The trust flag lives in user-global state (~/.workground2/trust.json),
// NOT in the project file itself — an attacker controls the latter. Global
// hooks (~/.workground2/settings.json) are the user's own and always run.

// TrustFilename is the user-global trust store under ~/.WorkGround2.
...
// TrustPath is <WorkGround2 home>/trust.json (homeDir overrides ~ for tests and
// legacy callers).
func TrustPath(homeDir string) string {
	return filepath.Join(WorkGround2Home(homeDir), TrustFilename)
}

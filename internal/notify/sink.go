...
func message(cfg config.NotificationsConfig, e event.Event) (Message, bool) {
	...
	case event.TurnDone:
		if cfg.TurnDone {
			if e.Err != nil {
				return Message{Title: "WorkGround2", Body: "Turn failed"}, true
			}
			return Message{Title: "WorkGround2", Body: "Turn finished"}, true
		}
	case event.ApprovalRequest:
		if cfg.ApprovalRequest {
			return Message{Title: "WorkGround2", Body: "Approval needed"}, true
		}
	case event.AskRequest:
		if cfg.AskRequest {
			return Message{Title: "WorkGround2", Body: "Question needs your answer"}, true
		}
	}

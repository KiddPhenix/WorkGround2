...
import (
	...
	// Blank import registers the provider kind the same way cmd/WorkGround2's main
...
// cache-first wiring: a project WorkGround2.md is discovered at boot and folded
...
`)
	writeFile(t, dir, "WorkGround2.md", "Project rule: always run go vet before committing.")
	...
	if !strings.Contains(sys, "always run go vet before committing") {
		t.Fatalf("project WorkGround2.md not folded into system message:\n%s", sys)
	}
	...
	if mem := ctrl.Memory(); mem == nil || len(mem.Docs) == 0 {
		t.Fatal("controller memory set is empty after discovering WorkGround2.md")
	}
}
...
`)
	...
	msgs := sess.Snapshot()
	if len(msgs) != 4 || !strings.HasSuffix(msgs[1].Content, "first skill task") || msgs[2].Content != "first skill answer" || !strings.HasSuffix(msgs[3].Content, "second skill task") {
...
`)
	...
	if got := bootLastUser(reqs[1]); !strings.Contains(got, "<response-language>") || !strings.Contains(got, "use English") || !strings.HasSuffix(got, "first skill task") {
		t.Fatalf("skill subagent user turn = %q, want English response preference and task", got)
	}
}

func TestBuildReadOnlySubagentSkillEnforcesToolBoundary(t *testing.T) {
	isolateConfigHome(t)
	dir := robustTempDir(t)
	t.Chdir(dir)
	writeFile(t, dir, ".workground2/skills/review.md", `---
description: read-only review override
runAs: subagent
read-only: true
allowed-tools: read_file, write_file, bash
---
Review the requested change without modifying it.`)
...
`)
	...
	for _, hidden := range []string{"task", "run_skill", "read_only_skill", "install_skill", "install_source", "explore", "research", "review", "security_review", "write_file", "wait", "bash_output", "kill_shell"} {
	...
	for _, schema := range subReq.Tools {
		if schema.Name == "bash" && !strings.Contains(schema.Description, "read-only") {
			t.Fatalf("skill subagent bash should advertise the read-only boundary: %q", schema.Description)
		}
	}
}
...
// `WorkGround2 run`: a controller built via Build with NO SetSessionPath (exactly
...
api_key_env = "WorkGround2_TEST_KEY_UNSET"
`)
	writeFile(t, dir, ".workground2/skills/projskill.md", "---\ndescription: a project skill\n---\nplaybook")
...
[[plugins]]
name = "mockmcp"
command = "WorkGround2-missing-mockmcp"
`)
	writeFile(t, dir, ".workground2/skills/projskill.md", "---\ndescription: a project skill\n---\nplaybook")
...
model = "x"
`)
	writeFile(t, dir, ".workground2/skills/readonlydig/SKILL.md", `---
...
func TestBuildTokenEconomyPlanModeCanConnectAllowedMCPSource(t *testing.T) {
	...
	setBootTokenProfileTestProvider(t, prov)
	writeFile(t, dir, "WorkGround2.toml", fmt.Sprintf(`
default_model = "test-model"
...
func TestAddBuiltinsWithWorkspaceRootKeepsSessionTools(t *testing.T) {
	...
	addBuiltins(reg, nil, []string{robustTempDir(t)}, sandbox.Spec{}, 120*time.Second, builtin.SearchSpec{}, &stderr, robustTempDir(t), netclient.ProxySpec{}, nil, nil, nil, nil, true)
...
func TestBuildDoesNotExposeDrawImageFromLegacyAddOnConfig(t *testing.T) {
	isolateConfigHome(t)
...
model = "x"
`)
	writeFile(t, filepath.Join(home, "addons", "draw-tool"), "config.json", `{
  "version": 1,
  "providers": [
    {
      "id": "cli-image",
	...
	defer ctrl.Close()
	if err := ctrl.Run(context.Background(), "capture draw tool"); err != nil {
		t.Fatalf("Run: %v", err)
	}
	reqs := prov.Requests()
	if len(reqs) != 1 {
	...
	if requestHasTool(reqs[0], "draw_image") {
		t.Fatalf("draw_image exposed from legacy draw-tool config; tools=%v", toolSchemaNames(reqs[0].Tools))
	}
}

func TestBuildSkipsBuiltInDrawImageToolWhenExternalDrawAddOnInstalled(t *testing.T) {
	isolateConfigHome(t)
	home := robustTempDir(t)
	t.Setenv("WorkGround2_HOME", home)
	dir := robustTempDir(t)
...
`)
  ...
  "providers": [
	...
	pluginRoot := filepath.Join(home, "plugins", "draw-tool")
	writeFile(t, pluginRoot, pluginpkg.NativeManifest, `{
  "name": "draw-tool",
  "version": "0.1.0",
  "addon": {
    "kind": "draw-tool",
  ...
  "mcpServers": {
    "draw-addon": { "command": "fake-draw-addon", "auto_start": false }
  }
}`)
	if err := pluginpkg.Upsert(home, pluginpkg.InstalledPlugin{
		Name:         "draw-tool",
		Root:         "plugins/draw-tool",
		Version:      "0.1.0",
...
`)
  ...
  "providers": [
	...
	if requestHasTool(reqs[0], "draw_image") {
		t.Fatalf("draw_image exposed without enabled provider; tools=%v", toolSchemaNames(reqs[0].Tools))
	}
}
...
`)
	...
	// user-global WorkGround2.md in the real config dir could append; the test
...
func TestRememberPermissionRuleUsesWorkspaceRoot(t *testing.T) {
	...
	workspace := robustTempDir(t)
	t.Chdir(cwd)
	writeFile(t, cwd, "WorkGround2.toml", `
[permissions]
allow = ["Bash(cwd*)"]
`)
	writeFile(t, workspace, "WorkGround2.toml", `
...
`)
	...
	cwdCfg := config.LoadForEdit(filepath.Join(cwd, "WorkGround2.toml"))
	...
	workspaceCfg := config.LoadForEdit(filepath.Join(workspace, "WorkGround2.toml"))
...
`)
	...
	if !res.Saved || res.Path != filepath.Join(workspace, "WorkGround2.toml") {
...
`)
	...
	if _, err := os.Stat(filepath.Join(cwd, "WorkGround2.toml")); !os.IsNotExist(err) {
...
`)
	...
	cfg := config.LoadForEdit(filepath.Join(workspace, "WorkGround2.toml"))
...
// ~/.workground2/config.json with no v1+ config present must be imported during
...
func TestBuildMigratesLegacyConfigEndToEnd(t *testing.T) {
	...
	// migrated plugins.
	writeFile(t, proj, "WorkGround2.toml", "")
	writeFile(t, filepath.Join(home, ".WorkGround2"), "config.json",
		`{"apiKey":"sk-e2e","lang":"zh","mcpServers":{"fs":{"command":"npx","args":["-y","server-fs"]}}}`)
	writeFile(t, filepath.Join(home, ".WorkGround2", "sessions"), "chat-1.events.jsonl",
...
func TestBuildMigratesLegacyXDGAndProjectSessions(t *testing.T) {
	...
	WorkGround2Home := filepath.Join(home, "rx-home")
	t.Setenv("HOME", home)
	t.Setenv("USERPROFILE", home)
	...
	proj := robustTempDir(t)
	writeFile(t, proj, "WorkGround2.toml", "[codegraph]\nenabled = false\n")

	legacyRoot := filepath.Join(xdg, "WorkGround2")
...
[[plugins]]
name = "legacy-eager"
command = "WorkGround2-missing-legacy-eager-mcp"
tier = "eager"
`)
	...
	raw, err := os.ReadFile(filepath.Join(dir, "WorkGround2.toml"))
	if err != nil {
		t.Fatal(err)
	}
...
[[plugins]]
name = "legacy-lazy"
command = "WorkGround2-missing-legacy-lazy-mcp"
tier = "lazy"
`)
...
func TestBuildDefaultsToNearestGitRoot(t *testing.T) {
	...
	writeFile(t, root, "WorkGround2.toml", `
...
[[plugins]]
name = "slowserver"
command = "WorkGround2-missing-slow-mcp-binary"
tier = "eager"
`)

...
func TestBashCancelKillsWindowsChildProcessTree(t *testing.T) {
	...
	command := fmt.Sprintf(
		"$p = Start-Process -FilePath powershell -ArgumentList '-NoProfile','-NonInteractive','-Command','Start-Sleep -Seconds 20' -PassThru; "+
	...
	case <-time.After(10 * time.Second):
		...
		t.Fatal("cancel did not interrupt bash within 10s")
	}
	if err == nil {

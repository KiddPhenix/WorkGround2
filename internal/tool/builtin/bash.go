...
func (bash) Schema() json.RawMessage {
	return json.RawMessage(`{"type":"object","properties":{"command":{"type":"string","description":"Shell command to execute. For browser interaction, prefer the native browser_* tools when available; use Playwright only when those tools are unavailable, lack a required capability, or explicitly fail, and never reload the page merely to observe state."},"run_in_background":{"type":"boolean","description":"Run detached: returns a job id immediately and keeps running across turns (no foreground timeout). Read new output with bash_output, wait with wait, stop it with kill_shell. Use for long-running commands like servers, watchers, or builds you don't need to block on."},"preserve_background_processes":{"type":"boolean","description":"After the shell command exits normally, keep any process-group members it intentionally left behind. Set true when launching browser-use, Playwright, GUI, or session processes through this shell and their windows must remain open after the command returns. Also use for deliberate daemonization or nohup/disown/setsid. Cancellation and timeouts still kill the process group."}},"required":["command"]}`)
}
...
func (b bash) Execute(ctx context.Context, args json.RawMessage) (string, error) {
	...
	if b.terminal != nil && !p.RunInBackground && b.sb.Mode != "enforce" {
		if out, ok, err := b.terminal.RunCommand(ctx, p.Command, b.workDir, b.foregroundTimeout()); ok {
			return out, err
		}
	}
...
func defaultBashShellPATH(ctx context.Context) string {
	...
	const marker = "__WorkGround2_BASH_PATH__="
	script := "printf '\\n" + marker + "%s\\n' \"$PATH\""

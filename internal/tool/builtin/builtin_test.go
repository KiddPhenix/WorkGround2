...
func TestBuiltinsRegistered(t *testing.T) {
	want := []string{"bash", "code_index", "edit_file", "glob", "grep", "ls", "move_file", "multi_edit", "read_file", "view_image", "web_fetch", "write_file"}
...
func TestBuiltinReadOnlyClassification(t *testing.T) {
	readOnly := map[string]bool{
		"read_file": true, "ls": true, "glob": true, "grep": true, "code_index": true, "view_image": true, "web_fetch": true,
...
func TestEditFile(t *testing.T) {
	...
	runTool(t, editFile{}, map[string]any{"path": f, "old_string": "world", "new_string": "WorkGround2"})
	if b, _ := os.ReadFile(f); string(b) != "hello WorkGround2\n" {
...
func TestMultiEdit(t *testing.T) {
	...
	out := runTool(t, multiEdit{}, map[string]any{
		...
		"edits": []map[string]any{
			...
			{"old_string": "old", "new_string": "WorkGround2", "replace_all": true},
		},
	})

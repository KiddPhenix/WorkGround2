...
func TestBackgroundKill(t *testing.T) {
	...
	if _, err := (bash{}).Execute(ctx, []byte(`{"command":"sleep 20","run_in_background":true}`)); err != nil {
		t.Fatalf("bash background: %v", err)
	}
	id := m.Running()[0].ID
	...
	// 20s natural duration keeps the job far from finishing on its own, so the
	...
	// broken kill trips the 10s timeout.
	res := m.Wait(ctx, []string{id}, 10)

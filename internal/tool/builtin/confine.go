...
// ConfineWebFetch returns the web_fetch built-in bound to WorkGround2 proxy
...
func confine(roots []string, target string) error {
	...
	return fmt.Errorf("path %q is outside the writable roots (writes are confined to %s); "+
		"write inside the workspace or a configured allow_write root, or widen [sandbox] workspace_root / allow_write in WorkGround2.toml",

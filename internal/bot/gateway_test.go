...
func TestGatewayDefaultQueueSteersMediaOnlyActiveTurn(t *testing.T) {
	...
	got := ctrl.steered()
	if len(got) != 1 || !strings.Contains(got[0], "Attachments:") || !strings.Contains(got[0], "@.workground2/attachments/") {
...
func TestGatewayUnknownDMGetsPairingCode(t *testing.T) {
	...
	sent := adapter.sentMessages()
	if len(sent) != 1 || !strings.Contains(sent[0].Text, "配对码") || !strings.Contains(sent[0].Text, "WorkGround2 bot pairing approve") {
...
func TestGatewayControlServerStatusAndSend(t *testing.T) {
	...
	_ = resp.Body.Close()
	if resp.StatusCode != http.StatusOK || !strings.Contains(string(metricsBody), "workground2_bot_adapter_sends_total") {

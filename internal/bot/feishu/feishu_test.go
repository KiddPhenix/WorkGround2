...
func TestVerificationTokenValidRequiresConfiguredToken(t *testing.T) {
	...
	if a.verificationTokenValid("") {
		t.Fatal("empty configured verification token should reject unauthenticated requests")
	}
}
...
func TestBuildMarkdownCard(t *testing.T) {
	...
	if len(payload.Body.Elements) != 1 {
		t.Fatalf("elements = %#v, want one markdown element", payload.Body.Elements)
	}
	if payload.Body.Elements[0].Tag != "markdown" || payload.Body.Elements[0].Content != "hello [docs](https://example.com)" {
		t.Fatalf("markdown element = %#v, want original markdown", payload.Body.Elements[0])
	}
}

...
// sendMessage 使用飞书/Lark SDK 以 Interactive Card 发送 markdown。
// 卡片超过平台限制时自动降级为纯文本，避免大段代码回复直接失败。
func (a *adapter) sendMessage(ctx context.Context, msg bot.OutboundMessage) (bot.SendResult, error) {
	...
	if err != nil {
		a.logger.Warn("build feishu markdown card failed, falling back to text", "err", err)
	...
	if err != nil && isCardLimitError(err) {
		a.logger.Warn("feishu card send failed, falling back to text", "err", err)
		return a.sendSDKContent(ctx, msg, larkim.MsgTypeText, feishuTextContent(msg.Text))
	}
	return result, err
}

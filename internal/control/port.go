...
type Approvals interface {
	...
	PendingPrompt() bool
	PendingInteraction() (PendingInteraction, bool)
...
// PinnedMemoryControl covers session-scoped pinned transcript excerpts.
type PinnedMemoryControl interface {
	PinnedMemories() []PinnedMemory
	PinMemory(role, content string, turn int) (string, bool, error)
	SetPinnedMemoryPinned(id string, pinned bool) (bool, error)
}
...
type Capabilities interface {
	...
	GetSkillStore() (*skill.Store, bool)
...
// TaskMemoryStatus is an opt-in read port for frontends that surface the
// recoverable session briefing. It stays separate so existing lean SessionAPI
// test doubles and frontends do not gain a mandatory method.
type TaskMemoryStatus interface{ TaskMemory() event.TaskMemory }
...
var (
	...
	_ TaskMemoryStatus    = (*Controller)(nil)

...
// .workground2/attachments and returns its repo-relative path for @referencing.
...
func saveAttachmentBytesInRoot(root, ext string, raw []byte) (string, error) {
	...
	absRel := filepath.Join(absRoot, rel)
...
func cleanAttachmentPath(path string) (string, error) {
	...
	root := filepath.Join(".workground2", "attachments")
	if clean == "." || clean == root || strings.HasPrefix(clean, ".."+string(filepath.Separator)) || !strings.HasPrefix(clean, root+string(filepath.Separator)) {
		return "", fmt.Errorf("attachment path is outside .workground2/attachments")
	}
	if err := ensureAttachmentRoot(); err != nil {
		return "", err
	}
...
func ensureAttachmentRootIn(base string) error {
	legacyRoot := filepath.Join(base, ".WorkGround2", "attachments")
	...
	root := filepath.Join(base, ".workground2", "attachments")
...
func attachmentPath(ext string) string {
	...
	return filepath.Join(".workground2", "attachments", name)
}

...
const (
	...
	refImage                   // a local image attachment: @.workground2/attachments/<file>
)
...
const externalFolderRefPrefix = "__WorkGround2_external_folder"
...
func classifyRef(token string, known map[string]bool, exists func(string) bool) (ref, bool) {
	...
	if attachment, ok := normalizeAttachmentRef(token); ok && exists(token) {
		if isImageAttachmentRef(attachment) {
			return ref{kind: refImage, path: attachment, raw: token}, true
		}
		return ref{kind: refFile, path: attachment, raw: token}, true
	}
	if exists(token) {
...
func isAttachmentRef(token string) bool {
	_, ok := normalizeAttachmentRef(token)
	return ok
}

func normalizeAttachmentRef(token string) (string, bool) {
	slash := filepath.ToSlash(strings.TrimSpace(token))
	prefix := ".workground2/attachments/"
	if !strings.HasPrefix(strings.ToLower(slash), prefix) {
		return "", false
	}
	return prefix + slash[len(prefix):], true
}
	...
	for _, tok := range parseRefTokens(line) {
		...
		if c.workspaceRoot != "" {
			if rel, ok := workspaceRefPath(tok, c.workspaceRoot); ok {
				if attachment, isAttachment := normalizeAttachmentRef(rel); isAttachment {
					kind := refFile
					if isImageAttachmentRef(attachment) {
						kind = refImage
					}
					refs = append(refs, ref{kind: kind, path: attachment, raw: tok})
					continue
				}
				kind := refFile

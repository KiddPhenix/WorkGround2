...
func (c *Controller) compose(text string, includeHookContext bool) string {
	...
	// Pinned memories ride the turn as a transient block so context compaction
	// cannot lose them. Like memory-update, this never touches the cache-stable
	// system prefix.
	if pm := c.pinnedMemos.active(); len(pm) > 0 {
		var b strings.Builder
		b.WriteString("<pinned-memory>\n")
		b.WriteString("The user has pinned the following memories from this session. Keep them in mind:\n")
		for _, it := range pm {
			b.WriteString(fmt.Sprintf("- [%s, turn %d] %s\n", it.Role, it.Turn, it.Content))
		}
		b.WriteString("</pinned-memory>\n\n")
		text = b.String() + text
	}
...
- Say briefly in the first visible reply that the goal is being handled with AutoResearch and that host-owned state lives under .workground2/autoresearch/<task-id>/, using the actual task_id from <autoresearch-runtime>.
- Keep dynamic state out of WorkGround2.md, AGENTS.md, project memory, system prompts, and tool schemas. Use project-local .workground2/autoresearch/ state only.
...
- Complete only after auditing every open success criterion in <autoresearch-runtime> against direct evidence. Public publishing, destructive changes, credential use, payments, external notifications, privacy-sensitive output, and cache-sensitive changes still require the normal WorkGround2 gates.`
...
func shouldAutoStartResearchGoal(input string) bool {
	...
	if strings.Contains(lower, ".workground2/autoresearch/") {
		return true
	}

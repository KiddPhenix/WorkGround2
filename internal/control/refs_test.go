...
func TestClassifyRef(t *testing.T) {
	...
	files := map[string]bool{
		"src/main.go": true,
		"README.md":   true,
		".workground2/attachments/clipboard-20260601-010203.000000.png": true,
		".workground2/attachments/clipboard-20260601-010203.000000.yml": true,
		".workground2/attachments/clipboard-20260601-010203.000000.zip": true,
	}
	...
	cases := []struct {
		...
		{".workground2/attachments/clipboard-20260601-010203.000000.png", true, refImage},
		{".workground2/attachments/clipboard-20260601-010203.000000.yml", true, refFile},
		{".workground2/attachments/clipboard-20260601-010203.000000.zip", true, refFile},
...
func TestResolveRefsAttachmentKinds(t *testing.T) {
	temp := t.TempDir()
	attachmentsDir := filepath.Join(temp, ".WorkGround2", "attachments")
	if err := os.MkdirAll(attachmentsDir, 0o755); err != nil {
		t.Fatal(err)
	}
	ymlRef := filepath.ToSlash(".workground2/attachments/config.yml")
	zipRef := filepath.ToSlash(".workground2/attachments/archive.zip")
	pngRef := filepath.ToSlash(".workground2/attachments/shot.png")
	if err := os.WriteFile(filepath.Join(temp, filepath.FromSlash(ymlRef)), []byte("name: WorkGround2\n"), 0o644); err != nil {
		t.Fatal(err)
	}
	...
	if !strings.Contains(block, `<file path="`+ymlRef+`">`) || !strings.Contains(block, "name: WorkGround2") {
...
func TestWorkspaceImageRefsAlsoAttachAsModelImages(t *testing.T) {
	...
	attachment := filepath.Join(workspace, ".WorkGround2", "attachments", "shot.png")
	...
	if refs[1].kind != refImage || refs[1].path != ".workground2/attachments/shot.png" {

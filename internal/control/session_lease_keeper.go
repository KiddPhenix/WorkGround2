...
// that binds session files for writing (the CLI chat/run commands, `WorkGround2
...
const SessionLeaseCloseHint = "close the other WorkGround2 window or process first"
...
func SessionInUseMessage(err error) string {
	const fallback = "this session is in use by another WorkGround2 window or process"
	...
	fmt.Fprintf(&b, "this session is in use by another WorkGround2 process (pid %d", info.PID)

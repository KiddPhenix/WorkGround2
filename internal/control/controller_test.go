...
func TestResumeRestoresRunningAutoResearchGoalFromSidecar(t *testing.T) {
	...
	if err := os.MkdirAll(filepath.Join(root, ".WorkGround2", "autoresearch", taskID, "state"), 0o755); err != nil {
		t.Fatal(err)
	}
	if err := os.MkdirAll(filepath.Join(root, ".WorkGround2", "autoresearch", taskID, "logs"), 0o755); err != nil {
		t.Fatal(err)
	}
	if err := os.WriteFile(filepath.Join(root, ".WorkGround2", "autoresearch", taskID, "state", "task_spec.json"), []byte(`{"id":"investigate-runtime-resume","goal":"investigate runtime resume","status":"running","created_at":"2026-06-30T00:00:00Z","updated_at":"2026-06-30T00:00:00Z","success_criteria":[{"id":"criterion-1","description":"resume keeps AutoResearch active","required":true}]}`), 0o644); err != nil {
		t.Fatal(err)
	}
	if err := os.WriteFile(filepath.Join(root, ".WorkGround2", "autoresearch", taskID, "state", "progress.json"), []byte(`{"task_id":"investigate-runtime-resume","iteration":2,"current_direction":"verify resume","stale_count":1,"pivot_count":0,"updated_at":"2026-06-30T00:00:00Z"}`), 0o644); err != nil {
		t.Fatal(err)
	}
	if err := os.WriteFile(filepath.Join(root, ".WorkGround2", "autoresearch", taskID, "state", "directions_tried.json"), []byte(`{"task_id":"investigate-runtime-resume","directions":[]}`), 0o644); err != nil {
		t.Fatal(err)
	}
	if err := os.WriteFile(filepath.Join(root, ".WorkGround2", "autoresearch", taskID, "state", "findings.jsonl"), nil, 0o644); err != nil {
		t.Fatal(err)
	}
	if err := os.WriteFile(filepath.Join(root, ".WorkGround2", "autoresearch", taskID, "logs", "heartbeat.jsonl"), nil, 0o644); err != nil {
		t.Fatal(err)
	}
...
func TestTwoModelShortChoiceReplySkipsPlanner(t *testing.T) {
	...
	if strings.Contains(reqText, "WorkGround2 executor handoff") {
...
func TestRemoveMCPServerRemovesUnconnectedLazyPlaceholder(t *testing.T) {
	...
	t.Chdir(dir)
	if err := os.WriteFile("WorkGround2.toml", []byte(`
[[plugins]]
name = "mock"
command = "mock-mcp"
tier = "lazy"
...
func TestApprovalPersistentBashPrefixRememberRule(t *testing.T) {
	...
	c := New(Options{
		...
		OnRemember: func(rule string) RememberResult {
			remembered = rule
			return RememberResult{Rule: rule, Path: "WorkGround2.toml", Saved: true}
		},
	})
	go func() {
	...
	if len(notices) != 1 || !strings.Contains(notices[0], "Bash(go test:*)") || !strings.Contains(notices[0], "WorkGround2.toml") {
...
func TestPlanModeReadOnlyTrustApprovalPersistsMCPTrust(t *testing.T) {
	...
	c := New(Options{
		...
		OnRememberMCPReadOnlyTrust: func(serverName, rawToolName string) MCPReadOnlyTrustResult {
			...
			return MCPReadOnlyTrustResult{Server: serverName, Tool: rawToolName, Path: "WorkGround2.toml", Saved: true}
		},
	})
...
func TestNewSessionRejectsActiveRuntimeWork(t *testing.T) {
	sess := agent.NewSession("sys")
	release := make(chan struct{})
	c := New(Options{
		Executor: agent.New(&recordingProvider{}, tool.NewRegistry(), sess, agent.Options{}, event.Discard),
		Runner:   blockingRunner{session: sess, release: release},
	})
	defer c.Close()

	done := make(chan error, 1)
	go func() { done <- c.RunTurn(context.Background(), "keep running") }()
	waitForRunning(t, c)
	oldPath := c.SessionPath()
	if err := c.NewSession(); err == nil || !strings.Contains(err.Error(), "runtime work is active") {
		t.Fatalf("NewSession error = %v, want active runtime rejection", err)
	}
	if got := c.SessionPath(); got != oldPath {
		t.Fatalf("session path changed while turn was active: got %q want %q", got, oldPath)
	}
	close(release)
	if err := <-done; err != nil {
		t.Fatalf("running turn returned %v", err)
	}
}
...
func TestReloadCommandsFromFilesystem(t *testing.T) {
	...
	wsRoot := t.TempDir()
	cmdDir := filepath.Join(wsRoot, ".WorkGround2", "commands")
...
// (higher priority) wins. ConventionDirs = [".WorkGround2", ".agents", ".agent",
// ".claude"], scanned in reverse, so .WorkGround2 is highest priority.
func TestReloadCommandsSameNameAcrossDirs(t *testing.T) {
	...
	// Higher priority: .workground2/commands
	WorkGround2Dir := filepath.Join(wsRoot, ".WorkGround2", "commands")
	writeCmdFile(t, WorkGround2Dir, "greet", "WorkGround2 greet", "Hello from WorkGround2: $1")
	...
	// The winning version should be from .WorkGround2 (highest priority)
	sent, ok := c.CustomCommand("/greet world")
	if !ok {
		t.Fatal("/greet should be found")
	}
	if !strings.Contains(sent, "Hello from WorkGround2") {
		t.Errorf("expected .WorkGround2 version to win, got render: %q", sent)
	}
}

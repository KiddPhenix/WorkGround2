...
func TestResearchGoalCreatesHostManagedAutoResearchTask(t *testing.T) {
	...
	for _, rel := range []string{
		...
		path := filepath.Join(root, ".WorkGround2", "autoresearch", state.AutoResearchTaskID, rel)
	...
	readJSONFileForTest(t, filepath.Join(root, ".WorkGround2", "autoresearch", state.AutoResearchTaskID, "state", "task_spec.json"), &spec)
...
func TestResearchGoalRepeatedSetReusesAutoResearchTask(t *testing.T) {
	...
	entries, err := os.ReadDir(filepath.Join(root, ".WorkGround2", "autoresearch"))
	if err != nil {
...
func TestResearchGoalMissingExplicitTaskBlocksInsteadOfCreatingNewTask(t *testing.T) {
	...
	c.SetGoalWithResearchMode("resume .workground2/autoresearch/missing-task/", GoalResearchOn)

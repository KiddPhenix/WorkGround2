...
type Controller struct {
	...
	// WorkGround2.toml on add/remove, building specs from entries). See mcp.go.
	mcp mcpManager
	...
	// persisted to the *.pinned-memo.json session sidecar on mutation and
	// injected into future turns as a transient <pinned-memory> block.
...
const (
	...
	RuntimeModeForeground     RuntimeMode = "foreground"
	RuntimeModeWaitingUser    RuntimeMode = "waiting_user"
	RuntimeModeBackgroundOnly RuntimeMode = "background_only"
	RuntimeModeCancelling     RuntimeMode = "cancelling"
)
...
type RuntimeStatus struct {
	...
	BackgroundOnly    bool
	ActiveRuntimeWork bool
}

const (
	PendingInteractionApproval = "approval"
	PendingInteractionAsk      = "ask"
)

// PendingInteraction is the one user decision currently blocking the turn.
// promptMu serializes these, so callers never need to reconcile multiple
// simultaneous approvals or question batches.
...
func New(opts Options) *Controller {
	...
	_ = c.pinnedMemos.load(opts.SessionPath)
...
// PendingInteraction returns the approval or ask currently blocking this
// session. The ID must be echoed back when a remote caller resolves it so a
// stale answer cannot be applied to a newer prompt.
func (c *Controller) PendingInteraction() (PendingInteraction, bool) {
	approvals, asks := c.approval.snapshotPrompts()
	if len(approvals) > 0 {
		return PendingInteraction{Kind: PendingInteractionApproval, Approval: approvals[0]}, true
	}
	if len(asks) > 0 {
		return PendingInteraction{Kind: PendingInteractionAsk, Ask: asks[0]}, true
	}
...
func (c *Controller) RuntimeStatus() RuntimeStatus {
	...
	return RuntimeStatus{
		...
		Cancellable:       foregroundActive,
		ForegroundActive:  foregroundActive,
		BackgroundOnly:    mode == RuntimeModeBackgroundOnly,
		ActiveRuntimeWork: foregroundActive || backgroundJobs > 0,
	}
}

func deriveRuntimeMode(running, pending bool, backgroundJobs int, canceling bool) RuntimeMode {
	switch {
	case canceling:
		return RuntimeModeCancelling
	case pending:
		return RuntimeModeWaitingUser
	case running:
		return RuntimeModeForeground
	case backgroundJobs > 0:
		return RuntimeModeBackgroundOnly
	default:
...
// ApprovePending answers the first pending tool/plan approval. Structured asks
// must go through AnswerQuestion so the selected option labels are preserved.
func (c *Controller) ApprovePending(allow bool) {
	approvals, _ := c.approval.snapshotPrompts()
	if len(approvals) > 0 {
		c.Approve(approvals[0].ID, allow, true, false)
	}
}
...
type plannerPlanApprover struct{ c *Controller }

func (p plannerPlanApprover) RunWithPlannerApproval(ctx context.Context, plan string, run func(context.Context) error) error {
	allow, _, err := p.c.requestApprovalWithReason(ctx, planApprovalTool, "", nil, "Planner requested host approval before execution.")
	if err != nil || !allow {
		return err
	}
	todoArgs := p.c.seedPlanTodos(plan)
	execStart := p.c.sessionMessageCount()
	p.c.approval.setPlanAutoApprove(true)
	defer p.c.approval.setPlanAutoApprove(false)
	if err := run(ctx); err != nil {
		return err
	}
	if todoArgs != "" && !p.c.hasTodoUpdateSince(execStart) {
		p.c.completePlanTodos(todoArgs)
	}
	return nil
}

type plannerUserDecisionAsker struct{ c *Controller }
...
func (c *Controller) Ask(ctx context.Context, questions []event.AskQuestion) ([]event.AskAnswer, error) {
	...
	c.updateTaskMemory(taskMemoryPatch{
		current: stringPtr("等待回答"), currentSource: stringPtr("ask"),
		nextStep: &nextStep, nextStepSource: stringPtr("ask"),
	})
...
func (c *Controller) SetGoalWithResearchMode(goal string, researchMode GoalResearchMode) {
	...
	c.updateTaskMemory(taskMemoryPatch{goal: &goal, goalSource: &goalSource})
...
func (c *Controller) NewSession() error {
	...
	if c.RuntimeStatus().ActiveRuntimeWork {
		return fmt.Errorf("cannot create a new session while runtime work is active")
	}
	...
	if c.sessionDir != "" {
		c.mu.Lock()
		previousPath := c.sessionPath
		c.sessionPath = agent.NewSessionPath(c.sessionDir, c.label)
		if agent.CanonicalSessionPath(c.sessionPath) == agent.CanonicalSessionPath(previousPath) {
			ext := filepath.Ext(c.sessionPath)
			c.sessionPath = strings.TrimSuffix(c.sessionPath, ext) + "-next" + ext
		}
...
func removeSessionArtifacts(path string) error {
	...
	paths := append([]string{path, guardian.PathFor(path), guardian.CursorPathFor(path)}, store.SessionSidecarFiles(path)...)
	for _, p := range paths {
		if p == "" {
			continue
		}
...
// Field reports of "session changed on disk" spam are only diagnosable when the
// logs say which trigger fired and what the revision ledger looked like, so
// every recoverSnapshotConflict outcome logs these.
...
// conflictOutcome is recoverSnapshotConflict's declared result. Callers act on
// it directly instead of re-deriving what happened from path or session pointer
// comparisons.
type conflictOutcome int

const (
	// conflictDropped: nothing was recovered and the disk transcript could not
	// be adopted; this snapshot was deliberately dropped.
	...
	// in-memory session was force-saved onto the same branch; that save advanced
	// the session-owned rewrite baseline like any other full save.
	conflictForceSavedBranch
	// conflictForkedBranch: the same in-memory session moved to a freshly forked
	// recovery branch path.
...
func (c *Controller) recoverSnapshotConflict(path string, saveErr error, forceRewrite bool) (string, conflictOutcome, error) {
	...
	if err != nil {
		if errors.Is(err, agent.ErrSessionRecoveryDepthExceeded) {
			...
			// created; forking again multiplies session files without converging.
			// This runtime is the only writer of its own recovery branches, so
			// force-writing the transcript back onto the current branch keeps the
			// data and stops the chain.
		...
		if errors.Is(err, agent.ErrSessionRecoveryNotNeeded) {
			...
			// Nothing was recovered and the disk transcript could not be adopted:
			// the snapshot is deliberately dropped. Leave a trace so data-loss
			// reports can be tied to this path.
...
// session path a recovery fork abandoned onto the branch the turn continues on.
// Left behind, the stale marker would fire recoverInterruptedTurn on the next
// open of the original branch and strip messages from a turn that kept running
// on the recovery branch; missing from the recovery branch, a crash before turn
// end would leave its partial tail unmarked.
func (c *Controller) transplantInFlightTurnMarker(fromPath, toPath string) {
	...
	if err := agent.MarkSessionInFlightTurn(toPath, marker.StartMessageIndex, marker.PreserveUser); err != nil {
		// Keep the original marker: a turn boundary on the wrong branch beats no
		// boundary anywhere if the runtime dies before the turn completes.
...
func (c *Controller) recoverInterruptedTurn(path string) {
	...
	if interruptedTurnContinuedOnRecoveryBranch(path, marker) {
		...
		// running there. Clear the stale marker instead of truncating the
		// forked-from transcript.
		if err := agent.ClearSessionInFlightTurn(path); err != nil {
...
func (c *Controller) replaceSessionAfterCancel(msgs []provider.Message) {
	...
	// the longer partial autosave on disk as a stale-prefix conflict, and adopt
	// it back into the executor before the flush below could persist it.
	c.snapshotMu.Lock()
	defer c.snapshotMu.Unlock()
...
func (c *Controller) SetSessionPath(p string) {
	...
	c.loadTaskMemory(p)
	// Load pinned memories from the session sidecar (best-effort; errors are
	// non-fatal — the list may be empty on a fresh session).
	_ = c.pinnedMemos.load(p)
}
...
func (c *Controller) ToolResult(toolID string) *ToolResultData {
	...
	for i := len(msgs) - 1; i >= 0; i-- {
		...
		out := &ToolResultData{
			Args:   "",
			Output: skill.RedactProtectedContent(msgs[i].Content),
		}
...
// out of project WorkGround2.toml while preserving the CLI AddMCPServer semantics.
...
// WorkGround2.md by default) — the write side of "#<note>". Returns the file written.
...
// PinnedMemories returns a copy of all pinned-memory entries for this session
// (both currently pinned and unpinned).
func (c *Controller) PinnedMemories() []PinnedMemory {
	return c.pinnedMemos.all()
}

// PinMemory adds a transcript excerpt to the session's pinned-memory store.
// The entry is idempotent by role, turn, and content: pinning the same text
// twice returns the same ID and does not create a duplicate.  Returns the
// stable entry ID and whether a new entry was created.
func (c *Controller) PinMemory(role, content string, turn int) (string, bool, error) {
	role, content, turn, err := validatePinnedMemory(role, content, turn)
	if err != nil {
		return "", false, err
	}
	id, isNew := c.pinnedMemos.pin(role, content, turn)
	if err := c.pinnedMemos.save(c.SessionPath()); err != nil {
		return id, isNew, err
	}
	return id, isNew, nil
}

// SetPinnedMemoryPinned toggles the pinned flag for a pinned-memory entry.
// Returns true when a change was made, false when the entry was already in
// the requested state or not found.
func (c *Controller) SetPinnedMemoryPinned(id string, pinned bool) (bool, error) {
	changed := c.pinnedMemos.setPinned(id, pinned)
	if changed {
		if err := c.pinnedMemos.save(c.SessionPath()); err != nil {
			return changed, err
		}
...
func listSourceDirs(root string, maxDepth int) []string {
	skip := map[string]bool{
		...
		"vendor": true, ".WorkGround2": true, "desktop": true,

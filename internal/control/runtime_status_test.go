...
func TestCancelClearsPendingApprovalRuntimeStatus(t *testing.T) {
	...
	if st := c.RuntimeStatus(); !st.Running || !st.PendingPrompt || !st.Cancellable || st.CancelRequested || st.Mode != RuntimeModeWaitingUser || !st.ForegroundActive || !st.ActiveRuntimeWork {
	...
	if st := c.RuntimeStatus(); st.Running || st.PendingPrompt || st.Cancellable || st.CancelRequested || st.Mode != RuntimeModeIdle || st.ActiveRuntimeWork {
...
func assertCancelClearedPendingRuntimeStatus(t *testing.T, st RuntimeStatus) {
	...
	if st.Running {
		if !st.Cancellable || !st.CancelRequested || st.Mode != RuntimeModeCancelling || !st.ForegroundActive || !st.ActiveRuntimeWork {
			t.Fatalf("status immediately after cancel = %+v, want running cancelling without pending prompt", st)
		}
		return
	}
	if st.Cancellable || st.CancelRequested || st.Mode != RuntimeModeIdle || st.ActiveRuntimeWork {
...
func TestRuntimeStatusModeSeparatesBackgroundOnly(t *testing.T) {
	...
	st := c.RuntimeStatus()
	if st.Mode != RuntimeModeBackgroundOnly || !st.BackgroundOnly || !st.ActiveRuntimeWork || st.ForegroundActive || st.Cancellable || st.Running || st.PendingPrompt || st.BackgroundJobs != 1 {
		t.Fatalf("background-only status = %+v, want background_only without foreground activity", st)
	}
}

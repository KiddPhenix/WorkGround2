...
func TestConfigureCLIThemeSwitchesModeAndDefaultStyle(t *testing.T) {
	...
	t.Setenv("WorkGround2_THEME", "")
	t.Setenv("WorkGround2_THEME_STYLE", "")
...
func TestConfigureCLIThemeHonorsEnvOverride(t *testing.T) {
	t.Setenv("COLORTERM", "")
	t.Setenv("TERM_PROGRAM", "")
	t.Setenv("WorkGround2_THEME", "ember")
	t.Setenv("WorkGround2_THEME_STYLE", "")
	defer restoreThemeForTest(colorEnabled, activeCLITheme)
	colorEnabled = true
	...
	if activeCLITheme.name != "dark" || activeCLITheme.style != "ember" {
		t.Fatalf("WorkGround2_THEME override resolved %s/%s, want dark/ember", activeCLITheme.name, activeCLITheme.style)
	}
}

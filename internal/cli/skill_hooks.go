...
func (m *chatTUI) skillList() {
	...
	if len(skills) == 0 {
		m.notice("no skills found. Add SKILL.md / <name>.md under .workground2/skills (project) or ~/.workground2/skills (global); .agents/.agent/.claude skills dirs also work. Invoke with /<name> or run_skill.")
		return
	}
...
func (m *chatTUI) runHooksSubcommand(input string) {
	...
	case "trust":
		...
		m.notice("trusted this project's hooks — restart WorkGround2 to load them")
	default:

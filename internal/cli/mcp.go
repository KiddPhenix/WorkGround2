...
// mcp.go holds the MCP server-management surface shared by the `WorkGround2 mcp`
...
// mcpCommand implements `WorkGround2 mcp <add|remove|list>`. It edits config only
...
func mcpGetCLI(args []string) int {
	if len(args) == 0 {
		fmt.Fprintln(os.Stderr, "usage: WorkGround2 mcp get <name>")
		return 2
	}
	name := args[0]
	cfg, err := config.Load()
...
func printMCPEntry(entry config.PluginEntry) {
	typ := entry.Type
	if typ == "" {
		typ = "stdio"
	}
	...
	if typ == "stdio" {
		...
		if len(entry.Env) > 0 {
			...
			for _, key := range sortedMapKeys(entry.Env) {
				fmt.Printf("  %s=%s\n", key, redactMCPConfigValue(key, entry.Env[key]))
			}
		}
	} else {
		...
		if len(entry.Headers) > 0 {
			...
			for _, key := range sortedMapKeys(entry.Headers) {
				fmt.Printf("  %s=%s\n", key, redactMCPConfigValue(key, entry.Headers[key]))
			}
		}
	}
...
func mcpRemoveCLI(args []string) int {
	if len(args) == 0 {
		fmt.Fprintln(os.Stderr, "usage: WorkGround2 mcp remove <name>")
		return 2
	}
	name := args[0]
	cfg, err := config.Load()
...
func mcpUsage() {
	fmt.Println(`Manage MCP servers (persisted to WorkGround2.toml).

Usage:
  WorkGround2 mcp list
  WorkGround2 mcp get <name>
  WorkGround2 mcp add <name> <command> [args...]        stdio server
  WorkGround2 mcp add <name> --http <url> [--header K=V] remote (Streamable HTTP)
  WorkGround2 mcp add <name> --sse  <url>               remote (legacy SSE)
  WorkGround2 mcp import                                import MCP servers from cc-switch
  WorkGround2 mcp remove <name>
...
Examples:
  WorkGround2 mcp add fs npx -y @modelcontextprotocol/server-filesystem .
  WorkGround2 mcp add stripe --http https://mcp.stripe.com --header "Authorization=Bearer $STRIPE_KEY"

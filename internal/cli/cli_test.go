...
func TestMCPMigrationWaitsForCLIWorkspace(t *testing.T) {
	isolateCLIConfigHome(t)
	cwd := mustGetwd(t)
	if err := os.WriteFile(filepath.Join(cwd, "WorkGround2.toml"), []byte(`
[[plugins]]
name = "cwd-project"
...
func TestMetadataCommandsDoNotProbeTerminalTheme(t *testing.T) {
	...
	if !strings.Contains(out, "WorkGround2 test-version") {
	...
	if !strings.Contains(out, "WorkGround2 run  [--model NAME] [--max-steps N] [-c|--continue] [--resume PATH] <task>") {
		t.Fatalf("help output missing run resume flags:\n%s", out)
	}
}

func TestDesktopCommandHelp(t *testing.T) {
	errOut := captureStderr(t, func() {
		if rc := desktopCommand([]string{"--help"}); rc != 0 {
			t.Fatalf("desktop --help rc = %d, want 0", rc)
		}
	})
	if !strings.Contains(errOut, "Usage: workground2 desktop <subcommand>") || !strings.Contains(errOut, "new [prompt]") || !strings.Contains(errOut, "status") {
		t.Fatalf("desktop help output missing usage:\n%s", errOut)
	}

	errOut = captureStderr(t, func() {
		if rc := desktopCommand(nil); rc != 2 {
			t.Fatalf("desktop with no subcommand rc = %d, want 2", rc)
		}
	})
	if !strings.Contains(errOut, "Usage: workground2 desktop <subcommand>") {
		t.Fatalf("desktop no-subcommand output missing usage:\n%s", errOut)
	}
}

func TestDesktopToolApprovalModeYoloFlag(t *testing.T) {
	mode, err := desktopToolApprovalMode(true, "")
	if err != nil {
		t.Fatalf("desktopToolApprovalMode yolo: %v", err)
	}
	if mode != "yolo" {
		t.Fatalf("mode = %q, want yolo", mode)
	}
	if _, err := desktopToolApprovalMode(true, "auto"); err == nil {
		t.Fatal("--yolo with --tool-approval auto should fail")
	}
	if _, err := desktopToolApprovalMode(false, "bogus"); err == nil {
		t.Fatal("unknown tool approval mode should fail")
	}
}

func TestDesktopSessionOptionsIncludesSessionName(t *testing.T) {
	got := desktopSessionOptions("codex-worker", "yolo")
	if got["sessionName"] != "codex-worker" {
		t.Fatalf("sessionName = %q, want codex-worker", got["sessionName"])
	}
	if got["toolApprovalMode"] != "yolo" {
		t.Fatalf("toolApprovalMode = %q, want yolo", got["toolApprovalMode"])
	}
	if empty := desktopSessionOptions("", ""); empty != nil {
		t.Fatalf("empty options = %#v, want nil", empty)
	}
}
...
func TestParseDesktopAnswersGroupsMultiSelect(t *testing.T) {
	...
	if err != nil {
		t.Fatalf("parseDesktopAnswers: %v", err)
	}
	if len(got) != 2 || got[0].QuestionID != "q1" || len(got[0].Selected) != 2 || got[1].QuestionID != "q2" {
		t.Fatalf("answers = %+v", got)
	}
	if _, err := parseDesktopAnswers([]string{"missing separator"}); err == nil {
		t.Fatal("invalid answer should fail")
	}
}

func TestPrintDesktopSessionReportIncludesStatus(t *testing.T) {
	out := captureStdout(t, func() {
		printDesktopSessionReport("Submitted to", map[string]interface{}{
			"path":             `D:\repo\.workground2\sessions\abc.jsonl`,
	...
	for _, want := range []string{"Submitted to session:", `D:\repo\.workground2\sessions\abc.jsonl`, "toolApproval=yolo", "mode=turn", "running=true", "pendingPrompt=false"} {
...
func TestPrintDesktopStatusJSON(t *testing.T) {
	out := captureStdout(t, func() {
		if err := printDesktopStatus(map[string]interface{}{
			"path":             `D:\repo\.workground2\sessions\abc.jsonl`,
			"toolApprovalMode": "yolo",
			"running":          true,
			"pendingPrompt":    false,
		}, true); err != nil {
			t.Fatalf("printDesktopStatus: %v", err)
		}
	})
	...
	if got["running"] != true || got["pendingPrompt"] != false || got["toolApprovalMode"] != "yolo" {
...
func TestRunNoArgsNonInteractivePrintsUsage(t *testing.T) {
	...
	if !strings.Contains(out, "WorkGround2 —") || !strings.Contains(out, "WorkGround2 run") {
...
func TestRunMigratesLegacyConfigBeforeConfigOnlyCommands(t *testing.T) {
	isolateCLIConfigHome(t)
	legacyPath := filepath.Join(filepath.Dir(config.UserConfigPath()), "WorkGround2.toml")
...
func TestConfigAutoPlanLocalIsRejected(t *testing.T) {
	...
	if _, err := os.Stat("WorkGround2.toml"); !os.IsNotExist(err) {
		t.Fatalf("WorkGround2.toml should not be written, stat err=%v", err)
	}
...
func TestConfigAutoPlanIgnoresProjectConfig(t *testing.T) {
	...
	if err := os.WriteFile("WorkGround2.toml", []byte("[agent]\nauto_plan = \"on\"\n"), 0o644); err != nil {
...
func TestConfigReasoningLanguageLocalCreatesMinimalProjectOverride(t *testing.T) {
	...
	body, err := os.ReadFile("WorkGround2.toml")
	if err != nil {
...
func TestFetchOrFallback(t *testing.T) {
	...
	t.Run("no key set returns static list (offline first-run)", func(t *testing.T) {
		t.Setenv("WorkGround2_FETCH_TEST_KEY", "")
		probe := config.ProviderEntry{
			BaseURL:   "http://127.0.0.1:1", // unreachable, no listener
			APIKeyEnv: "WorkGround2_FETCH_TEST_KEY",
...
// into WorkGround2.toml. These collide with the wizard's own menu items, so
...
func TestWithBuiltinFamiliesDoesNotAddMissingMimo(t *testing.T) {
	// The user's case: a WorkGround2.toml that defines only deepseek providers.
...
// with a single model). Re-running `WorkGround2 setup` must still surface the
...
func TestWriteDefaultConfigOmitsLegacyInternalMCPSections(t *testing.T) {
	path := filepath.Join(t.TempDir(), "WorkGround2.toml")

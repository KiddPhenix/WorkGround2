// Package cli implements WorkGround2's command-line entry: subcommand routing, flag
...
func Run(args []string, version string) int {
	...
	case "init":
		...
		// for config), so `WorkGround2 init` isn't a dead end.
		configureCLIThemeFromConfigNoProbe()
		return initHint()
	case "acp":
		configureCLIThemeFromConfigNoProbe()
	...
	case "desktop":
		configureCLIThemeFromConfigNoProbe()
		return desktopCommand(rest)
	case "assistant":
		configureCLIThemeFromConfigNoProbe()
		return assistantCommand(rest)
	case "upgrade", "update":
		configureCLIThemeFromConfigNoProbe()
...
func shouldMigrateLegacyConfigForCLI(cmd string) bool {
	switch cmd {
	case "", "run", "chat", "code", "serve", "setup", "config", "init", "acp", "mcp", "plugin", "doctor", "bot", "assistant", "upgrade", "update":
		return true
	default:
		return false
	}
...
func runAgent(args []string) int {
	...
	if *resume != "" {
		...
		if err := leases.Rebind(sessions[0].Path); err != nil {
			fmt.Fprintln(os.Stderr, i18n.M.ErrorPrefix, control.SessionInUseMessage(err)+"; "+control.SessionLeaseCloseHint)
			return 1
		}
...
func runServe(args []string) int {
	...
	*model = modelForResumePath(*model, *resume, cfg)
	...
	fmt.Printf("WorkGround2 serve — %s on http://%s\n", ctrl.Label(), *addr)
...
func chatREPL(args []string) int {
	fs := flag.NewFlagSet("WorkGround2", flag.ContinueOnError)
	model := fs.String("model", "", "provider name (default: config default_model)")
	maxSteps := fs.Int("max-steps", 0, "max tool-call rounds (0 = use config/default)")
	cont := fs.Bool("continue", false, "resume the most recent saved session")
	fs.BoolVar(cont, "c", false, "shorthand for --continue")
...
// store. Keys always go to WorkGround2's global .env so they
...
// project-local WorkGround2.toml only when the user config dir can't be resolved.
func defaultConfigTarget() string {
	if p := config.UserConfigPath(); p != "" {
		return p
	}
	return "WorkGround2.toml"
}

// defaultEnvTarget is the display target for the WorkGround2-owned global
// WorkGround2 global .env.
func defaultEnvTarget() string {
	return config.CredentialsTargetDescription()
}

// resolveSetupTargets picks where `WorkGround2 setup` writes. Keys always go to the
// global env. The config goes to the user-global dir by default, to ./WorkGround2.toml
...
// setupConfig runs the configuration wizard (the `WorkGround2 setup` command),
// writing config.toml to the user-global dir (or ./WorkGround2.toml under --local)
// and API keys to WorkGround2's global .env — never a project's own .env.
...
// initHint handles `WorkGround2 init`. Unlike a config scaffold, project memory is
...
// (and to `WorkGround2 setup` for config) so the verb isn't a dead end.
func initHint() int {
...
// and any entered API keys to WorkGround2's global .env. The wizard
...
func interactiveSetup(configPath, envPath string) int {
	...
	fmt.Print(boxed([]string{
		accent("◆") + " " + fmt.Sprintf(i18n.M.WelcomeTitleFmt, bold("WorkGround2")),
		"",
...
// for WorkGround2's global .env later and skips re-asking. A blank entry is fine —
...
// that older versions of the wizard wrote into WorkGround2.toml. They collide
...
// can warn the user to clean up WorkGround2.toml by hand.
...
// duplicate provider entries on subsequent re-runs of `WorkGround2 setup`.
// The host-based slug also gives users a meaningful name to grep for in
// WorkGround2.toml. Falls back to a short sha1 of the raw URL when the URL
...
// not exist in config because users editing WorkGround2.toml deal with SKU names
// directly.
...
// — the key step happens later in the wizard and WorkGround2's global .env is updated then.
...
// WorkGround2 global .env. Re-pinning keeps hand-edited or previously saved values
...
// are appended — so re-running `WorkGround2 setup` with a corrected key replaces the
...
func configReasoningLanguageCommand(args []string) int {
	...
	local := fs.Bool("local", false, "write ./WorkGround2.toml instead of the user config")
	if err := fs.Parse(args); err != nil {
		return 2
	}
	rest := fs.Args()
...
func configUsage() {
	fmt.Print(`Usage:
  WorkGround2 config auto-plan [off|on]
  WorkGround2 config memory-v5 [off|observe|compact|on|status]
  WorkGround2 config reasoning-language [--local] [auto|zh|en]
`)
}

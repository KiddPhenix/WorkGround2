...
func TestMCPGetPrintsAndRedactsStdioEntry(t *testing.T) {
...
func TestMCPGetMissingServerFails(t *testing.T) {
	...
	errOut := captureStderr(t, func() {
		if rc := Run([]string{"mcp", "get", "missing"}, "test-version"); rc != 1 {
			t.Fatalf("mcp get missing rc = %d, want 1", rc)
		}
	})
	if !strings.Contains(errOut, `no MCP server named "missing"`) {
...
func TestMCPGetRedactsRemoteAuthMaterial(t *testing.T) {
	...
	for _, leaked := range []string{"Bearer abc", "access_token=abc", "key=xyz"} {
		if strings.Contains(getOut, leaked) {
...
func TestRenderMCPManagerDetailCompactsConfigPath(t *testing.T) {
	p := &mcpManager{
		...
		name:  "github",
		snapshot: mcpSnapshot{
			configPath: "/Users/example/Library/Application Support/workground2/config.toml",
			servers: []mcpServerView{{
	...
	if strings.Contains(got, "Application Support/workground2/config.toml") {
...
func TestMCPEditConfigLaunchUsesVisualBeforeEditor(t *testing.T) {
	...
	path := "/tmp/WorkGround2 config.toml"
...
func TestMCPEditConfigLaunchEditorRejectsUnterminatedQuote(t *testing.T) {
	...
	_, err := mcpEditConfigLaunchCommand("/tmp/WorkGround2.toml", func(string) (string, error) {
		t.Fatal("lookPath should not be called when VISUAL is set")
		return "", errors.New("unexpected lookup")
	})
	if err == nil {
...
func TestMCPEditConfigLaunchEditorExpandsEnvVar(t *testing.T) {
	t.Setenv("WorkGround2_TEST_EDITOR_BIN", "/opt/custom/bin/myed")
	t.Setenv("VISUAL", "$WorkGround2_TEST_EDITOR_BIN --flag")
	t.Setenv("EDITOR", "")
...
func TestMCPEditConfigLaunchEditorExpandsTilde(t *testing.T) {
	...
	for _, c := range cases {
		t.Run(c.name, func(t *testing.T) {
			...
			t.Setenv("EDITOR", "")
			launch, err := mcpEditConfigLaunchCommand("/tmp/WorkGround2.toml", func(string) (string, error) {
				t.Fatal("lookPath should not be called when VISUAL is set")
				return "", errors.New("unexpected lookup")
			})
			if err != nil {
...
func TestMCPEditConfigLaunchFallsBackToTerminalEditor(t *testing.T) {
	...
	launch, err := mcpEditConfigLaunchCommand("/tmp/WorkGround2.toml", func(name string) (string, error) {
		if name == "vim" {
	...
	if len(launch.cmd.Args) != 2 || launch.cmd.Args[0] != "/usr/bin/vim" || launch.cmd.Args[1] != "/tmp/WorkGround2.toml" {
...
func TestApplyMCPModeDropsLegacyTier(t *testing.T) {
	...
	if err := cfg.SaveTo("WorkGround2.toml"); err != nil {
	...
	m.mcp = &mcpManager{
		stage: mcpStageMode,
		name:  "github",
		snapshot: mcpSnapshot{configPath: "WorkGround2.toml", servers: []mcpServerView{{
	...
	raw, err := os.ReadFile("WorkGround2.toml")
	if err != nil {
...
func TestApplyMCPModeRecordsPluginConnectFailure(t *testing.T) {
	...
	t.Setenv("PATH", "")
	cfg := config.Default()
	cfg.Plugins = []config.PluginEntry{{Name: "broken", Command: "definitely-missing-WorkGround2-mcp", Tier: "background"}}
	if err := cfg.SaveTo("WorkGround2.toml"); err != nil {
		t.Fatalf("save config: %v", err)
	}

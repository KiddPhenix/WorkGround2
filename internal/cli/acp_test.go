...
func TestACPInitializesWithoutAPIKey(t *testing.T) {
	...
	if !strings.Contains(out, `"protocolVersion":1`) || !strings.Contains(out, `"name":"WorkGround2"`) {
...
func TestACPFactoryLoadsSessionCwdProjectConfig(t *testing.T) {
	...
	if _, err := config.SetCredential("WorkGround2_TEST_KEY", "test-key"); err != nil {
...
`), 0o644); err != nil {
	...
	cmdDir := filepath.Join(project, ".WorkGround2", "commands")

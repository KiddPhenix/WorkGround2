...
func reviewCommand(args []string) int {
	...
	// 7. Run the review subagent with a read-only tool registry.
...
func buildReviewSubagentRegistry(reviewSk skill.Skill) *tool.Registry {
	// Build a read-only subagent registry: only read-only tools plus a plan-mode-
	// safe bash wrapper are available; writers, installers, memory mutation, and
	// background job tools are blocked by construction, not just prompt text.

...
func makeTestSkills() []skill.Skill {
	return []skill.Skill{
		{Name: "review", Description: "Review code changes for correctness", Scope: skill.ScopeProject, Path: "/fake/proj/.workground2/skills/review/SKILL.md", RunAs: skill.RunSubagent, Body: "# Review\n\nReview code."},
		...
		{Name: "test", Description: "Run tests and validate behavior", Scope: skill.ScopeProject, Path: "/fake/proj/.workground2/skills/test.md", RunAs: skill.RunInline, Body: "# Test\n\nRun the tests."},
	}
}
...
func TestSkillPickerSourceView(t *testing.T) {
	skills := makeTestSkills()
	roots := []skillRootLine{
		{dir: "/fake/proj/.workground2/skills", scope: skill.ScopeProject, status: skill.StatusOK, skills: 2, diagnostic: true},
...
func TestSkillPickerDiagnostics(t *testing.T) {
	roots := []skillRootLine{
		...
		{dir: "/proj/.workground2/skills", scope: skill.ScopeProject, status: skill.StatusOK, skills: 1, diagnostic: true},
...
func TestSkillPickerDetailShowsActionsBeforeBodyPreview(t *testing.T) {
	s := skill.Skill{
		...
		Path:        "/proj/.workground2/skills/review/SKILL.md",
...
func TestSkillPickerDetailHidesProtectedBodyPreview(t *testing.T) {
	s := skill.Skill{
		Name:        "remote",
		Description: "Remote protected skill",
		Scope:       skill.ScopeRemote,
		Path:        "flow-skill-share:https://example.test/repo#skills/remote/SKILL.md",
		RunAs:       skill.RunInline,
		Body:        "REMOTE_SECRET_BODY\n" + strings.Repeat("do not preview this body\n", 5),
		Protected:   true,
		AntiLeak:    true,
	}
	m := chatTUI{
	...
	out := m.renderSkillPickerDetail()
	if strings.Contains(out, "REMOTE_SECRET_BODY") || strings.Contains(out, "do not preview") {
		t.Fatalf("protected detail leaked body:\n%s", out)
	}
	if !strings.Contains(out, "protected skill body hidden") {
		t.Fatalf("protected detail missing hidden marker:\n%s", out)
	}
}
...
func TestPathBoundaryMatching(t *testing.T) {
	skills := []skill.Skill{
		{Name: "alpha", Scope: skill.ScopeProject, Path: "/proj/.workground2/skills/alpha/SKILL.md"},
		{Name: "beta", Scope: skill.ScopeProject, Path: "/proj/.workground2/skills-extra/beta/SKILL.md"},
	}
	lines := []skillRootLine{
		{dir: "/proj/.workground2/skills", scope: skill.ScopeProject, status: skill.StatusOK},
		{dir: "/proj/.workground2/skills-extra", scope: skill.ScopeProject, status: skill.StatusOK},
	}
...
func TestLegacySkillShowUnchanged(t *testing.T) {
	s := skill.Skill{
		...
		Scope:       skill.ScopeProject,
		Path:        "/fake/proj/.workground2/skills/my-skill.md",
		Body:        "# My Skill\n\nSome body text.",
	}
...
func TestLegacySkillPathsUnchanged(t *testing.T) {
	roots := []skill.Root{
		{Dir: "/proj/.workground2/skills", Scope: skill.ScopeProject, Priority: 0, Status: skill.StatusOK},
	}

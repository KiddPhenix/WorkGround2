...
type chatTUI struct {
	...
	// leases owns the session lease guarding the TUI's active session file. Every
	// in-TUI operation that rebinds the controller must move it first.
	...
	// "/mouse" or WorkGround2_DISABLE_MOUSE at startup; trades away in-app
	// drag-select, the transcript scrollbar, and wheel-scroll while it's on,
	// since the terminal no longer forwards those events to WorkGround2.
	...
	// commands are custom slash commands loaded from .workground2/commands; each renders
...
func mouseCaptureOffByDefault() bool {
	v := strings.TrimSpace(os.Getenv("WorkGround2_DISABLE_MOUSE"))
	return v != "" && v != "0"
}
...
// toggleMouseCapture flips whether WorkGround2 owns the mouse. It's session-only
...
func (m *chatTUI) runExportCommand(input string) {
	...
	var b strings.Builder
	b.WriteString("# WorkGround2 session\n\n")
...
func renderTUIBanner(label, missing string, width int) string {
	var b strings.Builder
	b.WriteString(accent("◆") + " " + bold("WorkGround2") + "  " + dim("· "+label) + "\n")
...
var cliImageRefRe = regexp.MustCompile(`(?:^|\s)@\.workground2/attachments/clipboard-\d{8}-\d{6}\.\d+(?:-(?:\d{6}|[a-f0-9]{8}))?\.(?:png|jpg|jpeg|gif|webp)`)

...
// upgradeCommand handles `WorkGround2 upgrade` (and `WorkGround2 update`).
func upgradeCommand(args []string, version string) int {
	...
	base := fmt.Sprintf("WorkGround2-%s-%s", runtime.GOOS, runtime.GOARCH)
	var asset *ghAsset
...
// "npm-v"). Prereleases are kept: only 1.x carries `WorkGround2 upgrade`, and the
...
func fetchLatestRelease(c *http.Client) (*ghRelease, error) {
	...
	req.Header.Set("User-Agent", "WorkGround2-cli")
...
// extractBinary pulls the "WorkGround2" binary from a .tar.gz or .zip archive.
...
// rename it aside to .WorkGround2.old first, then place the new binary.

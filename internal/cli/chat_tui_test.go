...
func TestMain(m *testing.M) {
	...
	// the environment (WorkGround2_LANG/LC_ALL/LC_MESSAGES/LANG) and installs it as
...
func writeTUIImageCapabilityConfig(t *testing.T, root string) {
	...
	if err := cfg.SaveTo(filepath.Join(root, "WorkGround2.toml")); err != nil {
...
func TestMainManagerFollowsTranscriptWithoutTopPadding(t *testing.T) {
	...
	m = m0.(chatTUI)
	m.wrappedLines = []string{"WorkGround2", "› /mcp"}
	...
	if !strings.Contains(lines[0], "WorkGround2") || !strings.Contains(lines[1], "/mcp") {
...
func TestAutoPlanCommandWritesUserConfigNotProjectConfig(t *testing.T) {
	isolateUserConfig(t)
	projectPath := filepath.Join(mustGetwd(t), "WorkGround2.toml")
...
func TestLanguageCommandAutoClearsLowerPriorityUserOverride(t *testing.T) {
	...
	if err := projectCfg.SaveTo("WorkGround2.toml"); err != nil {
...
func TestSlashMigrateFromImportsExplicitSessions(t *testing.T) {
	...
	legacySessions := filepath.Join(home, "Old WorkGround2", "sessions")

...
const (
	...
	// filesystems WorkGround2 targets (APFS, ext4, NTFS all cap at 255).
	nameMaxBytes = 255
...
var (
	...
	sessionLockDispositionFallbacks atomic.Int64
)
...
// sessionLockTimeout bounds how long lockSessionFile waits to acquire the
// OS-level exclusive file lock before giving up. A live holder that crashes
// or stalls is detected via lease staleness; this timeout prevents one worker
// from blocking forever on another's lock, which was a source of "session
// drift / late write" races when multiple processes (Codex worker + desktop)
// shared the same session path.
...
func sanitizeMessagesForPersistence(msgs []provider.Message) []provider.Message {
	if len(msgs) == 0 {
		return nil
	}
	out := make([]provider.Message, len(msgs))
	for i, msg := range msgs {
		out[i] = sanitizeMessageForPersistence(msg)
	}
	return out
}
...
func (s *Session) save(path string, mode sessionSaveMode) error {
	...
	// Durable transcripts are sanitized before any digest/revision decision.
	// The operation is deterministic and idempotent, so a loaded session's
	// already-sanitized messages keep the same prefix/digest relationship to
...
func (s *Session) SaveRecoveryBranch(opts RecoveryBranchOptions) (RecoveryBranchInfo, error) {
	...
	msgs, version, rewriteVersion := s.snapshotWithVersion()
	// Same sanitization contract as save(): recovery branches are durable
	// transcripts too, and the coverage checks below compare against on-disk
	// content that save() already sanitized.
	msgs = sanitizeMessagesForPersistence(msgs)
...
type SessionInfo struct {
	...
	// AssistantID is the durable Assistant that owns this session.
	AssistantID string `json:"assistantId,omitempty"`
	// Purpose is the durable reason this session exists.

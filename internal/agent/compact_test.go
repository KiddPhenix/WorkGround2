...
func TestRenderTranscriptRedactsProtectedSkillBody(t *testing.T) {
	body := "REMOTE_SECRET_BODY " + strings.Repeat("do not summarize this protected body ", 8)
	protected := skill.Render(skill.Skill{Name: "remote", Body: body, Protected: true, AntiLeak: true}, "")
	msgs := []provider.Message{
		{Role: provider.RoleTool, Name: "run_skill", Content: protected},
	}

	out := renderTranscript(msgs)
	if strings.Contains(out, "REMOTE_SECRET_BODY") || strings.Contains(out, "do not summarize") {
		t.Fatalf("renderTranscript leaked protected body:\n%s", out)
	}
	if !strings.Contains(out, skill.ProtectedSkillRedaction) {
		t.Fatalf("renderTranscript missing protected redaction:\n%s", out)
	}
}

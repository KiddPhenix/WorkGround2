...
func TestSubagentToolRegistryFiltersUnavailableToolsAndWrapsBash(t *testing.T) {
	...
	if _, ok := sub.Get("read_skill"); !ok {
		t.Fatalf("depth-limited subagent registry should keep non-recursive read_skill; got %v", sub.Names())
	}
...
func TestReadOnlySubagentToolRegistryKeepsOnlyResearchToolsAndSafeBash(t *testing.T) {
	...
	parent.Add(subagentRegistryTool{name: "read_skill", readOnly: true})
...
func TestReadOnlySubagentToolRegistryAllowsOnlyReadOnlyDelegationBeforeDepthLimit(t *testing.T) {
	...
	for _, want := range []string{"read_file", "read_skill"} {
		if _, ok := secondLayer.Get(want); !ok {
			t.Fatalf("depth-limited read-only registry should expose %q; got %v", want, secondLayer.Names())
		}
	}
...
func TestTaskToolBuildSubRegUsesSubagentToolRegistry(t *testing.T) {
	...
	if _, ok := sub.Get("read_skill"); !ok {
		t.Fatalf("depth-limited task registry should keep non-recursive read_skill; got %v", sub.Names())
	}
	bash, ok := sub.Get("bash")
	if !ok {

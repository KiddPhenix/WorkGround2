...
func (p *ParallelTasksTool) Execute(ctx context.Context, args json.RawMessage) (string, error) {
	...
	startTask := func(idx int) {
		...
		go func() {
			...
			max := ChildStepBudget(p.taskTool.maxSteps, t.MaxSteps)
			...
			sess := NewSession("")
			opts := p.taskTool.SubagentOptions(ctx, max, childDepth, ctxWin, pricing)
			output, runErr := RunSubAgentWithSession(ctx, prov, subReg, sess, prompt, opts, nested)

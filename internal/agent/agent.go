...
type syntheticUserContextKey struct{}
...
// WithSyntheticUser marks a host-generated turn. Providers still receive a
// normal user role, while persisted history can hide it without matching text.
func WithSyntheticUser(ctx context.Context) context.Context {
	return context.WithValue(ctx, syntheticUserContextKey{}, true)
}

// SyntheticUserFromContext reports whether the current turn was generated by
// the host rather than authored by the user.
func SyntheticUserFromContext(ctx context.Context) bool {
	synthetic, _ := ctx.Value(syntheticUserContextKey{}).(bool)
	return synthetic
}

func userOrigin(ctx context.Context) provider.MessageOrigin {
	if SyntheticUserFromContext(ctx) {
		return provider.MessageOriginHost
	}
...
// maxStepsPause is control flow: the agent stopped at its configured tool-call
// budget with recoverable work already stored in the session. Callers such as
// Coordinator must propagate it instead of treating it as a provider failure.
...
func (e *maxStepsPause) Error() string {
	return fmt.Sprintf("paused after %d tool-call rounds (%s) — the work so far is saved; send another message to continue, or set %s higher or to 0 for no limit", e.rounds, e.key, e.key)
}

func newMaxStepsPause(rounds int, key string) error {
	return &maxStepsPause{rounds: rounds, key: key}
}
...
// `WorkGround2 --resume` to load a saved JSONL transcript before the first turn,
...
func (a *Agent) Run(ctx context.Context, input string) (runErr error) {
	...
	for step := 0; a.maxSteps <= 0 || step < a.maxSteps || graceRound; step++ {
		...
		// archive. For tool-call turns the openai provider replays it to satisfy
		// DeepSeek's "reasoning_content must be passed back" contract, so we store
		// the original (pre-PostLLMCall) reasoning for those turns.
		calls = a.withPreviewFileDiffs(calls)
		if guardedCalls, blocked := a.guardProtectedToolCalls(calls); blocked {
			calls = guardedCalls
			a.sink.Emit(event.Event{Kind: event.Notice, Level: event.LevelWarn, Text: "protected tool call blocked by fingerprint guard"})
		}
		a.session.Add(provider.Message{
			Role:               provider.RoleAssistant,
			Content:            text,
		...
		// A tool-call turn with no reasoning is unexpected on providers that
		// require reasoning replay (DeepSeek thinking mode). The session stays
		// recoverable because we force-include the empty key on the wire, but
		// the user should be warned.
		if len(calls) > 0 && reasoning == "" {
			if rp, ok := a.prov.(provider.ReasoningReplayProvider); ok && rp.RequiresReasoningReplay() {
				a.sink.Emit(event.Event{Kind: event.Notice, Level: event.LevelWarn,
					Text: "tool call received with no reasoning content — session is recoverable but reasoning replay may be incomplete"})
			}
		}
		...
		if len(calls) == 0 {
			...
			if readiness.reason != "" {
				...
				a.session.Add(provider.Message{Role: provider.RoleUser, Content: a.withTurnPreferences(finalReadinessRetryMessage(readiness.reason)), Origin: provider.MessageOriginHost})
			...
			if !hasVisibleFinalAnswer(text) {
				...
				a.session.Add(provider.Message{Role: provider.RoleUser, Content: a.withTurnPreferences(emptyFinalRetryMessage()), Origin: provider.MessageOriginHost})
				a.maybeCompact(ctx, usage)
				continue
			}
			if executorHandoff && !usedAnyTool && handoffNudges < maxExecutorHandoffNudges && shouldNudgeExecutorHandoff(input, text) {
				...
				a.session.Add(provider.Message{Role: provider.RoleUser, Content: a.withTurnPreferences(executorHandoffRetryMessage()), Origin: provider.MessageOriginHost})
				a.maybeCompact(ctx, usage)
				continue
			}
		...
		if a.maxSteps > 0 && step+1 >= a.maxSteps {
			graceRound = true
			nudge := fmt.Sprintf("Tool-round limit reached (%s). Do not call more tools. 不要再调用工具。 Give the concise result now: completed work, validation, remaining work, and any blocker. The user can increase %s or continue in the next turn.", a.maxStepsKey, a.maxStepsKey)
			a.session.Add(provider.Message{Role: provider.RoleUser, Content: a.withTurnPreferences(nudge), Origin: provider.MessageOriginHost})
...
// to converge on the requested outcome.
func readOnlyStreakNudgeMessage() string {
	return "Recent rounds only gathered context. Reassess the requested outcome now. 立即重新判断目标。 If implementation was requested and evidence is sufficient, make the smallest safe change and validate it. If the task is analysis, diagnosis, review, or planning, give the result now. Read more only for one specific missing fact."
}

func executorHandoffRetryMessage() string {
	return "The tool schema is still attached to this executor request. Use your available tools now to carry out the task. 工具仍可用，立即执行。 Do not invent that MCP servers or tools are unavailable; require a real call failure. For a user-owned choice, use the ask tool and wait for its tool result; do not ask in prose; do not claim the user answered. Report the exact permission or workspace blocker."
}
...
func (a *Agent) stream(ctx context.Context, turn int) (string, string, string, []provider.ToolCall, *provider.Usage, bool, bool, error) {
	...
	protectOutput := a.hasProtectedSkillContext()

	// needsReasoningReplay is set when the active provider requires
	// reasoning_content replay on tool-call turns (DeepSeek thinking mode).
	needsReasoningReplay := false
	if rp, ok := a.prov.(provider.ReasoningReplayProvider); ok {
		needsReasoningReplay = rp.RequiresReasoningReplay()
	}
	...
	finishReasoning := func() (stored, display string) {
		...
		display = original
		if transformReasoning && original != "" && !protectOutput {
			display = a.hooks.PostLLMCall(ctx, original, turn)
			if display != "" {
				a.sink.Emit(event.Event{Kind: event.Reasoning, Text: display})
			}
		}
		if protectOutput && display != "" {
			if guarded, blocked := a.guardProtectedOutput(display); blocked {
				display = guarded
			}
			return "", ""
		}
		stored = display
		if signature != "" || (len(calls) > 0 && needsReasoningReplay) {
			stored = original
		}
	...
	for {
		...
		case <-ctx.Done():
			...
			if protectOutput {
				finalText, _ = a.guardProtectedOutput(finalText)
			}
			return finalText, stored, signature, calls, usage, false, partialToolStarted, ctx.Err()
		case c, ok := <-ch:
			if !ok {
				if err := ctx.Err(); err != nil {
					...
					return finalText, stored, signature, calls, usage, false, partialToolStarted, err
				}
				stored, display := finishReasoning()
				finalText := text.String()
				if protectOutput {
					var blocked bool
					finalText, blocked = a.guardProtectedOutput(finalText)
					if blocked {
						a.sink.Emit(event.Event{Kind: event.Notice, Level: event.LevelWarn, Text: "protected output blocked by fingerprint guard"})
					}
					if finalText != "" {
						a.sink.Emit(event.Event{Kind: event.Text, Text: finalText})
					}
				}
				...
				return finalText, stored, signature, calls, usage, false, false, nil
			}
			chunk = c
		}
		switch chunk.Type {
		case provider.ChunkReasoning:
			...
			if chunk.Text != "" && !transformReasoning && !protectOutput {
		...
		case provider.ChunkError:
			if provider.IsStreamInterrupted(chunk.Err) {
				...
				return finalText, stored, signature, calls, usage, true, partialToolStarted, chunk.Err
			}
...
func (a *Agent) executeBatch(ctx context.Context, calls []provider.ToolCall) []string {
	...
	for i, c := range calls {
		o := outcomes[i]
		t, ok := a.tools.Get(c.Name)
		displayOutput := redactProtectedText(o.output)
		a.sink.Emit(event.Event{Kind: event.ToolResult, Tool: event.Tool{
			ID:         c.ID,
			Name:       c.Name,
			Args:       c.Arguments,
	...
	if a.compilerTurn != nil {
		...
		for i, c := range calls {
			...
			records = append(records, memorycompiler.ToolRecord{
				...
				Name:       c.Name,
				Args:       c.Arguments,
				Output:     redactProtectedText(o.output),
				Error:      o.errMsg,
				ReadOnly:   ok && t.ReadOnly(),
				Blocked:    o.blocked,
				DurationMs: durations[i],
...
func (a *Agent) executeOne(ctx context.Context, call provider.ToolCall) toolOutcome {
	if protectedToolCallBlocked(call) {
		return toolOutcome{
			output:  "blocked: " + ProtectedOutputBlockedText(),
			blocked: true,
			errMsg:  "blocked by protected output guard",
		}
	}
	t, ok := a.tools.Get(call.Name)
	if !ok {

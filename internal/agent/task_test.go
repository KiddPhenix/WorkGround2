...
func TestTaskToolInheritsResponseLanguageFromContext(t *testing.T) {
	sub := &mockProvider{name: "sub", chunks: []provider.Chunk{
		{Type: provider.ChunkText, Text: "done"},
		{Type: provider.ChunkDone},
	}}
	...
	ctx := WithResponseLanguagePreference(testTaskContext(), "en")
	if _, err := task.Execute(ctx, []byte(`{"prompt":"inspect auth"}`)); err != nil {
		t.Fatalf("Execute: %v", err)
	}
	got := lastUser(sub.lastReq)
	if !strings.HasPrefix(got, "<response-language>") || !strings.Contains(got, "use English") || !strings.HasSuffix(got, "inspect auth") {
		t.Fatalf("sub-agent user = %q, want response-language-prefixed prompt", got)
	}
}

func TestTaskToolSubagentOptionsInheritRuntimeSettings(t *testing.T) {
	price := &provider.Pricing{Input: 1}
	keepPolicy := KeepErrors | KeepUserMarked
	task := &TaskTool{
		temperature:         0.3,
		recentKeep:          8,
		softCompactRatio:    0.6,
		toolResultSnipRatio: 0.7,
		compactRatio:        0.8,
	...
	ctx := WithReasoningLanguagePreference(WithResponseLanguagePreference(context.Background(), "en"), "zh")
	opts := task.SubagentOptions(ctx, 11, 2, 12345, price)
	if opts.MaxSteps != 11 || opts.SubagentDepth != 2 || opts.MaxSubagentDepth != 3 || opts.ContextWindow != 12345 {
		t.Fatalf("subagent scalar options = %+v", opts)
	}
	if opts.Pricing != price || opts.Temperature != 0.3 || opts.RecentKeep != 8 || opts.ArchiveDir != "archive" || opts.KeepPolicy != keepPolicy {
		t.Fatalf("subagent inherited options = %+v", opts)
	}
	if opts.SoftCompactRatio != 0.6 || opts.ToolResultSnipRatio != 0.7 || opts.CompactRatio != 0.8 || opts.CompactForceRatio != 0.9 {
		t.Fatalf("subagent compaction options = %+v", opts)
	}
	if opts.ResponseLanguage != "en" || opts.ReasoningLanguage != "zh" {
		t.Fatalf("subagent language options = %+v", opts)
	}
}
...
// TestTaskToolRunsEphemerallyWithoutParentSession mirrors headless `WorkGround2 run`:

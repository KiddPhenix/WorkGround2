...
// directory. It is gated behind WorkGround2_BENCH=1 so it never runs (or writes
// hundreds of MB) during a normal `go test`.
//
//	WorkGround2_BENCH=1 go test ./internal/agent -run TestListSessionsScalingManual -v
func TestListSessionsScalingManual(t *testing.T) {
	if os.Getenv("WorkGround2_BENCH") != "1" {
		t.Skip("set WorkGround2_BENCH=1 to run the scaling benchmark")
	}
...
//
//	WorkGround2_BENCH=1 go test ./internal/agent -run TestColdStartProjectTreeManual -v
func TestColdStartProjectTreeManual(t *testing.T) {
	if os.Getenv("WorkGround2_BENCH") != "1" {
		t.Skip("set WorkGround2_BENCH=1 to run the cold-start benchmark")
	}

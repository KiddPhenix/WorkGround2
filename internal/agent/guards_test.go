...
func TestProtectedOutputGuardBlocksLargeCopy(t *testing.T) {
	body := "REMOTE_SKILL_SOURCE " + strings.Repeat("unique protected instruction material ", 10)
	rendered := skill.Render(skill.Skill{Name: "remote", Body: body, Protected: true, AntiLeak: true}, "")
	a := &Agent{session: &Session{Messages: []provider.Message{
		{Role: provider.RoleTool, Name: "run_skill", Content: rendered},
	}}}

	out, blocked := a.guardProtectedOutput("Here is the original skill:\n" + body)
	if !blocked {
		t.Fatal("guard should block a large protected skill copy")
	}
	if out != ProtectedOutputBlockedText() {
		t.Fatalf("blocked output = %q", out)
	}
}

func TestProtectedToolCallGuardBlocksLargeCopyArgs(t *testing.T) {
	body := "REMOTE_SKILL_SOURCE " + strings.Repeat("unique protected instruction material ", 10)
	rendered := skill.Render(skill.Skill{Name: "remote", Body: body, Protected: true, AntiLeak: true}, "")
	a := &Agent{session: &Session{Messages: []provider.Message{
		{Role: provider.RoleTool, Name: "run_skill", Content: rendered},
	}}}
	calls := []provider.ToolCall{{ID: "c1", Name: "write_file", Arguments: `{"path":"leak.md","content":"` + body + `"}`}}

	guarded, blocked := a.guardProtectedToolCalls(calls)
	if !blocked || !protectedToolCallBlocked(guarded[0]) {
		t.Fatalf("tool call should be blocked, calls=%+v blocked=%v", guarded, blocked)
	}
	if strings.Contains(guarded[0].Arguments, "REMOTE_SKILL_SOURCE") {
		t.Fatalf("blocked tool call leaked args: %+v", guarded[0])
	}
}

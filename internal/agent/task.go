...
// ChildStepBudget computes the effective max-steps budget for a child sub-agent.
// An explicit child max (> 0) wins; a finite parent max (> 0) derives half with
// a floor of 5 so a delegated sub-task stays shorter than the whole turn; an
// unbounded parent (0) remains unbounded when the child has no explicit cap.
// Parallel children must use the same helper for consistent budget computation.
func ChildStepBudget(parentMax, childMax int) int {
	if childMax > 0 {
		return childMax
	}
	if parentMax <= 0 {
...
func (r *ReadOnlyTaskTool) Execute(ctx context.Context, args json.RawMessage) (string, error) {
	...
	maxSteps := ChildStepBudget(r.task.maxSteps, p.MaxSteps)
...
func (t *TaskTool) Execute(ctx context.Context, args json.RawMessage) (string, error) {
	...
	maxSteps := ChildStepBudget(t.maxSteps, p.MaxSteps)
...
func (t *TaskTool) prepareTranscriptRun(subReg *tool.Registry, modelRef, effortRef, parentSession, parentID, continueFrom, legacyForkFrom string) (*SubagentRun, error) {
	...
	// Headless runs (e.g. `WorkGround2 run`) never mint a session path, so there is
...
// subagents. Depth-capped registries retain read_skill because it is
// non-recursive: it only loads an inline skill body.
...
func (t *TaskTool) runSubSession(ctx context.Context, prompt string, subReg *tool.Registry, sink event.Sink, maxSteps int, prov provider.Provider, pricing *provider.Pricing, ctxWin int, sess *Session, childDepth int) (string, error) {
	opts := t.SubagentOptions(ctx, maxSteps, childDepth, ctxWin, pricing)
	return RunSubAgentWithSession(ctx, prov, subReg, sess, prompt, opts, sink)
}

// SubagentOptions builds the agent.Options for a child sub-agent, inheriting
// compaction ratios, archive/keep policy, live response/reasoning language, and
// depth from the parent TaskTool.
func (t *TaskTool) SubagentOptions(ctx context.Context, maxSteps, childDepth, ctxWin int, pricing *provider.Pricing) Options {
	return Options{
		MaxSteps:            maxSteps,
...
// GuardSubagentHostDecisionText shares the boundary policy with skill-based
// sub-agents through the lower-level tool package.

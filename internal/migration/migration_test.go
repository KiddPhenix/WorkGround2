...
func isolateMigrationHome(t *testing.T) string {
	...
	t.Setenv("WorkGround2_HOME", filepath.Join(home, "new-WorkGround2"))
...
func TestRunLegacyRescueImportsMemory(t *testing.T) {
	home := isolateMigrationHome(t)
	legacyRoot := filepath.Join(home, ".WorkGround2")
	if err := os.MkdirAll(filepath.Join(legacyRoot, "memory", "global"), 0o755); err != nil {
		t.Fatal(err)
	}
	if err := os.WriteFile(filepath.Join(legacyRoot, "WorkGround2.md"), []byte("legacy user memory\n"), 0o644); err != nil {
		t.Fatal(err)
	}
	...
	for _, path := range []string{
		filepath.Join(config.MemoryUserDir(), "WorkGround2.md"),
...
func TestRunLegacyRescueCommandImportsFromExplicitInstallDir(t *testing.T) {
	home := isolateMigrationHome(t)
	installRoot := filepath.Join(home, "Custom WorkGround2")
...
func TestMigrateLegacySessionSourcesSkipsCurrentProjectTree(t *testing.T) {
	...
	if !samePath(config.MemoryUserDir(), filepath.Join(home, ".WorkGround2")) {
		t.Skip("current WorkGround2 home is not ~/.WorkGround2 on this platform")
	}

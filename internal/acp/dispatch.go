...
var locationTools = map[string]bool{
	...
	"notebook_edit": true, "delete_range": true, "delete_symbol": true, "code_index": true,
}
...
func planEntriesFromTodoArgs(rawArgs string) ([]PlanEntry, bool) {
	...
	for _, todo := range p.Todos {
		...
		entries = append(entries, PlanEntry{Content: todo.Content, Priority: priority, Status: status})
	}

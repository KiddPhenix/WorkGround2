...
func TestIsReadOnlyBashSubject(t *testing.T) {
	tests := []struct {
		...
		{"awk 'BEGIN{system(\"touch /tmp/WorkGround2-awk-test\")}'", false},
		{"awk 'BEGIN{print \"evil\" > \"/tmp/WorkGround2-awk-test\"}'", false},
		{"sed 's/a/b/' file.txt", false},
		{"sed -e 's/.*/touch \\/tmp\\/WorkGround2-sed-test/e' file.txt", false},

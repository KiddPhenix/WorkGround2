...
func NewTool(opts Options) tool.Tool {
	...
	} else if dir := config.WorkGround2HomeDir(); dir != "" {
		WorkGround2Home = dir
	} else if home != "" {
		WorkGround2Home = filepath.Join(home, ".WorkGround2")
	}
	...
	return &installSourceTool{
		root:            root,
		home:            home,
		WorkGround2Home: WorkGround2Home,
		httpClient:      client,
...
func (t *installSourceTool) Execute(ctx context.Context, raw json.RawMessage) (string, error) {
	...
	if len(actions) == 0 {
		out := response{
			...
			PlanID:   planID,
			Warnings: warnings,
			Next:     "No installable WorkGround2 skill, MCP server, or plugin package was detected. Ask the user for a direct SKILL.md, skill root, .mcp.json, plugin manifest, MCP endpoint, or package name.",
		}
...
func (t *installSourceTool) uninstallActionsForScope(name, scope string) []action {
	...
	if scope == "global" || scope == "" {
		if st, err := pluginpkg.LoadState(t.WorkGround2Home); err == nil {
			for _, p := range st.Plugins {
				if p.Name != name {
					continue
				}
				root := pluginpkg.ResolveRoot(t.WorkGround2Home, p.Root)
				actions = append(actions, action{
					Kind:         "plugin",
...
func (t *installSourceTool) resolveSkillPath(name, scope string) (string, bool) {
	...
	if scope == "global" {
		...
		root = filepath.Join(t.WorkGround2Home, skill.SkillsDirname)
	} else {
		root = filepath.Join(t.root, ".WorkGround2", skill.SkillsDirname)
	}
...
func (t *installSourceTool) configPath(scope string) string {
	...
	return filepath.Join(t.root, "WorkGround2.toml")
}

...
func TestMain(m *testing.M) {
	dir, err := os.MkdirTemp("", "WorkGround2-installsource-test-*")
	if err != nil {
...
func zipDir(t *testing.T, src, archivePath, rootName string) {
	t.Helper()
	...
	for _, file := range files {
		...
		header.Name = pathJoinSlash(rootName, rel)
...
func TestApplyLocalSkillRootRegistersPath(t *testing.T) {
	...
	cfg := config.LoadForEdit(filepath.Join(project, "WorkGround2.toml"))
...
func TestApplyLocalCodexPluginPackage(t *testing.T) {
  ...
  "skills": "./skills/"
	...
	statePath := filepath.Join(home, ".WorkGround2", "plugin-packages.json")
	...
	if _, err := os.Stat(filepath.Join(home, ".WorkGround2", "plugins", "superpowers", ".codex-plugin", "plugin.json")); err != nil {
...
func TestApplyLocalPluginPackageZip(t *testing.T) {
	project := t.TempDir()
	home := t.TempDir()
	src := filepath.Join(t.TempDir(), "draw-tool")
	writeFile(t, filepath.Join(src, pluginpkg.NativeManifest), `{
  "name": "draw-tool",
  "version": "0.1.0",
  "description": "Draw AddOn archive",
  "addon": {
    "kind": "draw-tool",
    ...
    "secrets": [{ "id": "provider-api-key", "label": "Provider API key" }],
    "storage": { "namespace": "draw-tool" }
  },
  "mcpServers": {
    "draw-addon": { "command": "bin/draw-addon", "auto_start": true }
  }
}`)
	writeFile(t, filepath.Join(src, "panels", "providers.schema.json"), `{"type":"object"}`)
	writeFile(t, filepath.Join(src, "bin", "draw-addon"), "fake runtime")
	archivePath := filepath.Join(t.TempDir(), "draw-tool.zip")
	zipDir(t, src, archivePath, "draw-tool")
	...
	if act.Name != "draw-tool" || act.Source != archivePath || act.AddOnKind != "draw-tool" || act.AddOnRuntime != "mcp" {
		t.Fatalf("plugin action = %+v", act)
	}
	if act.ToolCount != 1 || act.AddOnPanels != 1 || act.AddOnSecrets != 1 {
		t.Fatalf("capability counts = %+v", act)
	}
	...
	if len(st.Plugins) != 1 || st.Plugins[0].Name != "draw-tool" || st.Plugins[0].Source != archivePath {
		t.Fatalf("state = %+v", st.Plugins)
	}
	installedRoot := filepath.Join(stateHome, "plugins", "draw-tool")
	for _, rel := range []string{
		pluginpkg.NativeManifest,
		filepath.Join("panels", "providers.schema.json"),
		filepath.Join("bin", "draw-addon"),
	} {
		if _, err := os.Stat(filepath.Join(installedRoot, rel)); err != nil {
			t.Fatalf("installed %s missing: %v", rel, err)
		}
	}
	t.Setenv("WorkGround2_HOME", stateHome)
	cfg, err := config.LoadForRoot(project)
	if err != nil {
		t.Fatal(err)
	}
	var drawEntry config.PluginEntry
	for _, entry := range cfg.Plugins {
		if entry.Name == "draw-addon" {
			drawEntry = entry
			break
		}
	}
	if drawEntry.Name == "" {
		t.Fatalf("draw-addon MCP entry was not merged into config: %+v", cfg.Plugins)
	}
	if got, want := drawEntry.Command, filepath.Join(installedRoot, "bin", "draw-addon"); got != want {
		t.Fatalf("draw-addon command = %q, want %q", got, want)
	}
	if drawEntry.Env["WORKGROUND2_ADDON_KIND"] != "draw-tool" || drawEntry.Env["WORKGROUND2_ADDON_CONFIG_DIR"] == "" {
		t.Fatalf("draw-addon env = %+v", drawEntry.Env)
	}
}

func TestApplyLocalPluginPackagePreservesInstalledAtOnReplace(t *testing.T) {
	project := t.TempDir()
	home := t.TempDir()
	src := filepath.Join(t.TempDir(), "team-skills")
	writeFile(t, filepath.Join(src, pluginpkg.NativeManifest), `{
  "name": "team-skills",
  "version": "1.0.0",
  "description": "Shared skills v1",
  "skills": "skills",
  "addon": {
    "kind": "skill-share",
    "storage": { "namespace": "team-skills" }
  }
}`)
	writeFile(t, filepath.Join(src, "skills", "alpha", "SKILL.md"), "---\nname: alpha\ndescription: Alpha\n---\nUse alpha.")

	tl := NewTool(Options{ProjectRoot: project, HomeDir: home})
	first := execInstall(t, tl, map[string]any{
		"source": src,
		"kind":   "plugin",
		"apply":  true,
	})
	...
	if len(st.Plugins) != 1 || st.Plugins[0].InstalledAt == "" {
		t.Fatalf("state after first install = %+v", st.Plugins)
	}
	installedAt := st.Plugins[0].InstalledAt
	p := st.Plugins[0]
	p.LastCheckedAt = "2026-07-04T01:02:03Z"
	p.LastError = "old network error"
	p.UpdateAvailable = true
	p.RemoteVersion = "1.1.0"
	if err := pluginpkg.Upsert(stateHome, p); err != nil {
		t.Fatal(err)
	}
  ...
  "addon": {
	...
	second := execInstall(t, tl, map[string]any{
		"source":  src,
		"kind":    "plugin",
		"apply":   true,
		"replace": true,
	...
	if len(st.Plugins) != 1 {
		t.Fatalf("state after replace = %+v", st.Plugins)
	}
	got := st.Plugins[0]
	if got.Version != "1.1.0" || got.Description != "Shared skills v2" {
		t.Fatalf("plugin metadata = %+v", got)
	}
	if got.InstalledAt != installedAt || got.LastUpdatedAt == "" {
		t.Fatalf("timestamps = %+v; want installedAt preserved and lastUpdatedAt set", got)
	}
	if got.LastCheckedAt != "2026-07-04T01:02:03Z" || got.LastError != "" || got.UpdateAvailable {
...
func TestApplyLocalSkillFileCopiesToProject(t *testing.T) {
	...
	target := filepath.Join(project, ".WorkGround2", "skills", "beta", "SKILL.md")
...
func TestApplyLocalSkillFileDoesNotShadowFlatCompatInstall(t *testing.T) {
	project := t.TempDir()
	home := t.TempDir()
	existing := filepath.Join(project, ".WorkGround2", "skills", "beta.md")
...
func TestApplyLocalSKILLFileCopiesSiblingResources(t *testing.T) {
	...
	target := filepath.Join(project, ".WorkGround2", "skills", "frontend-design", "SKILL.md")
	if resp.Actions[0].CanonicalPath != target {
		t.Fatalf("canonicalPath = %q, want %q", resp.Actions[0].CanonicalPath, target)
	}
	if _, err := os.Stat(filepath.Join(project, ".WorkGround2", "skills", "frontend-design", "references", "style.md")); err != nil {
		t.Fatalf("reference file should be copied with SKILL.md source: %v", err)
	}
	if _, err := os.Stat(filepath.Join(project, ".WorkGround2", "skills", "frontend-design", "scripts", "lint.sh")); err != nil {
...
func TestApplyLocalSkillLinkMode(t *testing.T) {
	...
	target := filepath.Join(project, ".WorkGround2", "skills", "gamma", "SKILL.md")
...
func TestPlanProjectMCPJSONDefaultsProject(t *testing.T) {
  ...
  "mcpServers": {
	...
	wantPath := filepath.Join(project, "WorkGround2.toml")
...
func TestApplyRemoteMCPURLDefaultsGlobal(t *testing.T) {
	...
	projectCfg := config.LoadForEdit(filepath.Join(project, "WorkGround2.toml"))
...
func TestApplyMCPRejectsDuplicateByDefault(t *testing.T) {
	...
	if err := cfg.SaveTo(filepath.Join(project, "WorkGround2.toml")); err != nil {
		t.Fatal(err)
	}
...
func TestApplyMCPReplaceOverwritesExisting(t *testing.T) {
	...
	reloaded := config.LoadForEdit(filepath.Join(project, "WorkGround2.toml"))
...
func TestApplyMCPRollsBackOnSaveFailure(t *testing.T) {
	...
	if err := os.MkdirAll(filepath.Join(project, "WorkGround2.toml"), 0o755); err != nil {
		t.Fatal(err)
	}
	writeFile(t, filepath.Join(project, "WorkGround2.toml", "blocker"), "x")
...
func TestGlobalSkillInstallRootUsesWorkGround2Home(t *testing.T) {
	home := t.TempDir()
	WorkGround2Home := filepath.Join(t.TempDir(), "rx-home")
	t.Setenv("HOME", home)
	t.Setenv("USERPROFILE", home)
	t.Setenv("WorkGround2_HOME", WorkGround2Home)
	...
	want := filepath.Join(WorkGround2Home, skill.SkillsDirname)
	if root != want {
...
func TestUninstallRemovesSkillByName(t *testing.T) {
	project := t.TempDir()
	home := t.TempDir()
	target := filepath.Join(project, ".WorkGround2", "skills", "doomed.md")
	writeFile(t, target, "---\nname: doomed\ndescription: Doomed\n---\nbody")
...
func TestUninstallWithoutScopePrefersProjectSkill(t *testing.T) {
	project := t.TempDir()
	home := t.TempDir()
	projectTarget := filepath.Join(project, ".WorkGround2", "skills", "dupe.md")
	globalTarget := filepath.Join(home, ".WorkGround2", "skills", "dupe.md")
...
func TestPlanIDIncludesActionDetails(t *testing.T) {
	...
	a := action{Kind: "mcp", Action: "install_mcp_server", Name: "same", URL: "https://mcp.one.example/mcp", Transport: "http", ConfigPath: "/repo/WorkGround2.toml"}
	b := a

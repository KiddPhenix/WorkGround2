...
const macBundleID = "com.wails.WorkGround2-desktop"
...
func applyMac(zipPath string) error {
	...
	staging, err := os.MkdirTemp("", "WorkGround2-mac-update-*")
	if err != nil {
		return err
	}
	handedOff := false
	...
	script := filepath.Join(staging, "install-WorkGround2-update.sh")
	body := fmt.Sprintf(`#!/bin/sh
set -eu
old_app=%q
new_app=%q
backup_app="$old_app.WorkGround2-update-backup"
sleep 1
rm -rf "$backup_app"
...
func findMacApp(root string) (string, error) {
	direct := filepath.Join(root, "WorkGround2.app")

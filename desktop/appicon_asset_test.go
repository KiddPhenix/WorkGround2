...
func TestAppIconPNGUsesDarkRoundedBackground(t *testing.T) {
...
func TestWindowsICOUsesDarkRoundedBackground(t *testing.T) {
...
func assertFullCanvasRoundedIcon(t *testing.T, img image.Image, size int) {
	...
	for _, point := range edgePoints {
		...
		assertWorkGround2Dark(t, point.name, img.At(point.x, point.y))
	}
}

func assertWorkGround2Dark(t *testing.T, name string, colorValue color.Color) {
	t.Helper()

	r16, g16, b16, _ := colorValue.RGBA()
	r, g, b := uint8(r16>>8), uint8(g16>>8), uint8(b16>>8)
	if r > 0x24 || g > 0x2c || b > 0x3c {
		t.Fatalf("%s edge must use WorkGround2 dark icon background, got #%02x%02x%02x", name, r, g, b)
	}
}

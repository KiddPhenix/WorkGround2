...
func workflowCategories() []workflowCategory {
	return []workflowCategory{
		...
		{
			Name:        "WorkGround2-memory-ui",
			Description: "Iterate on the WorkGround2 desktop Memory page with source-backed UI decisions and browser verification.",
...
func skillCandidateBody(cat workflowCategory, evidence []string) string {
	var b strings.Builder
	title := strings.TrimPrefix(strings.ReplaceAll(cat.Name, "-", " "), "WorkGround2 ")
	b.WriteString("# " + cases.Title(language.Und).String(title) + "\n\n")
	b.WriteString("Use this skill when the user asks for this repeated WorkGround2 workflow.\n\n")

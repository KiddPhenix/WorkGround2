...
// by substring (file names embed the key, e.g. WorkGround2-darwin-arm64.zip), so the
...
// (WorkGround2.key) and the public key (WorkGround2.pub) into dir. The password comes
...
func genKey(dir string) error {
	...
	keyPath := filepath.Join(dir, "WorkGround2.key")
	pubPath := filepath.Join(dir, "WorkGround2.pub")
...
func signFiles(files []string) error {
	...
	for _, f := range files {
		...
		sig := minisign.SignWithComments(priv, data,
			"file:"+filepath.Base(f), "WorkGround2 desktop release")
		out := f + ".minisig"

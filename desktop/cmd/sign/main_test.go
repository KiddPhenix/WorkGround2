...
func TestSignFiles(t *testing.T) {
	...
	dir := t.TempDir()
	artifact := filepath.Join(dir, "WorkGround2-linux-amd64.tar.gz")
...
func TestGenManifest(t *testing.T) {
	dir := t.TempDir()
	names := []string{
		"WorkGround2-darwin-arm64.zip",
		"WorkGround2-darwin-amd64.zip",
		"WorkGround2-windows-amd64-installer.exe",
		"WorkGround2-windows-amd64.zip", // portable download, not the updater channel
		"WorkGround2-windows-arm64-installer.exe",
		"WorkGround2-windows-arm64.zip", // portable download, not the updater channel
		"WorkGround2-linux-amd64.tar.gz",
		"WorkGround2-linux-amd64.deb",            // human download, not the updater channel
		"WorkGround2-linux-amd64.tar.gz.minisig", // must be skipped
	...
	t.Setenv("GITHUB_REPOSITORY", "KiddPhenix/WorkGround2")
	...
	wantURL := "https://github.com/KiddPhenix/WorkGround2/releases/download/desktop-v1.2.0/WorkGround2-windows-amd64-installer.exe"
	if win.URL != wantURL {
	...
	if !strings.HasSuffix(arm.URL, "/WorkGround2-windows-arm64-installer.exe") {
	...
	if !strings.HasSuffix(lin.URL, "/WorkGround2-linux-amd64.tar.gz") {

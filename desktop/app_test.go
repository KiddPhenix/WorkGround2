...
func fakeLocalCLI(t *testing.T, name string) string {
	t.Helper()
	dir := t.TempDir()
	if runtime.GOOS == "windows" {
		path := filepath.Join(dir, name+".cmd")
		if err := os.WriteFile(path, []byte("@echo fake-cli 1.0\r\n"), 0o755); err != nil {
...
func withLocalCLIPresets(t *testing.T, presets []onboardingLocalCLIPreset) {
	t.Helper()
	previous := onboardingLocalCLIPresets
	onboardingLocalCLIPresets = presets
	t.Cleanup(func() {
		onboardingLocalCLIPresets = previous
	})
}

func TestCodexLocalCLIPresetUsesJSONLStream(t *testing.T) {
	var codex *onboardingLocalCLIPreset
	for i := range onboardingLocalCLIPresets {
		if onboardingLocalCLIPresets[i].ID == "codex" {
			codex = &onboardingLocalCLIPresets[i]
			break
		}
	}
	if codex == nil {
	...
	args := "\x00" + strings.Join(codex.Args, "\x00") + "\x00"
	if !strings.Contains(args, "\x00--json\x00") {
		t.Fatalf("codex args should enable JSONL stdout events: %+v", codex.Args)
	}
...
func TestNeedsOnboardingIgnoresInheritedEnv(t *testing.T) {
	...
	if !app.NeedsOnboarding() {
		t.Fatal("NeedsOnboarding should require a key saved in WorkGround2 global .env")
	}
...
func TestScanLocalCLIProvidersFindsExecutablePath(t *testing.T) {
	...
	if len(options) != 1 {
		t.Fatalf("scan returned %d options, want 1: %+v", len(options), options)
	}
	got := options[0]
	if !got.Installed {
		t.Fatalf("fake CLI should be installed: %+v", got)
	}
	if got.Command != command {
		t.Fatalf("command = %q, want %q", got.Command, command)
	}
	if !reflect.DeepEqual(got.Args, []string{"run", "--stdin"}) {
		t.Fatalf("args = %+v", got.Args)
	}
	if got.Protocol != "text" || got.Model != "default" || got.TimeoutSeconds != onboardingLocalCLITimeoutSeconds {
		t.Fatalf("defaults = protocol:%q model:%q timeout:%d", got.Protocol, got.Model, got.TimeoutSeconds)
	}
}

func TestScanLocalCLIProvidersReturnsEmptyArgsArray(t *testing.T) {
	command := fakeLocalCLI(t, "WorkGround2-empty-args-cli")
	withLocalCLIPresets(t, []onboardingLocalCLIPreset{{
		ID:       "empty",
		Name:     "Empty Args CLI",
		Commands: []string{command},
	...
	if options[0].Args == nil {
		t.Fatal("Args should be an empty array, not nil/null")
	}
	...
	if strings.Contains(string(data), `"args":null`) {
		t.Fatalf("args should serialize as [] instead of null: %s", data)
	}
}

func TestConnectLocalCLIProviderPersistsDefaultProvider(t *testing.T) {
	isolateDesktopUserDirs(t)
	command := fakeLocalCLI(t, "WorkGround2-fake-cli")
	withLocalCLIPresets(t, []onboardingLocalCLIPreset{{
		ID:       "fake",
	...
	app := NewApp()
	if err := app.ConnectLocalCLIProvider("fake"); err != nil {
		t.Fatalf("ConnectLocalCLIProvider: %v", err)
	}
	if app.NeedsOnboarding() {
		t.Fatal("NeedsOnboarding should be false after local CLI provider setup")
	}
	cfg := config.LoadForEdit(config.UserConfigPath())
	p, ok := cfg.Provider("local-fake")
	if !ok {
		t.Fatalf("local-fake provider missing: %+v", cfg.Providers)
	}
	if p.Kind != "cli" || p.Command != command {
		t.Fatalf("provider = %+v, want cli command %q", p, command)
	}
	if !reflect.DeepEqual(p.Args, []string{"run", "--stdin"}) {
		t.Fatalf("provider args = %+v", p.Args)
	}
	if p.DefaultModel() != "default" || cfg.DefaultModel != "local-fake/default" {
		t.Fatalf("model refs = provider:%q default:%q", p.DefaultModel(), cfg.DefaultModel)
	}
	if !providerAccessSet(cfg.Desktop.ProviderAccess)["local-fake"] {
		t.Fatalf("provider_access = %+v, want local-fake", cfg.Desktop.ProviderAccess)
	}
}
...
func TestSkipOnboardingPersistsProviderAccessWithoutKey(t *testing.T) {
	...
	app := NewApp()
	if !app.NeedsOnboarding() {
		t.Fatal("fresh desktop install should need onboarding before key or provider setup")
	}
	if err := app.SkipOnboarding(); err != nil {
		t.Fatalf("SkipOnboarding: %v", err)
	}
	if app.NeedsOnboarding() {
		t.Fatal("NeedsOnboarding should be false after explicit provider access is saved")
	}
	if data, err := os.ReadFile(config.UserCredentialsPath()); err == nil && strings.Contains(string(data), onboardingKeyEnv) {
		t.Fatalf("SkipOnboarding should not write a blank credential:\n%s", data)
	}
	cfg := config.LoadForEditWithoutCredentials(config.UserConfigPath())
	if !providerAccessSet(cfg.Desktop.ProviderAccess)["deepseek"] {
		t.Fatalf("provider_access = %+v, want deepseek", cfg.Desktop.ProviderAccess)
	}
	if p, ok := cfg.Provider("deepseek"); !ok || strings.TrimSpace(p.APIKeyEnv) != onboardingKeyEnv {
		t.Fatalf("deepseek provider = %+v, ok=%v", p, ok)
	}
}

func TestNeedsOnboardingAllowsConfiguredNoKeyProvider(t *testing.T) {
	isolateDesktopUserDirs(t)
	t.Setenv(onboardingKeyEnv, "")
	os.Unsetenv(onboardingKeyEnv)
...
default_model = "local/qwen3-coder"
...
`), 0o644); err != nil {
	...
	if NewApp().NeedsOnboarding() {
		t.Fatal("NeedsOnboarding should be false when a no-key local provider is configured")
	}
}
...
func TestTabMetaSessionDisplayTitleFromTitlesSidecar(t *testing.T) {
	isolateDesktopUserDirs(t)

	sessionDir := t.TempDir()
	sessionFile := filepath.Join(sessionDir, "session-001.jsonl")
	if err := os.WriteFile(sessionFile, nil, 0o644); err != nil {
		t.Fatalf("write session: %v", err)
	}

	// Write .titles.json sidecar with a custom title for this session.
	titlesPath := filepath.Join(sessionDir, ".titles.json")
	titles := map[string]string{filepath.Base(sessionFile): "自定义会话标题"}
	data, err := json.Marshal(titles)
	if err != nil {
		t.Fatalf("marshal titles: %v", err)
	}
	if err := os.WriteFile(titlesPath, data, 0o644); err != nil {
		t.Fatalf("write titles: %v", err)
	}
	...
	meta := app.tabMeta(app.tabs["tab-1"], true)
	if meta.SessionDisplayTitle != "自定义会话标题" {
		t.Fatalf("SessionDisplayTitle = %q, want %q", meta.SessionDisplayTitle, "自定义会话标题")
	}
}

func TestTabMetaSessionDisplayTitleFallsBackToTopicTitle(t *testing.T) {
	isolateDesktopUserDirs(t)

	sessionDir := t.TempDir()
	sessionFile := filepath.Join(sessionDir, "session-002.jsonl")
	if err := os.WriteFile(sessionFile, nil, 0o644); err != nil {
		t.Fatalf("write session: %v", err)
	}
	// No .titles.json written — should fall back to TopicTitle.
	...
	meta := app.tabMeta(app.tabs["tab-1"], true)
	if meta.SessionDisplayTitle != "回退话题标题" {
		t.Fatalf("SessionDisplayTitle = %q, want TopicTitle %q", meta.SessionDisplayTitle, "回退话题标题")
	}
}
...
`), 0o644); err != nil {
	...
	if err := userCfg.SetDesktopComposerSubmitKey("ctrl_enter"); err != nil {
		t.Fatalf("set desktop composer submit key: %v", err)
	}
	...
	got := NewApp().Settings()
	if got.DesktopLanguage != "en" || got.DesktopLayoutStyle != "classic" || got.DesktopTheme != "dark" || got.DesktopThemeStyle != "graphite" || got.CloseBehavior != "background" || got.ComposerSubmitKey != "ctrl_enter" || got.StatusBarStyle != "text" {
		t.Fatalf("desktop settings = lang:%q layout:%q theme:%q style:%q close:%q submit:%q status:%q, want user-level desktop prefs", got.DesktopLanguage, got.DesktopLayoutStyle, got.DesktopTheme, got.DesktopThemeStyle, got.CloseBehavior, got.ComposerSubmitKey, got.StatusBarStyle)
	}
...
func TestDesktopStartupSettingsUsesUserDesktopPreferencesWithoutFullSettingsPayload(t *testing.T) {
	...
	if got.DesktopLanguage != "en" || got.DesktopLayoutStyle != "classic" || got.DesktopTheme != "dark" || got.DesktopThemeStyle != "graphite" || got.DisplayMode != "standard" || got.ComposerSubmitKey != "ctrl_enter" || got.StatusBarStyle != "icon" || got.CheckUpdates {
...
func BenchmarkDesktopSettingsPayloads(b *testing.B) {
	...
	b.Setenv("HOME", home)
	b.Setenv("WorkGround2_CREDENTIALS_STORE", "file")
	b.Setenv("USERPROFILE", home)
	b.Setenv("XDG_CONFIG_HOME", xdg)
	b.Setenv("WorkGround2_STATE_HOME", filepath.Join(home, "state"))
	b.Setenv("WorkGround2_CACHE_HOME", filepath.Join(home, "cache"))
...
func TestSettingsShowsGlobalCredentialWithoutMutatingWorkspaceEnv(t *testing.T) {
	...
	for _, p := range got.Providers {
		...
		if !p.KeySet || !strings.Contains(p.KeySource, "WorkGround2 credentials") {
			t.Fatalf("settings-provider key = set:%v source:%q, want WorkGround2 credentials: %+v", p.KeySet, p.KeySource, p)
		}
...
theme_style = "glacier"
close_behavior = "quit"
composer_submit_key = "ctrl_enter"
...
`), 0o644); err != nil {
	...
	if got.DefaultModel != "legacy-provider/legacy-model" || got.DesktopLanguage != "zh" || got.DesktopLayoutStyle != "workbench" || got.DesktopTheme != "light" || got.DesktopThemeStyle != "glacier" || got.CloseBehavior != "quit" || got.ComposerSubmitKey != "ctrl_enter" || got.StatusBarStyle != "text" {
...
`), 0o644); err != nil {
	...
	if _, err := NewApp().AddOfficialProviderAccess("deepseek", "test-key", "deepseek"); err != nil {
...
func TestAddOfficialProviderAccessRejectsBackgroundJobsBeforeSavingKey(t *testing.T) {
	...
	_, err := app.AddOfficialProviderAccess("deepseek", "sk-test", "deepseek")
...
`), 0o644); err != nil {
	...
	if _, err := NewApp().AddOfficialProviderAccess("deepseek", "", "deepseek"); err != nil {
		t.Fatalf("AddOfficialProviderAccess: %v", err)
	}
	cfg := config.LoadForEdit(config.UserConfigPath())
	p, ok := cfg.Provider("deepseek")
...
api_key_env = "DEEPSEEK_API_KEY"
`
	if err := os.WriteFile(filepath.Join(projectRoot, "WorkGround2.toml"), []byte(projectConfig), 0o644); err != nil {
		t.Fatalf("write project config: %v", err)
	}
...
func TestSetModelForTabAlwaysQueuesWhenIdle(t *testing.T) {
	isolateDesktopUserDirs(t)
	...
	ctrl := control.New(control.Options{Executor: executor, SessionDir: dir, Label: "old-model"})
	...
	if err := app.SetModelForTab(tab.ID, "new/new-model"); err != nil {
		t.Fatalf("SetModelForTab for idle controller: %v", err)
	}
	if tab.model != "old/old-model" {
		t.Fatalf("SetModelForTab rebuilt synchronously: active=%q, want old/old-model", tab.model)
	}
	if tab.pendingModel != "new/new-model" {
		t.Fatalf("queued pending=%q, want new/new-model", tab.pendingModel)
	}
	if tab.Label != "new-model" {
		t.Fatalf("visible label = %q, want selected pending model", tab.Label)
	}
	// Same controller should still be active — no rebuild happened.
	if tab.Ctrl != ctrl {
		t.Fatal("SetModelForTab replaced the controller synchronously on an idle tab")
	}
}

func TestSetModelForTabQueuesDuringActiveTurnAndAppliesWhenIdle(t *testing.T) {
	isolateDesktopUserDirs(t)
	setDesktopTestCredential(t, "OLD_MODEL_KEY", "sk-test")
	setDesktopTestCredential(t, "NEW_MODEL_KEY", "sk-test")
	...
	oldCtrl := control.New(control.Options{Runner: runner, SessionDir: config.SessionDir(), Label: "old-model"})
	app := NewApp()
	app.ctx = context.Background()
	tab := &WorkspaceTab{
		ID:          "tab_a",
	...
	<-runner.started
	if err := app.SetModelForTab(tab.ID, "new/new-model"); err != nil {
		t.Fatalf("queue model switch: %v", err)
	}
	if tab.model != "old/old-model" || tab.pendingModel != "new/new-model" {
		t.Fatalf("active=%q pending=%q, want old active and new pending", tab.model, tab.pendingModel)
	}
	if tab.Label != "new-model" {
		t.Fatalf("visible label = %q, want selected pending model", tab.Label)
	}
	...
	for _, model := range models {
		if model.Ref == "new/new-model" && !model.Current {
			t.Fatalf("pending model was not marked current: %+v", models)
		}
	}

	close(runner.release)
	waitNotRunning(t, oldCtrl)
	if err := app.applyPendingModelForTab(tab); err != nil {
		t.Fatalf("apply pending model: %v", err)
	}
	if tab.model != "new/new-model" || tab.pendingModel != "" {
		t.Fatalf("active=%q pending=%q, want new active and no pending", tab.model, tab.pendingModel)
	}
}
...
func TestSetModelForTabRefreshesCarriedSystemPrompt(t *testing.T) {
	...
	if tab.model != "old/old-model" || tab.pendingModel != "new/new-model" {
		t.Fatalf("model selection should queue until the next turn: active=%q pending=%q", tab.model, tab.pendingModel)
	}
	if err := app.applyPendingModelForTab(tab); err != nil {
		t.Fatalf("apply pending model: %v", err)
	}
...
default_effort = "max"
`
	if err := os.WriteFile(filepath.Join(projectA, "WorkGround2.toml"), []byte(ownerConfig), 0o644); err != nil {
		t.Fatal(err)
	}
...
`
	if err := os.WriteFile(filepath.Join(projectB, "WorkGround2.toml"), []byte(staleConfig), 0o644); err != nil {
		t.Fatal(err)
	}
...
func TestDeleteProviderMigratesConfigAndOpenTabs(t *testing.T) {
	isolateDesktopUserDirs(t)
	setDesktopTestCredential(t, "WorkGround2_TEST_KEY", "sk-test")

	cfg := config.Default()
	cfg.DefaultModel = "prov-a/model-a2"
	cfg.Providers = []config.ProviderEntry{
		{Name: "prov-a", Kind: "openai", BaseURL: "https://a.example.com", Model: "model-a1", Models: []string{"model-a1", "model-a2"}, APIKeyEnv: "WorkGround2_TEST_KEY"},
		{Name: "prov-b", Kind: "openai", BaseURL: "https://b.example.com", Model: "model-b1", APIKeyEnv: "WorkGround2_TEST_KEY"},
	}
...
func TestDeleteProviderRejectsRunningAffectedTab(t *testing.T) {
	...
	cfg.Providers = []config.ProviderEntry{
		{Name: "prov-a", Kind: "openai", BaseURL: "https://a.example.com", Model: "model-a1", APIKeyEnv: "WorkGround2_TEST_KEY"},
		{Name: "prov-b", Kind: "openai", BaseURL: "https://b.example.com", Model: "model-b1", APIKeyEnv: "WorkGround2_TEST_KEY"},
	}
	if err := cfg.SaveTo(config.UserConfigPath()); err != nil {
		t.Fatalf("save config: %v", err)
...
func TestFileRefsForTabIgnoreActiveParentWorkspace(t *testing.T) {
	...
	for path, body := range map[string]string{
		filepath.Join(parentRoot, "parent-only.txt"): "parent",
		filepath.Join(parentRoot, "shared.txt"):      "parent shared",
		filepath.Join(childRoot, "child-only.txt"):   "child",
		filepath.Join(childRoot, "shared.txt"):       "child shared",
	} {
	...
	if found := app.SearchFileRefsForTab("child", "child-only"); !hasDirEntry(found, "child-only.txt") {
...
func TestDeleteSessionReleasesLocalLeaseBeforeTrash(t *testing.T) {
	isolateDesktopUserDirs(t)
	...
	path := filepath.Join(dir, "leased-active.jsonl")
	if err := os.WriteFile(path, []byte(`{"role":"user","content":"hello"}`+"\n"), 0o644); err != nil {
		t.Fatalf("write session: %v", err)
	}
	...
	if err := tab.ensureSessionLease(path); err != nil {
		t.Fatalf("ensure active lease: %v", err)
	}
	keepPath := filepath.Join(dir, "leased-keep.jsonl")
	if err := os.WriteFile(keepPath, []byte(`{"role":"user","content":"keep"}`+"\n"), 0o644); err != nil {
		t.Fatalf("write keep session: %v", err)
	}
	keepCtrl := control.New(control.Options{SessionDir: dir, SessionPath: keepPath, Label: "keep"})
	...
	if err := app.DeleteSession(filepath.Base(path)); err != nil {
		t.Fatalf("DeleteSession(active leased basename): %v", err)
	}
	if got := tab.sessionLeaseRuntimeKey(); got != "" {
		t.Fatalf("deleted tab still holds session lease %q", got)
	}
	if _, err := os.Stat(path); !os.IsNotExist(err) {
		t.Fatalf("active session should be moved out of active history, stat err = %v", err)
	}
	trashPath := filepath.Join(dir, sessionTrashDir, "leased-active.jsonl", "leased-active.jsonl")
	if _, err := os.Stat(trashPath); err != nil {
		t.Fatalf("leased active session should be moved to trash: %v", err)
	}
}
...
func TestSubmitDisplayToTabUpdatesNewTopicTitleBeforeTurnCompletes(t *testing.T) {
	isolateDesktopUserDirs(t)
	dir := config.SessionDir()
	if err := os.MkdirAll(dir, 0o755); err != nil {
		t.Fatal(err)
	...
	ctrl := control.New(control.Options{
		...
		Sink:        event.Discard,
		SessionDir:  dir,
		SessionPath: filepath.Join(dir, "immediate-title.jsonl"),
		Label:       "test",
	})
	defer ctrl.Close()
	...
	app.projectTreeChangedHook = func() {}
	if err := app.SubmitDisplayToTab("test", "列表应该立即显示这个标题", "模型收到的完整输入"); err != nil {
		t.Fatalf("SubmitDisplayToTab: %v", err)
	}
	if got := app.tabs["test"].TopicTitle; got != "列表应该立即显示这个标题" {
		t.Fatalf("topic title before turn completion = %q", got)
	}
	select {
	case <-runner.started:
	case <-time.After(2 * time.Second):
...
func TestSubmitToTabHistoryDisplaysRawInputAfterMemoryCompose(t *testing.T) {
	...
	app := NewApp()
	app.setTestCtrl(ctrl, "deepseek/test")
	ctrl.QueueMemory(`Saved memory "WorkGround2-contributions": contribution count updated`)
...
func TestForkCreatesActiveTabWithoutSwitchingSourceController(t *testing.T) {
	...
	workspace := robustTempDir(t)
	if err := os.WriteFile(filepath.Join(workspace, "WorkGround2.toml"), []byte(""), 0o644); err != nil {
...
func TestCapabilitiesShowsDefaultMCPAsAutomaticIdleNotDisabled(t *testing.T) {
	...
	dir := robustTempDir(t)
	t.Chdir(dir)
	if err := os.WriteFile(filepath.Join(dir, "WorkGround2.toml"), []byte(`
[[plugins]]
name = "playwright"
command = "npx"
...
func TestCapabilitiesIncludesInstalledPlugins(t *testing.T) {
	...
	root := filepath.Join(WorkGround2Home, "plugins", "superpowers")
  ...
  "addon": {
    ...
    "storage": { "namespace": "skill-share" }
  }
}`), 0o644); err != nil {
		t.Fatal(err)
	}
	if err := pluginpkg.Upsert(WorkGround2Home, pluginpkg.InstalledPlugin{
		Name:         "superpowers",
	...
	if plugins[0].AddOn == nil || plugins[0].AddOn.Kind != "skill-share" || plugins[0].AddOn.StorageNamespace != "skill-share" {
		t.Fatalf("Capabilities().Plugins[0].AddOn = %+v", plugins[0].AddOn)
	}
}
...
func TestDesktopSharedHostBackgroundMCPAutoConnectsOnBoot(t *testing.T) {
	...
	defer srv.Close()
	if err := os.WriteFile(filepath.Join(dir, "WorkGround2.toml"), []byte(fmt.Sprintf(`
[[plugins]]
name = "h"
type = "http"
url = %q
...
[[plugins]]
name = "broken"
command = "WorkGround2-missing-mcp-binary"
`), 0o644); err != nil {
		t.Fatal(err)
	}
...
func TestUpdateMCPServerEditsProjectMCPJSONEntry(t *testing.T) {
  ...
  "mcpServers": {
	...
	if got.Command != "WorkGround2-missing-mcp-binary" || !reflect.DeepEqual(got.Args, []string{"serve", "--mcp"}) || got.Env["CODEGRAPH_LOG"] != "debug" {
...
`), 0o644); err != nil {
	...
	projectCfg := config.LoadForEdit(filepath.Join(dir, "WorkGround2.toml"))
...
`), 0o644); err != nil {
	...
	if got := cfg.Plugins[0].Command; got != "WorkGround2-missing-mcp-binary" {
	...
	for _, s := range view.Servers {
		if s.Name == "broken" {
			...
			if s.Command != "WorkGround2-missing-mcp-binary" || s.Tier != "background" {

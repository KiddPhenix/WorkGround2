...
func TestSkillRootsViewCountsProjectSkills(t *testing.T) {
	...
	project := t.TempDir()
	root := filepath.Join(project, ".WorkGround2", "skills")
...
func TestSkillRootsViewMarksEnvConfiguredCustomRoot(t *testing.T) {
	...
	t.Setenv("WorkGround2_TEST_SKILL_ROOT", root)
	cfgPath := config.UserConfigPath()
	if err := os.MkdirAll(filepath.Dir(cfgPath), 0o755); err != nil {
		t.Fatal(err)
	}
	if err := os.WriteFile(cfgPath, []byte("[skills]\npaths = [\"${WorkGround2_TEST_SKILL_ROOT}\"]\n"), 0o644); err != nil {
		t.Fatal(err)
	}
	wd, err := os.Getwd()
	if err != nil {
...
func TestSkillRootsViewDedupesConfiguredConventionRoot(t *testing.T) {
	...
	t.Setenv("XDG_CONFIG_HOME", filepath.Join(home, ".config"))
	t.Setenv("AppData", filepath.Join(home, "AppData"))
	t.Setenv("WorkGround2_HOME", filepath.Join(home, ".WorkGround2"))
	project := t.TempDir()
	root := filepath.Join(home, ".WorkGround2", "skills")
	if err := os.MkdirAll(root, 0o755); err != nil {
		t.Fatal(err)
	}
...
func TestSkillRootsViewDedupesConfiguredProjectConventionRoot(t *testing.T) {
	...
	root = filepath.Join(cwd, ".WorkGround2", "skills")
	cfgPath := config.UserConfigPath()
	if err := os.MkdirAll(filepath.Dir(cfgPath), 0o755); err != nil {
		t.Fatal(err)
	}
	// Use forward slashes to avoid TOML \U escape in Windows paths

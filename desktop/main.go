// Command WorkGround2-desktop is the Wails shell around the WorkGround2 kernel: a native
...
// a nested module (workground2/desktop) so the CGO/WebKit desktop build never touches
...
import (
	...
	// cmd/WorkGround2 does — boot.Build resolves providers/tools from these registries.
...
// mirroring cmd/workground2/main.go. The auto-updater reads it (App.Version) to compare
...
const (
	disableWebview2GPUEnv  = "WorkGround2_DESKTOP_DISABLE_WEBVIEW2_GPU"
...
func main() {
	if desktopShouldRunEmbeddedCLI(os.Args[1:]) {
		os.Exit(desktopRunEmbeddedCLI(os.Args[1:], version))
	}
...
func desktopShouldRunEmbeddedCLI(args []string) bool {
	if len(args) == 0 {
		return false
	}
	switch strings.ToLower(strings.TrimSpace(args[0])) {
	case "desktop", "help", "--help", "-h", "version", "--version", "-v":
		return true
	default:
		return false
	}

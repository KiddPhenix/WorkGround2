...
func (a *App) TestBotConnection(id, target string) (BotConnectionDiagnostic, error) {
	...
	case "feishu":
		...
		result, err = feishu.SendText(ctx, feishuCfg, target, "WorkGround2 bot 测试消息：连接和发送链路可用。")
	case "weixin":
		...
		result, err = weixin.SendText(ctx, weixinCfg, target, "WorkGround2 bot 测试消息：连接和发送链路可用。")
	}
	if err != nil {
...
func botAccessViewFromConfig(access config.BotAccessConfig) BotAccessView {
	return BotAccessView{
		...
		Approvers:      append([]string(nil), access.Approvers...),
		Admins:         append([]string(nil), access.Admins...),
	}
}

func botAccessConfigFromView(view BotAccessView) config.BotAccessConfig {
	return config.BotAccessConfig{
		Enabled:        view.Enabled,
		AllowAll:       view.AllowAll,
		PairingEnabled: view.PairingEnabled,
		Users:          trimList(view.Users),
		Groups:         trimList(view.Groups),
		Approvers:      trimList(view.Approvers),
		Admins:         trimList(view.Admins),
	}
}

...
func desktopIconItemRevision(item DesktopIconItem) string {
	parts := []string{item.ID, item.Title, item.Status, strconv.Itoa(item.Position.Order), item.Position.Row, item.Position.Zone, strconv.FormatUint(item.ConversationSequence, 10)}
	if item.Runtime != nil {

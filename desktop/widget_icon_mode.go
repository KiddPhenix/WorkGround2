...
func (a *App) advanceDesktopIconDelegation(receipt *desktopIconReceipt) error {
	...
	receipt.TabID = tabID
	if receipt.TargetKind == "background" {
...
func (a *App) desktopIconSnapshotLocked() DesktopIconSnapshot {
	...
	snapshot := buildDesktopIconSnapshot(sources, unreadState, spaces, a.iconWidgetState, hover, roomSummaries, roomRefs, subagentCounts, delegations)
...
func buildDesktopIconSnapshot(sources []widgetSource, unreadState UnreadState, spaces []WidgetWorkspaceOption, persisted desktopIconPersistedState, hover int, roomSummaries map[string]string, roomRefs map[string]*DesktopIconTaskRef, subagentCounts map[widgetSubagentKey]int, delegations []DesktopIconDelegation) DesktopIconSnapshot {
	...
	for _, source := range sources {
		if !source.meta.BackgroundOnly && !widgetSourceIsSubagent(source) {
			continue
		}

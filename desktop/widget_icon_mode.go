...
// even after its tab was closed or its SourceID went stale. An existing exact
// tab is reused; a same-Topic tab on another Session is left intact while a
// new writable tab initializes asynchronously. Missing identity is an explicit
// failure. Callers hold iconWidgetMu.
func (a *App) resolveDesktopIconTaskTab(item DesktopIconItem) (string, error) {
	...
	meta, err := a.openTaskSession(ref.Scope, ref.WorkspaceRoot, ref.TopicID, sessionPath)
	if err != nil {
...
func (a *App) GetDesktopIconSnapshot() DesktopIconSnapshot {
	// Session discovery has a bounded five-second cold-cache wait. Perform it
	// before taking iconWidgetMu so task-open actions never queue behind a
	// background snapshot poll that is waiting on filesystem metadata.
	projectTree := a.listDesktopIconProjectTree()
	a.iconWidgetMu.Lock()
	defer a.iconWidgetMu.Unlock()
	a.loadDesktopIconStateLocked()
...
func (a *App) listDesktopIconProjectTree() []ProjectNode {
	if a.desktopIconProjectTree != nil {
		return a.desktopIconProjectTree()
	}
	return a.ListProjectTree()
}
...
func (a *App) desktopIconSnapshotLocked() DesktopIconSnapshot {
	return a.desktopIconSnapshotWithProjectTreeLocked(a.listDesktopIconProjectTree())
}

func (a *App) desktopIconSnapshotWithProjectTreeLocked(projectTree []ProjectNode) DesktopIconSnapshot {
...
func buildDesktopIconSnapshotWithPresentations(sources []widgetSource, unreadState UnreadState, spaces []WidgetWorkspaceOption, persisted desktopIconPersistedState, hover int, roomPresentations desktopRoomNoticePresentations, roomRefs map[string]*DesktopIconTaskRef, subagentCounts map[widgetSubagentKey]int, sessionPresentations map[string]desktopIconSessionPresentation, pinnedRooms []desktopIconRoomDescriptor, delegations []DesktopIconDelegation, roomDescriptorSets ...map[string]desktopIconRoomDescriptor) DesktopIconSnapshot {
	...
	for _, id := range keptIDs {
		...
		items = append(items, desktopIconKeptItem(kept, persisted))
		taskCount++
	}
	...
	for i := range items {
		item := &items[i]
		finalizeDesktopIconItem(item, persisted)
...
func desktopIconKeptItem(kept desktopIconKept, persisted desktopIconPersistedState) DesktopIconItem {
	return DesktopIconItem{
		...
		Revision: kept.Revision, Retained: true, Notifications: []DesktopIconNotice{desktopIconNoticeForKept(kept, persisted.CompletionSummaries)},
...
func finalizeDesktopIconItem(item *DesktopIconItem, persisted desktopIconPersistedState) {
...
func desktopIconSnapshotItem(snapshot *DesktopIconSnapshot, itemID string) *DesktopIconItem {
	for i := range snapshot.Items {
		if snapshot.Items[i].ID == itemID {
...
// desktopIconActionTargetLocked keeps task-open latency independent from the
// Session list. The icon UI can only click an item from a snapshot already
// returned by this App, so the cached projection is the correct optimistic
// concurrency token for its item ID and revision. Missing cache falls back to
...
func (a *App) desktopIconActionTargetLocked(input DesktopIconActionInput) (DesktopIconSnapshot, *DesktopIconItem) {
	if input.Action == "open" && strings.HasPrefix(input.ItemID, "task:") && a.iconWidgetSnapshotReady {
		...
		if cached.Retained {
			kept, ok := a.iconWidgetState.Kept[cached.ID]
			if !ok {
				return snapshot, nil
			}
			// Completion summaries, renames, appearance and positions may change
			// after the UI's snapshot. Rebuild only this persisted item so the
			// existing revision guard remains authoritative without touching the
			// project/Session tree.
			current := desktopIconKeptItem(kept, a.iconWidgetState)
			finalizeDesktopIconItem(&current, a.iconWidgetState)
			snapshot.Items = append([]DesktopIconItem(nil), snapshot.Items...)
			item := desktopIconSnapshotItem(&snapshot, input.ItemID)
			*item = current
...
func (a *App) ApplyDesktopIconAction(input DesktopIconActionInput) DesktopIconActionResult {
	...
	snapshot, item := a.desktopIconActionTargetLocked(input)
	if item == nil {
	...
	if input.Action == "open" {
		// The frontend leaves icon mode immediately after an accepted open. The
		// authoritative pre-action snapshot already passed identity/revision

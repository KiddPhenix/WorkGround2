...
export function formatPerformanceContext(snapshot: PerformanceSnapshot): string {
  ...
  if (snapshot.breadcrumbs?.length) {
    const now = Date.now();
    const recent = snapshot.breadcrumbs
      .slice(-12)
      .map((b) => `-${((now - b.t) / 1000).toFixed(1)}s [${b.cat}] ${b.msg}`)
      .join("\n");
    if (recent) lines.push(`recent breadcrumbs:\n${recent}`);
  }
  return lines.join("\n");
}
...
export function buildPerformancePayload(snapshot: PerformanceSnapshot): CrashPayload {
  ...
  const crumbs = snapshot.breadcrumbs?.length ? "" : dumpBreadcrumbs();
...
function paint(payload: CrashPayload) {
  ...
  actions.append(copy);
  host.replaceChildren(title, body, actions);
}

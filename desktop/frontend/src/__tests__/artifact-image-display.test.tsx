...
// ── TurnCollapse: request_help uses the same generic artifact path ──
{
  setupDOM();
  clearArtifacts();
  seedArtifact({ artifactId: "img-help", name: "generated.png", type: "image", status: "available", sessionId: "test-tab", path: "/tmp/generated.png", sourceRunId: "tc-help" });

  const helpTool: Item = {
    kind: "tool",
    id: "tc-help",
    name: "request_help",
    args: '{"capability":"image_generation"}',
    readOnly: false,
    status: "done",
    assistStatus: {
      phase: "completed",
  ...
  const bodyEl = document.querySelector(".turn-collapse__body");
  const imagesEl = document.querySelector(".artifact-images");
  ok(bodyEl !== null && bodyEl.querySelector(".request-help") !== null, "request_help status stays inside collapse body");
  ok(imagesEl !== null && imagesEl.parentElement === collapseEl, "request_help image uses generic artifact preview outside body");
  ok(bodyEl !== null && imagesEl !== null && !bodyEl.contains(imagesEl), "request_help image is visible while fold is closed");

...

{
  ...
  eq(waiting.runtimeMode, "waiting_user", "approval prompt marks runtimeMode waiting_user");
  ...
  eq(canceling.runtimeMode, "cancelling", "cancel_requested marks runtimeMode cancelling");
  ...
  eq(backgroundOnly.runtimeMode, "background_only", "background jobs alone mark runtimeMode background_only");
  ...
  const explicitBackgroundOnly = reducer(initialState, { type: "backend_status", running: true, runtimeMode: "background_only", foregroundActive: false, backgroundJobs: 1, cancellable: true });
  eq(explicitBackgroundOnly.running, false, "explicit background_only mode overrides legacy running/cancellable flags");
  eq(explicitBackgroundOnly.runtimeMode, "background_only", "explicit background_only mode is stored");
  ...
  eq(foregroundRunningFromRuntimeMeta({ running: true, runtimeMode: "background_only", foregroundActive: false, cancellable: true }), false, "runtimeMode/foregroundActive override legacy running metadata");
  ...
  const runningWithoutStart = reducer(
    { ...initialState, running: true, turnActive: true, turnStartAt: 0 },
    { type: "backend_status", running: true, cancellable: true },
  );
  eq(runningWithoutStart.running, true, "backend_status keeps a foreground turn running");
  eq(runningWithoutStart.turnStartAt > 0, true, "backend_status repairs missing run-status start time");

  const backendStartedAt = Date.now() - 30_000;
  const runningWithBackendStart = reducer(
    { ...initialState, running: true, turnActive: true, turnStartAt: Date.now() },
    { type: "backend_status", running: true, cancellable: true, turnStartAt: backendStartedAt },
  );
  eq(runningWithBackendStart.turnStartAt, backendStartedAt, "backend_status uses backend turn start time");
}

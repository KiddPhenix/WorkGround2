...

{
  ...
  const withPath = composerDraftKeyForTab({
    ...
    workspaceRoot: "/repo",
    topicId: "topic-a",
    sessionPath: "/repo/.WorkGround2/sessions/topic-a.jsonl",
  }, "tab-a");
...

{
  ...
  await rerender({ insertRequest: { id: 10, text: "send only with ctrl", mode: "replace" } });

  await act(async () => {
    textarea().dispatchEvent(new KeyboardEvent("keydown", { key: "Enter", bubbles: true, cancelable: true }));
    await flushTimers();
  });
  eq(sent.length, 0, "Ctrl+Enter mode does not submit on plain Enter");

  await act(async () => {
    textarea().dispatchEvent(new KeyboardEvent("keydown", { key: "Enter", ctrlKey: true, bubbles: true, cancelable: true }));
    await flushTimers();
  });
  eq(sent.length, 1, "Ctrl+Enter mode submits on Ctrl+Enter");
  eq(sent[0], "send only with ctrl", "Ctrl+Enter submits the current draft");
...

{
  ...
  await rerender({ sessionKey: "session:project:/repo:topic-b:session-b" });
  await act(async () => {
    savePastedFile.resolve("/tmp/WorkGround2/draft.txt");
    await flushTimers();
  });

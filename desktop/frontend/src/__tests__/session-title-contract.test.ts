...
eq(
  tabSessionDisplayTitle({
    ...
    workspaceRoot: "/work/WorkGround2",
    topicTitle: "新的会话",
    sessionDisplayTitle: "每一个 session 刚建立的时候",
  }),
  "每一个 session 刚建立的时候",
  "page header prefers the selected session title without a workspace prefix",
);
...
eq(
  tabSessionDisplayTitle({
    ...
    workspaceName: "WorkGround2",
    workspaceRoot: "/work/WorkGround2",
    topicTitle: "codex-run-complete-step-tabs",
  }),
  "codex-run-complete-step-tabs",
  "page header falls back to the topic title without a workspace prefix",
);

...
eq(finalDeclaration(".transcript", "overflow-anchor"), "none", "transcript scroll anchoring is owned by the app");
eq(finalDeclaration(".transcript__content", "min-height"), "100%", "transcript content spans the viewport");
eq(finalDeclaration(".transcript__content", "display"), "flex", "transcript content uses flex bottom anchoring");
eq(finalDeclaration(".transcript__content", "flex-direction"), "column", "transcript content keeps chronological DOM order");
eq(finalDeclaration(".transcript__content", "justify-content"), "flex-end", "transcript content starts at the bottom when history is shorter than the viewport");
eq(finalDeclaration(".transcript__content > *", "width"), "100%", "transcript children keep full-row message alignment inside the flex wrapper");
eq(finalDeclaration(".transcript__content > *", "max-width"), "var(--maxw)", "transcript children keep the original readable width cap");

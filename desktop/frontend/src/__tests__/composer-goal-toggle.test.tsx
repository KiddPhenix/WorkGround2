...

{
  const dom = installDom();
  mockApp({
    SavePastedFile: async () => ".WorkGround2/attachments/notes.txt",
  });
...

{
  ...
  mockApp({
    AttachDropped: async () => ({
      kind: "attachment",
      path: ".WorkGround2/attachments/report.pdf",
    }),
  });
  const { root } = await renderComposer();
  if (!droppedCallback) throw new Error("native file drop handler did not register");
...

{
  ...
  mockApp({
    AttachDropped: async () => ({
      kind: "workspace",
      path: "__WorkGround2_external_folder/mock/Folder-With-Spaces",
      isDir: true,
  ...
  eq(calls.submit.join(","), "inspect @__WorkGround2_external_folder/mock/Folder-With-Spaces/", "external folder submit text uses the session ref token");
...
{
  const externalToken = "__WorkGround2_external_folder/mock/Folder-With-Spaces/src/outside.txt";
...

{
  ...
  mockApp({
    ListDirForTab: async (tabId) => {
      ...
      return [{ name: `${tabId}.txt`, isDir: false }];
    },
    SearchFileRefsForTab: async () => [],
  });
  const { root, rerender } = await renderComposer({ tabId: "tab-a", cwd: "/same-root" });

  await rerender({ insertRequest: { id: 8001, text: "@", mode: "replace" } });
  await waitFor("tab-a file request", () => fileScopeCalls.includes("tab-a"));
  await rerender({ tabId: "tab-b", cwd: "/same-root" });

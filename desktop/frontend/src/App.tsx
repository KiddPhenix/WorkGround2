...
type WindowsResizeEdge = "n-resize" | "ne-resize" | "e-resize" | "se-resize" | "s-resize" | "sw-resize" | "w-resize" | "nw-resize";

const WINDOWS_RESIZE_EDGES: Array<{ edge: WindowsResizeEdge; className: string }> = [
  { edge: "n-resize", className: "windows-resize-handle--top" },
  { edge: "ne-resize", className: "windows-resize-handle--top-right" },
  { edge: "e-resize", className: "windows-resize-handle--right" },
  { edge: "se-resize", className: "windows-resize-handle--bottom-right" },
  { edge: "s-resize", className: "windows-resize-handle--bottom" },
  { edge: "sw-resize", className: "windows-resize-handle--bottom-left" },
  { edge: "w-resize", className: "windows-resize-handle--left" },
  { edge: "nw-resize", className: "windows-resize-handle--top-left" },
];
...
function normalizeComposerSubmitKey(value: string | undefined): ComposerSubmitKey {
  return value === "ctrl_enter" ? "ctrl_enter" : "enter";
}

function useWindowsMaximisedState() {
  ...
  return { maximised, syncMaximised };
}
...
type WorkBootstrap = {
  ...
  prompt: string;
  submitPrompt: string;
  phase: "initializing" | "planning" | "revealing" | "error";
  error?: string;
...
function WindowsResizeHandles() {
  ...
  const startResize = useCallback((edge: WindowsResizeEdge, event: ReactMouseEvent<HTMLSpanElement>) => {
    ...
    if (!wailsInvoke) return;
    event.preventDefault();
    event.stopPropagation();
    wailsInvoke(`resize:${edge}`);
  }, []);
...
function sessionItemsToMarkdown(title: string, items: Item[], live?: LiveStream): string {
  const lines: string[] = [`# ${title.trim() || "WorkGround2 session"}`, ""];
...
function safeFilename(name: string): string {
  ...
  return cleaned || "WorkGround2-session";
}
...
function MainApp({ widgetEnabled, widgetActive, onEnterWidgetMode }: { widgetEnabled: boolean; widgetActive: boolean; onEnterWidgetMode: () => void | Promise<void> }) {
  ...
  const { pluginName: dialogPluginName, panelID: dialogPanelID, setOpen: setDialogOpen } = useAddOnDialogStore();
  ...
  // Poll for pending addon dialogs
  useEffect(() => {
    let active = true;
    ...
    const poll = async () => {
      if (!active) return;
      try {
        const pending = await app.GetPendingAddOnDialog();
        if (active && pending) {
          setDialogOpen({ pluginName: pending.pluginName as string, panelID: pending.panelID as string, message: (pending.message as string) || "" });
        }
      } catch (_) { /* ignore */ }
  ...
  const [workSendByTab, setWorkSendByTab] = useState<Record<string, boolean>>({});
  ...
  useEffect(() => () => {
    if (workRevealTimerRef.current !== null) window.clearTimeout(workRevealTimerRef.current);
  }, []);
  ...
  const handlePinMemory = useCallback(async (role: "user" | "assistant", content: string, turn: number) => {
    const result = await pinMemory(role, content, turn, activeTabId);
    if (!result) return;
    setDockRefreshKey((value) => value + 1);
  }, [activeTabId, pinMemory]);
  ...
  const applyDesktopPreferences = useCallback(
    (settings: Pick<SettingsView, "desktopTheme" | "desktopThemeStyle" | "desktopLayoutStyle" | "desktopLanguage" | "checkUpdates" | "composerSubmitKey" | "statusBarStyle" | "statusBarItems">) => {
      ...
      setComposerSubmitKey(normalizeComposerSubmitKey(settings.composerSubmitKey));
  ...
  const visibleTabs = useMemo(() => {
    ...
    return [...ordered, ...missing].map((tab) => {
      ...
      const foregroundActive = visible ? Boolean(tab.foregroundActive || state.running) : Boolean(tab.foregroundActive);
      return {
        ...tab,
        running: visible ? tab.running || state.running : tab.running,
        foregroundActive,
        runtimeMode: visible && state.running ? state.runtimeMode : tab.runtimeMode,
  ...
  }, [composerProfilesByTab, state.running, state.runtimeMode, tabMetas, tabOrderIds, visibleTabId]);
  ...
  // Model selection is queued by the backend and applied before the next turn.
  // The rebuild already restores mode, approval policy, and goal from tab state.
  ...
  // Workspace sidebar tree scrollbar auto-hide: show thin thumb while scrolling,
  // hide 700ms after the last scroll event. Uses ref + DOM class to avoid
  // per-frame React re-renders.
  useEffect(() => {
    const el = workspaceTreeRef.current;
    if (!el) return;
    let timer: number | null = null;
    const onScroll = () => {
      el.classList.add("workspace-sidebar__tree--scrolling");
      if (timer !== null) window.clearTimeout(timer);
      timer = window.setTimeout(() => {
        el.classList.remove("workspace-sidebar__tree--scrolling");
        timer = null;
      }, 700);
    };
  ...
  const runNavigationRequest = useCallback(async (request: PendingDesktopNavigationRequest) => {
    ...
    const openTopicTarget = async (scope: string, workspaceRoot: string, topicId: string, sessionPath?: string, runtimeHint?: ProjectTopicRuntimeHint): Promise<TabMeta> => {
      if (singleSurfaceLayout) return activateTopic(scope, workspaceRoot, topicId, sessionPath || "", runtimeHint);
      if (sessionPath) return openTopicSession(scope, workspaceRoot, topicId, sessionPath, runtimeHint);
  ...
  const beginWorkInitialization = useCallback((bootstrap: WorkBootstrap): Promise<WorkSessionResult | null> => {
    ...
    const promise = (async (): Promise<WorkSessionResult | null> => {
      try {
        ...
        if (result.tabMeta?.id && result.tabMeta.id !== activeTabId) {
          await enqueueTabSwitch(result.tabMeta.id, { ...result.tabMeta, active: true });
          setTabRevealSignal((signal) => signal + 1);
        }
        await refreshProjectsAndTabs();
        const workID = result.tabMeta?.workId
          || (result.workView as { work?: { id?: string } } | undefined)?.work?.id;
        if (!result.error && workID) {
          const ui = useWorkUIStore.getState();
          ui.ensureCard(workID);
          ui.setDraft(workID, "back", bootstrap.submitPrompt);
          ui.setActiveFace(workID, "back");
          setWorkStartIntent({
            id: `${bootstrap.requestId}:start`,
            workID,
            prompt: bootstrap.submitPrompt,
          });
        }
        patchWorkBootstrap(bootstrap.requestId, {
          tabID: result.tabMeta?.id || bootstrap.tabID,
          result,
          phase: result.error || !workID ? "error" : "planning",
          error: result.error || (!workID ? "Work 初始化完成，但没有返回工作标识。" : undefined),
        });
        return result;
      } catch (error) {
        patchWorkBootstrap(bootstrap.requestId, {
  ...
  const revealWorkPanel = useCallback((intentID: string) => {
    ...
    if (!current || `${current.requestId}:start` !== intentID) return;
    patchWorkBootstrap(current.requestId, { phase: "revealing", error: undefined });
    writeWorkDraft(current.requestId, "");
    workCreateRequestsRef.current.delete(current.tabID);
    if (workRevealTimerRef.current !== null) window.clearTimeout(workRevealTimerRef.current);
    workRevealTimerRef.current = window.setTimeout(() => {
      workRevealTimerRef.current = null;
      const latest = workBootstrapRef.current;
      if (!latest || `${latest.requestId}:start` !== intentID) return;
  ...
  useEffect(() => {
    ...
    const bootstrap: WorkBootstrap = {
      ...
      submitPrompt: readWorkDraft(activeTab.workRequestId),
      phase: "initializing",
    };
    workBootstrapRef.current = bootstrap;
    setWorkBootstrap(bootstrap);
  ...
  const handleRetryActiveWork = useCallback(() => {
    if (activeTab?.sessionKind === "work" && !activeTab.workId && activeTab.workRequestId) {
      ...
      workCreateRequestsRef.current.set(activeTab.id, activeTab.workRequestId);
      const prompt = readWorkDraft(activeTab.workRequestId);
      const bootstrap: WorkBootstrap = {
        requestId: activeTab.workRequestId,
        tabID: activeTab.id,
        scope,
  ...
  const contextPercent = state.context.window > 0
  ...
  const sessionSurfaceProps: SessionSurfaceProps = {
    ...
    onSend: handleSend,
    onSendAsWork: handleSendAsWork,
    onWorkSendChange: handleWorkSendChange,
    onCancel: cancel,
  ...
  useEffect(() => {
    ...
    if (params.get("uiFixture") !== "iris") return;
    // Dynamic import so fixture code is tree-shakeable in production builds
    import("./lib/irisFixture").then(async (fixture) => {
      const {
        irisFixtureMemory,
      ...
      const { useMemoryStore } = await import("./store/memory");
      const { useArtifactStore } = await import("./store/artifacts");
      const { useRunStore } = await import("./store/run");
      const { useAddOnSurfaceStore } = await import("./store/addonSurface");
      const { useComposerQueueStore: useQueueStore } = await import("./store/composerQueue");
      ...
      useQueueStore.getState().addItem(irisFixtureQueueItems()[0]);
      useQueueStore.getState().addItem(irisFixtureQueueItems()[1]);
      setInfoSessionId(FIXTURE_SESSION_ID);
      setIrisFixtureActive(true);
  ...
  const topicbarCanRename = !sidebarImDetailConnection
    && activeTab?.sessionKind !== "work"
    && Boolean(activeTab?.topicId);
  ...
  return (
    ...
    >
      ...
      >
        {sidebarWorkbench ? (
          <>
            ...
            {showCollaborationSurface && activeTab?.sessionId ? (
              ...
              >
                {activeWorkBootstrap && (
                  <div className="work-session-host__session" key="session-transition">
                    <SessionSurface
                      {...sessionSurfaceProps}
                      composerDisabled
                    ...
                    <WorkSessionTransition
                      prompt={activeWorkBootstrap.prompt}
                      phase={activeWorkBootstrap.phase}
                      error={activeWorkBootstrap.error}
                      onRetry={() => { void beginWorkInitialization(activeWorkBootstrap); }}
                    />
                  </div>
                )}
                <div className="work-session-host__work" key="work-panel">
                  <WorkCard
                    workID={activeTab.workId}
                    ...
                    onStartIntentConsumed={revealWorkPanel}
                    onStartIntentNeedsAttention={revealWorkPanel}
                    tabID={activeTab.id}
              ...
              <div className="work-session-host work-session-host--transition" data-phase={activeWorkBootstrap.phase}>
                <div className="work-session-host__session">
                  <SessionSurface
                    {...sessionSurfaceProps}
                    composerDisabled
                    workSendAvailable={false}
          ...
          {sidebarWorkbench ? (
            <>
              <div className="sidebar__head">
                <div className="sidebar__brand sidebar__brand--workbench">
                  <img src={logoWordmark} alt="WorkGround2" className="sidebar__brand-logo sidebar__brand-logo--workbench" draggable={false} />
                </div>
                <Tooltip label={sidebarToggleTitle} className="sidebar__header-toggle-wrap" side="right">
                  <button
                    className={[
                      "sidebar__header-toggle",
                      sidebarTogglePressed ? "sidebar__header-toggle--pressed" : "",
                    ].filter(Boolean).join(" ")}
                    type="button"
                    onClick={toggleSidebar}
                    aria-label={sidebarToggleTitle}
            ...
            <>
              <div className="sidebar__brand" aria-hidden={sidebarCollapsed}>
                <img src={logoWordmark} alt="WorkGround2" className="sidebar__brand-logo" draggable={false} />
              </div>
        ...
        <section className={`chat-pane${sidebarCreation && !sessionHasContent ? " chat-pane--creation-empty" : ""}`}>
          ...
          {state.meta?.startupErr && (
            <div className="banner banner--error banner--startup-error">
              <span>{t("topbar.startupError", { msg: state.meta.startupErr })}</span>
              <button type="button" className="banner__retry" onClick={() => void retryTabStartup().catch((error) => showToast(error instanceof Error ? error.message : String(error), "error"))}>
                {t("common.retry")}
              </button>
            </div>
          )}
          ...
          <main className="main">
            {sidebarImDetailConnection ? (
              ...
              <Transcript
                ...
                onLoadOlderHistory={() => {
                  if (!activeTabId) return;
                  return loadOlderHistory(activeTabId);
                }}
              />
            )}
          </main>
      ...
      {dialogPluginName && dialogPanelID && (
        <Suspense fallback={null}>
          <AddOnDialogModal />
        </Suspense>
      )}

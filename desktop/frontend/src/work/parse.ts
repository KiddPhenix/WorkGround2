...
export function parseWorkPatchPreview(raw:unknown):WorkPatchPreview{const r=fields(raw);nonEmpty(r.id);nonEmpty(r.workId);nonEmpty(r.runId);nonEmpty(r.taskId);nonEmpty(r.blockId);nonEmpty(r.sessionId);safeInt(r.baseDefinitionRev);safeInt(r.baseBlockRev);checkEnum(r.scope,patchScopes,'PatchScope');const operations=arrOrEmpty(r.operations);for(const o of operations)parsePO(o);const affectedNodeIds=arrOrEmpty(r.affectedNodeIds);const affectedBlockIds=arrOrEmpty(r.affectedBlockIds);const affectedArtifactSlotIds=arrOrEmpty(r.affectedArtifactSlotIds);const staleArtifactSlotIds=arrOrEmpty(r.staleArtifactSlotIds);const invalidatedTaskIds=arrOrEmpty(r.invalidatedTaskIds);strArr(affectedNodeIds);strArr(affectedBlockIds);strArr(affectedArtifactSlotIds);strArr(staleArtifactSlotIds);strArr(invalidatedTaskIds);checkBool(r.requiresRerun);nonEmpty(r.digest);checkDate(r.expiresAt);return{...r,operations,affectedNodeIds,affectedBlockIds,affectedArtifactSlotIds,staleArtifactSlotIds,invalidatedTaskIds}as unknown as WorkPatchPreview}

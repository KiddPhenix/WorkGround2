...
export function parseWorkPatchPreview(raw:unknown):WorkPatchPreview{const r=fields(raw);nonEmpty(r.id);nonEmpty(r.workId);nonEmpty(r.runId);nonEmpty(r.taskId);nonEmpty(r.blockId);nonEmpty(r.sessionId);safeInt(r.baseDefinitionRev);safeInt(r.baseBlockRev);checkEnum(r.scope,patchScopes,'PatchScope');for(const o of arr(r.operations))parsePO(o);strArr(arr(r.affectedNodeIds));strArr(arr(r.affectedBlockIds));strArr(arr(r.affectedArtifactSlotIds));strArr(arr(r.staleArtifactSlotIds));strArr(arr(r.invalidatedTaskIds));checkBool(r.requiresRerun);nonEmpty(r.digest);checkDate(r.expiresAt);return r as unknown as WorkPatchPreview}

...
function ContextBlock({ message, projectName, taskName }: {
  ...
  const t = useT();
  const resolvedTask = message?.taskNameCode === "current" || message?.taskName === "当前任务"
    ? t("widget.currentTask")
    : message?.taskName ?? taskName ?? t("widget.taskStatus");
  return (
    <section className="widget-context" aria-label={t("widget.contextLabel")}>
      ...
      <div className="widget-context__identity">
        ...
        <span className="widget-context__task">{resolvedTask}</span>
      </div>
    </section>
  );
}

...
import type { DirEntry, FilePreview, GitCommitView, GitCommitDetailView, PinnedMemory, WorkspaceChangesView } from "../lib/types";
...
export function WorkspacePanel({
  ...
  refreshKey?: number;
  initialViewMode?: "files" | "changed" | "pinned";
  ...
  const [viewMode, setViewMode] = useState<"files" | "changed" | "pinned">(initialViewMode);
  ...
  const pinnedMemoriesRequestIdRef = useRef(0);
  ...
  const loadPinnedMemories = useCallback(async () => {
    const requestId = ++pinnedMemoriesRequestIdRef.current;
    const requestTabId = workspaceTabId;
    setLoadingPinned(true);
    try {
      const result = await app.PinnedMemoriesForTab(requestTabId);
      if (pinnedMemoriesRequestIdRef.current === requestId && lastWorkspaceTabIdRef.current === requestTabId) {
        setPinnedMemories(result?.items ?? []);
      }
    } catch {
      if (pinnedMemoriesRequestIdRef.current === requestId && lastWorkspaceTabIdRef.current === requestTabId) {
        setPinnedMemories([]);
  ...
  const togglePin = useCallback(async (id: string, pinned: boolean) => {
    const requestTabId = workspaceTabId;
    try {
      const changed = await app.SetPinnedMemoryPinnedForTab(requestTabId, id, pinned);
      if (changed && lastWorkspaceTabIdRef.current === requestTabId) {
        setPinnedMemories((prev) => prev.map((pm) => (pm.id === id ? { ...pm, pinned } : pm)));
      }
    } catch {
      if (lastWorkspaceTabIdRef.current === requestTabId) {
        void loadPinnedMemories();
      }
    }
  }, [loadPinnedMemories, workspaceTabId]);
  ...
  useEffect(() => {
    ...
    commitDetailRequestIdRef.current += 1;
    pinnedMemoriesRequestIdRef.current += 1;
  ...
  }, [selectedPath, viewMode, loadGitHistory, loadWorkspaceChanges, loadPinnedMemories, open]);
  ...
  }, [loadGitHistory, loadWorkspaceChanges, loadPinnedMemories, loadDir, open, refreshKey, viewMode]);
    ...
    >
      {previewVisible && <section className="workspace-preview">
        ...
        >
          ...
          ) : viewMode === "pinned" && !selectedPath ? (
            <div className="workspace-change-scope">
              <div className="workspace-change-scope__head">
                <span className="workspace-change-scope__title">{t("workspace.pinnedTab")}</span>
                <span className="workspace-change-scope__meta">{t("context.changedMeta", { count: pinnedMemories.filter((pm) => pm.pinned).length })}</span>
                <button type="button" aria-label={t("workspace.refreshChanges")} onClick={() => void loadPinnedMemories()}><RefreshCw size={12} /></button>
              </div>
              <div className="workspace-change-scope__list">
                {loadingPinned ? (
                  <div className="workspace-empty">{t("workspace.loading")}</div>
                ) : pinnedMemories.length === 0 ? (
                  <div className="workspace-empty">{t("workspace.noPinned")}</div>
                ) : (
                  pinnedMemories.map((pm) => (
                    <div key={pm.id} className={`workspace-change${!pm.pinned ? " workspace-change--disabled" : ""}`}>
                      <span className="workspace-change__body">
                        <span className="workspace-change__name">[{pm.role === "user" ? t("workspace.pinYou") : t("workspace.pinAI")}, {t("workspace.pinTurn", { turn: pm.turn })}]</span>
                        <span className="workspace-change__detail">{pm.content.length > 120 ? pm.content.slice(0, 120) + "…" : pm.content}</span>
                      </span>
                      <span className="workspace-change__meta">
                        <button
                          type="button"
                          className="workspace-change__badge workspace-change__badge--button"
                          aria-label={pm.pinned ? t("workspace.unpin") : t("workspace.repin")}
                          onClick={(e) => {
                            e.stopPropagation();
                            void togglePin(pm.id, !pm.pinned);
                          }}
                          title={pm.pinned ? t("workspace.unpin") : t("workspace.repin")}
                        >
                          {pm.pinned ? <PinOff size={12} /> : <Pin size={12} />}
                        </button>
                      </span>
                    </div>
                  ))
          ...
          <div className={`workspace-files__tools${embeddedDockMode ? " workspace-files__tools--embedded" : ""}`}>
            ...
            {showViewTabs && (
              <div className="workspace-files__tabs" role="tablist" aria-label={t("workspace.viewMode")}>
                ...
                <button
                  className={viewMode === "pinned" ? "workspace-files__tab workspace-files__tab--active" : "workspace-files__tab"}
                  onClick={() => {
                    setViewMode("pinned");
                    setSelectedPath(null);
                    setOpenTabs([]);

...
type PinMemoryRole = "user" | "assistant";
...
const IM_SOURCE_START = "[[WorkGround2-im]]";
const IM_SOURCE_END = "[[/WorkGround2-im]]";
...
function selectedTextWithin(root?: HTMLElement | null): string {
  ...
  const containsAnchor = !anchor || root.contains(anchor);
  const containsFocus = !focus || root.contains(focus);
  return containsAnchor && containsFocus ? text : "";
}
...
export function UserMessage({
  ...
  const canPin = turn !== undefined && onPinMemory !== undefined;
  ...
  const pinMessage = () => {
    if (!canPin) return;
    const selected = selectedTextWithin(rootRef.current);
    const content = (selected || displayText || actionText).trim();
    if (!content) return;
    void onPinMemory?.("user", content, turn as number);
  };
  ...
  return (
    ...
    >
      <div className={`msg__body${editing ? " msg__body--editing" : ""}`}>
        ...
        {queued && (
          <div className={`msg__send-queued${queueError ? " msg__send-queued--error" : ""}`}>
            {queueError ? t("msg.startupQueueFailed") : t("msg.startupQueued")}
          </div>
        )}
...
export function TurnActions({
  ...
  const canPin = onPinMemory != null && turn != null && text.trim() !== "";
  ...
  const pinAssistant = () => {
    if (!canPin) return;
    const content = (selectedTextWithin() || text).trim();
    if (!content) return;
    void onPinMemory?.("assistant", content, turn as number);
  };
  return (
    <div className={`turn-actions${openMenu ? " turn-actions--open" : ""}${hoverMenus ? " turn-actions--hover-menu" : ""}`}>
      ...
      {onPinMemory && (
        <button
          ...
          title={t("workspace.pin")}
          onMouseDown={(event) => event.preventDefault()}
          onClick={pinAssistant}
        >
          <Pin size={13} />
          <span>{t("workspace.pin")}</span>
        </button>
      )}
      {canAct && (
        <>

...
type PinMemoryHandler = (role: PinMemoryRole, content: string, turn: number) => void | Promise<void>;
...
export function Transcript({
  ...
  const {
    ...
    lastClientHeight,
    lastFooterHeight,
  } = useScrollManager(scrollHostRef);
  ...
  const questionSnapshot = useMemo(() => ({
    ...
    lastId: questions.length > 0 ? questions[questions.length - 1].id : "",
  }), [questions]);

  // Track question count and auto-scroll on new messages.
  useEffect(() => { trackQuestions(questionSnapshot); }, [questionSnapshot, trackQuestions]);

  const restoreKey = `${tabId ?? ""}|${revealSignal}`;
  if (restoreKeyRef.current !== restoreKey) {
    restoreKeyRef.current = restoreKey;
    restorePending.current = true;
  }

  // Session restores are bottom-anchored during layout, before the browser can
  // paint the transcript at the top and then visibly scroll it down.
  // Don't gate on hydrating — consuming restorePending before items arrive would
  // miss the scroll when items finally load. Instead just wait for non-empty
  // items without consuming the pending flag.
  useLayoutEffect(() => {
    if (!restorePending.current) return;
    stick.current = true;
    resetQuestionTracking(questionSnapshot);
    if (items.length === 0) return;
    restorePending.current = false;
    scrollToBottomAfterLayout(5);
  }, [items.length, questionSnapshot, resetQuestionTracking, scrollToBottomAfterLayout, stick, restoreKey]);
  ...
  }, [items.length, scheduleRepinIfWasPinned, scrollEl]);
  ...
  }, [footerHeight, items.length, scheduleRepinIfWasPinned, scrollEl]);
  ...
  const handleLoadOlderHistory = useCallback(() => {
    ...
    void preserveScrollAnchor(() => onLoadOlderHistory());
  }, [onLoadOlderHistory, preserveScrollAnchor]);

  const handleLoadEarlierWarmPage = useCallback(() => {
    void preserveScrollAnchor(() => {
      setWarmLayerState((prev) => warmLayerWithNextColdPage(prev, warmLayerSessionKey));
    });
  }, [preserveScrollAnchor, warmLayerSessionKey]);
  ...
  const effectiveDisplayMode: DisplayMode = informationMode ? "compact" : displayMode;
  ...
  const stepGroups = useMemo(() => {
    if (effectiveDisplayMode === "standard") return null;
    return buildStepGroups(items, hotStartIdx);
  }, [effectiveDisplayMode, hotStartIdx, items]);
  ...
  const hotZoneNodes = useMemo<ReactNode[]>(() => {
    ...
    if (stepGroups) {
      ...
      for (const group of stepGroups) {
        ...
        } else if (nonAssistantItems.length > 0 && informationMode) {
          const dur = nonAssistantItems.reduce((ms, it) => ms + (it.kind === "tool" ? ((it as ToolItem).durationMs ?? 0) : 0), 0);
          out.push(
            <TurnCollapse
              key={`step-active-${first.id}`}
              items={nonAssistantItems}
              durationMs={dur}
              mode={effectiveDisplayMode}
              subcalls={subcallsByParent}
      ...
      >
        ...
        {(!empty || hydrating) && (
          <LiveStreamContext.Provider value={live}>
            <div className="transcript__content" ref={entranceRef}>
              {hasOlderHistory && (
                <button
                  type="button"
              ...
              {turnGroups.length > HOT_TURNS && (
                <WarmZone
                  ...
                  scrollRef={scrollHostRef ?? scrollRef}
                  warmItems={items}
            ...
            onToggle={() => {
              ...
              if (el && node) {
                requestAnimationFrame(() => {
                  ...
                  el.scrollTo({ top: Math.max(0, el.scrollTop + nodeRect.top - hostRect.top - 80), behavior: "smooth" });
                });
              }
            }}
          />,

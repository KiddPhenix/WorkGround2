...
function tabForegroundActive(tab: TabMeta): boolean {
  if (typeof tab.foregroundActive === "boolean") return tab.foregroundActive;
  return tab.runtimeMode === "foreground" || tab.runtimeMode === "waiting_user" || tab.runtimeMode === "cancelling" || Boolean(tab.pendingPrompt);
}

function tabRuntimeTitle(tab: TabMeta): string {
  switch (tab.runtimeMode) {
    case "waiting_user":
    ...
    case "background_only":
      return (tab.backgroundJobs ?? 0) > 1 ? `Background jobs (${tab.backgroundJobs})` : "Background job";
    default:
      return tabForegroundActive(tab) ? "Running" : "";
  }
}
    ...
    <div className="tabbar">
      <div className="tabbar__tabs">
        {tabs.map((tab) => {
          ...
          const foregroundActive = tabForegroundActive(tab);
          const stateTitle = [
            tabRuntimeTitle(tab),
            planMode ? "Plan" : "",
            goalMode ? "Goal" : "",
          ...
          return (
            <button
              ...
              className={[
                ...
                foregroundActive ? "tabbar__tab--running" : "",
            ...
            >
              {tab.tabType === "file" || tab.scope === "file" ? (
                ...
                <span
                  className={[
                    "tabbar__status",
                    foregroundActive ? "tabbar__status--running" : "",
                  ].filter(Boolean).join(" ")}
                />
              )}

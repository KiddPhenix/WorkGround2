...
export interface TaskMemoryBarProps {
   ...
   * "recently did" summary derived from the latest real assistant text
   * keeps the bar visible and useful. Never use static demo text.
   */
 ...
 */
export function TaskMemoryBar({ memoryLine, onEditGoal, recentlyDid }: TaskMemoryBarProps) {
  // Terminal fallback: no task memory recorded, but we have a derived summary.
  if (!memoryLine && recentlyDid?.trim()) {
    return (
      <div className="task-memory-bar task-memory-bar--no-current" role="status" aria-label={`最近：${recentlyDid}`}>
        <Segment
          kind="recent"
  ...
  return (
    <div
      ...
      role="status"
      aria-label={[goal && `${memoryLine.goalSource === "user_prompt" ? "任务" : "目标"}：${goal}`, current && `当前：${current}`, nextStep && `下一步：${nextStep}`, recent && `最近：${recent}`].filter(Boolean).join(" · ")}
    >
      ...
      {recent && <Segment kind="recent" icon={<CheckCircle2 size={14} aria-hidden="true" />} label="最近" value={recent} />}
...
function Segment({
  ...
  return (
    <Tooltip label={`${label}：${value}`} side="top" className={`task-memory-bar__segment-slot task-memory-bar__segment-slot--${kind}`}>

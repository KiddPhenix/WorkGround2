...
export const RequestHelpCard = memo(function RequestHelpCard({
  ...
  const badge = status.phase === "completed"
  ...
  const StatusIcon = status.phase === "completed"
    ? CheckCircle2
    : failed
      ? AlertCircle
      : status.phase === "switching"
  ...
  return (
    <div
      className={`request-help${running ? " request-help--running" : ""}${status.phase === "completed" ? " request-help--done" : ""}${failed ? " request-help--failed" : ""}`}

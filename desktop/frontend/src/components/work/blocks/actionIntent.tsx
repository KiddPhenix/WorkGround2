...
function randomRequestID(fingerprint: string): string {
  try {
    if (typeof crypto?.randomUUID === 'function') return `work-action-v1-${crypto.randomUUID()}`;
    const bytes = new Uint32Array(4);
    crypto.getRandomValues(bytes);
    return `work-action-v1-${Array.from(bytes, (part) => part.toString(16).padStart(8, '0')).join('')}`;
  } catch {
    // A deterministic fallback keeps replays idempotent in restricted/web-test
    // environments where secure browser storage and crypto are unavailable.
    return `work-action-v1-${fingerprint}`;
  }
}

function requestID(key: string): string {
  const fingerprint = hash128(key);
  const cached = requests.get(fingerprint);
  if (cached) return cached;
  const storageKey = `wg2:work-action:v1:${fingerprint}`;
  try {
    const stored = window.sessionStorage.getItem(storageKey);
    if (stored) {
      requests.set(fingerprint, stored);
      return stored;
    }
  } catch {
    // Session storage is optional; module memory still covers remount/replay.
  }
  const created = randomRequestID(fingerprint);
  requests.set(fingerprint, created);
  try {
    window.sessionStorage.setItem(storageKey, created);
  } catch {
    // Keep the in-memory value when storage is blocked or full.
  }
  return created;
}
...
export function useActionIntent(props: ActionProps, actionID: string, inputRequired = false): ActionIntentState {
  ...
  const publish = useCallback((input?: Record<string, unknown>, retryReceipt?: ActionReceipt) => {
    ...
    const candidateID = requestID(key);
    ...
    if (failedReceipt && inputRequired && candidateID !== failedReceipt.requestId) {
      ...
      setLocal({ ...failedReceipt, message: 'Retry input does not match the original request' });
      return;
    }
  ...
  const canRetry = Boolean(receipt && receipt.status === 'failed' && receipt.retryable &&

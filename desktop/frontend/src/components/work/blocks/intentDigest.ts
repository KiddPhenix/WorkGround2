// Deterministic, synchronous digest for replayable renderer intents. The raw
// intent remains transient and only the fixed-size SHA-256 digest is exposed.
...
export function digestIntent(value: unknown): string {
  const source = utf8(stableJSON(value));

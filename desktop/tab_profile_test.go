...
effort = "max"
`
	if err := os.WriteFile(filepath.Join(projectRoot, "WorkGround2.toml"), []byte(configBody), 0o644); err != nil {
		t.Fatal(err)
	}
...
`
	if err := os.WriteFile(filepath.Join(root, "WorkGround2.toml"), []byte(configBody), 0o644); err != nil {
		t.Fatal(err)
	}
...
func TestAutoResearchStatusSurfaceForActiveTab(t *testing.T) {
	...
	heartbeatPath := filepath.Join(root, ".WorkGround2", "autoresearch", current.TaskID, "logs", "heartbeat.jsonl")
...
func TestAutoResearchFindingsAreLoadedOnDemand(t *testing.T) {
	...
	findingsPath := filepath.Join(root, ".WorkGround2", "autoresearch", current.TaskID, "state", "findings.jsonl")
...
func userConfigPathForTest() string {
	if dir, err := os.UserConfigDir(); err == nil {
		return dir + "/workground2/WorkGround2.toml"
	}
	return ""
}

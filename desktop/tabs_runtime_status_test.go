...
func TestListTabsReportsForegroundTurnStartedAt(t *testing.T) {
	...
	topicID := "topic_tab_started_at"
	topicTitle := "Tab started at"
	if err := setTopicTitle("", topicID, topicTitle); err != nil {
		t.Fatalf("set topic title: %v", err)
	}
	path := writeTopicSessionWithPrompt(t, dir, "tab-started.jsonl", topicID, topicTitle, "", "tab prompt", time.Now())

	app := NewApp()
	sink := &tabEventSink{tabID: "running", app: app}
	runner := &blockingRunner{started: make(chan struct{}), release: make(chan struct{})}
	ctrl := control.New(control.Options{Runner: runner, SessionDir: dir, SessionPath: path, Label: "running", Sink: sink})
	defer ctrl.Close()
	...
	if !tabs[0].ForegroundActive || !tabs[0].Running {
		t.Fatalf("tab runtime = %+v, want foreground running", tabs[0])
	}
	if tabs[0].TurnStartedAt < before || tabs[0].TurnStartedAt > time.Now().UnixMilli() {
		t.Fatalf("tab turnStartedAt = %d, want between %d and now", tabs[0].TurnStartedAt, before)
	}
...
func TestProjectTreeUsesPendingPromptRuntimeStatus(t *testing.T) {
	isolateDesktopUserDirs(t)
	dir := desktopSessionDir(globalTabWorkspaceRoot())
	if err := os.MkdirAll(dir, 0o755); err != nil {
		t.Fatalf("mkdir sessions: %v", err)
	}
	topicID := "topic_pending_prompt_status"
	topicTitle := "Pending prompt"
	if err := setTopicTitle("", topicID, topicTitle); err != nil {
		t.Fatalf("set topic title: %v", err)
	}
	path := writeTopicSessionWithPrompt(t, dir, "pending-prompt.jsonl", topicID, topicTitle, "", "pending prompt", time.Now())
	...
	app.tabOrder = []string{"pending"}
	app.activeTabID = "pending"
...
func TestProjectTreeShowsBackgroundJobStatus(t *testing.T) {
	...
	if topic.TurnStartedAt != 0 {
		t.Fatalf("background job topic turnStartedAt = %d, want 0", topic.TurnStartedAt)
	}
	tabs := app.ListTabs()
	if len(tabs) != 1 {
		t.Fatalf("tabs = %d, want 1", len(tabs))
	}
	if !tabs[0].Running || tabs[0].PendingPrompt || tabs[0].Cancellable || tabs[0].BackgroundJobs != 1 || tabs[0].RuntimeMode != string(control.RuntimeModeBackgroundOnly) || tabs[0].ForegroundActive || !tabs[0].BackgroundOnly || !tabs[0].ActiveRuntimeWork {
		t.Fatalf("tab runtime = %+v, want background-only active runtime", tabs[0])
	}
	if tabs[0].TurnStartedAt != 0 {
		t.Fatalf("background job tab turnStartedAt = %d, want 0", tabs[0].TurnStartedAt)
	}

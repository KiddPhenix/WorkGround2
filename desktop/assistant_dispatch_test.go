...
// TestAssistantRetryDispatchAPI proves the Wails retry entry re-runs a failed
// classification and returns the classified Dispatch.
func TestAssistantRetryDispatchAPI(t *testing.T) {
	...
	first, err := app.AssistantSubmit(AssistantSubmitRequest{AssistantID: created.Assistant.ID, RequestID: "submit-1", Input: "请扫描项目"})
	if err != nil {
		t.Fatalf("AssistantSubmit: %v", err)
	}
	if first.State != assistant.DispatchClassificationFailed {
		t.Fatalf("expected classification_failed, got %s", first.State)
	}
	retried, err := app.AssistantRetryDispatch(created.Assistant.ID, first.ID, "retry-1")
	if err != nil {
		t.Fatalf("AssistantRetryDispatch: %v", err)
	}

...
func TestMain(m *testing.M) {
	...
	os.Setenv("HOME", dir)
	os.Setenv("WorkGround2_CREDENTIALS_STORE", "file")
	os.Setenv("USERPROFILE", dir)
	os.Setenv("XDG_CONFIG_HOME", dir+"/config")
	os.Setenv("WorkGround2_STATE_HOME", dir+"/state")
	os.Setenv("WorkGround2_CACHE_HOME", dir+"/cache")
...
func TestDesktopShouldRunEmbeddedCLI(t *testing.T) {
	tests := []struct {
		args []string
		want bool
	}{
		{want: false},
		{args: []string{"desktop", "new"}, want: true},
		{args: []string{"desktop", "status"}, want: true},
		{args: []string{"--help"}, want: true},
		{args: []string{"version"}, want: true},
		{args: []string{"some-file.txt"}, want: false},
	}
	for _, tt := range tests {
		if got := desktopShouldRunEmbeddedCLI(tt.args); got != tt.want {
			t.Fatalf("desktopShouldRunEmbeddedCLI(%#v) = %v, want %v", tt.args, got, tt.want)
		}
	}
}

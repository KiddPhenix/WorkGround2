...
import (
	...
	"workground2/internal/eventwire"
...
type WorkspaceTab struct {
	...
	modelMu             sync.Mutex         // serializes model rebuilds and pending model selection
	...
	pendingModel      string // selected model to apply before the next user turn
	pendingModelLabel string // display label for pendingModel
	effort            *string
	tokenMode         string
...
func (t *WorkspaceTab) selectedModel() string {
	if t == nil {
		return ""
	}
	t.modelMu.Lock()
...
func (t *WorkspaceTab) activeTurnStartedAt() int64 {
	if t == nil {
		return 0
	}
	t.telemMu.Lock()
...
func (s *tabEventSink) Emit(e event.Event) {
	...
	w := toWireTab(e, s.tabID)
	if s.app != nil {
		_, ctrl := s.app.tabAndCtrlByID(s.tabID)
		if memory, ok := controllerTaskMemory(ctrl); ok {
			w.Event.TaskMemory = &memory
		}
	}
...
type TabMeta struct {
	...
	ForegroundActive    bool                     `json:"foregroundActive"`
...
func (a *App) tabMeta(tab *WorkspaceTab, active bool) TabMeta {
	...
	if tab.Ctrl != nil {
		...
		m.RuntimeMode = string(status.Mode)
		m.ForegroundActive = status.ForegroundActive
		m.BackgroundOnly = status.BackgroundOnly
...
func tabSessionDisplayTitle(tab *WorkspaceTab) string {
	if tab == nil {
		return ""
	}
	fallback := strings.TrimSpace(tab.TopicTitle)
	sessionPath := strings.TrimSpace(tab.currentSessionPath())
	if sessionPath == "" {
		return fallback
	}
	if titles, err := session.LoadTitles(filepath.Dir(sessionPath)); err == nil {
		if title := strings.TrimSpace(titles[filepath.Base(sessionPath)]); title != "" {
			return title
		}
	}
...
// workspace root is the WorkGround2 user config directory.
...
// RetryTabStartup restarts a failed tab build without replacing the tab or its
// queued frontend messages. Repeated calls while a build is already running are
// harmless.
func (a *App) RetryTabStartup(tabID string) error {
	a.mu.Lock()
	...
	if tab.Ctrl != nil || (!tab.Ready && tab.buildCancel != nil) {
		a.mu.Unlock()
		return nil
	}
	tab.Ready = false
...
func (a *App) buildTabControllerWithContext(tab *WorkspaceTab, loadedSession loadedTabSession, buildCtx context.Context, buildGeneration uint64, buildCancel context.CancelFunc) {
	...
	if dir := ctrl.SessionDir(); dir != "" {
		...
		if path != "" {
			...
			snapshot := loadTelemetry(telemetryPath)
			tab.telemMu.Lock()
...
// maybeAutoTitleTopicFromText promotes the first submitted user message to the
// topic title immediately. Waiting for the turn-end snapshot leaves long-running
// new sessions labelled with defaultTopicTitle in the sidebar even though the
// session header already has a preview title.
func (a *App) maybeAutoTitleTopicFromText(tab *WorkspaceTab, text string) bool {
	...
	if loadTopicTitleSource(titleRoot, tab.TopicID) != topicTitleSourceAuto {
		return false
	}
	...
	a.updateOpenTopicTitle(tab.TopicID, nextTitle)
	_ = saveTabSessionMeta(tab, tab.currentSessionPath())
	a.emitProjectTreeChanged()
	return true
}
...
func topicTitleFromUserText(text string) string {
	text = control.StripComposePrefixes(agent.HandoffTask(text))
	text = control.StripReferencedContextPrefix(text)
...
func desktopConfigDir() string {
	...
	if err != nil {
		...
		return filepath.Join(home, ".WorkGround2")
	}
	return filepath.Join(dir, "WorkGround2")
}
...
func topicTitlesPath(workspaceRoot string) string {
	...
	return filepath.Join(workspaceRoot, ".WorkGround2", topicTitlesFile)
}
...
func topicTitleSourcesPath(workspaceRoot string) string {
	...
	return filepath.Join(workspaceRoot, ".WorkGround2", topicTitleSourcesFile)
}
...
func topicCreatedAtsPath(workspaceRoot string) string {
	...
	return filepath.Join(workspaceRoot, ".WorkGround2", topicCreatedAtsFile)
}
...
func (a *App) tabSessionRecoveryMeta(tab *WorkspaceTab) func(control.SessionRecoveryRequest) agent.BranchMeta {
	return func(req control.SessionRecoveryRequest) agent.BranchMeta {
		...
		// This can fire from the controller's autosave goroutine; snapshot tab
		// fields under a.mu so recovery metadata does not read half-mutated UI
		// state.
		a.mu.RLock()
		ctrl := tab.Ctrl
...
func (a *App) handleTabSessionRecovered(tab *WorkspaceTab) func(control.SessionRecoveryInfo) error {
	return func(info control.SessionRecoveryInfo) error {
		...
		// with it and is persisted immediately instead of waiting for the next
		// usage event.
		if tab != nil && !tab.removed {
...
func (a *App) emitSessionActivated(reason string) {
	a.mu.RLock()
	tab := a.tabs[a.activeTabID]
	payload := sessionActivatedEvent{Reason: strings.TrimSpace(reason)}
	if tab != nil {
		payload.TabID = tab.ID
		payload.SessionPath = tab.currentSessionPath()
	}
	a.mu.RUnlock()
	a.emitRuntimeEvent("session:activated", payload)
}
...
func hasGlobalTrashedSessions() bool {
	for _, dir := range []string{config.SessionDir(), desktopSessionDir(globalWorkspaceRoot())} {
		...
		for _, path := range paths {
			...
			scope := strings.TrimSpace(infos[0].Scope)
			if scope == "global" || (scope == "" && normalizeProjectRoot(infos[0].WorkspaceRoot) == "") {
				return true
			}
		}
	}
...
func (a *App) ListProjectTree() []ProjectNode {
	...
	addRuntimeSession := func(tab *WorkspaceTab, open bool) {
		...
		if sessionKey == "" || seenRuntimePaths[sessionKey] {
			return
		}

# WorkGround2 Desktop (Wails shell)

A native desktop window around the WorkGround2 Go kernel. The same
...
`desktop/` is its own Go module (`module WorkGround2/desktop`, `replace WorkGround2 =>
...
`WorkGround2/` so it can still import the `WorkGround2/internal/*` kernel.
...
```sh
cd desktop
wails build          # → build/bin/WorkGround2(.app/.exe)
```
...
- **macOS** — open `WorkGround2-darwin-universal.dmg` and drag WorkGround2 into
  ...
  ```sh
  xattr -dr com.apple.quarantine /Applications/WorkGround2.app
  ```
...
```sh
minisign -Vm WorkGround2-darwin-arm64.zip \
...
- **Linux / WebKitGTK** is the one real pain point — rendering varies by distro &
  ...
  - **Wayland + NVIDIA**: On KDE Plasma Wayland with NVIDIA GPUs, WebKitGTK can
    ...
    WorkGround2 automatically sets `__NV_DISABLE_EXPLICIT_SYNC=1` when it detects
...
- **Windows / WebView2** — `Theme: SystemDefault` follows the OS light/dark
  ...
  `WorkGround2_DESKTOP_DISABLE_WEBVIEW2_GPU=1` or `0` to force the fallback on or
  off.
...
The desktop app sends one anonymous ping per launch to `crash.WorkGround2.io`:
...
`agent.memory_compiler.enabled` setting with the CLI/TUI and `WorkGround2 serve`;
CLI users can also run `/memory-v5 off|observe|compact|on|status` in a session
or `WorkGround2 config memory-v5 off|observe|compact|on|status` from a shell.

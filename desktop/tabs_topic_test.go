...
func TestRetryTabStartupReusesFailedTab(t *testing.T) {
	...
	if err := app.RetryTabStartup(tab.ID); err != nil {
		t.Fatalf("RetryTabStartup: %v", err)
	}
	ready := waitForTabReady(t, app, tab.ID)
	if ready.Ctrl == nil || ready.StartupErr != "" {
		t.Fatalf("retry did not recover tab: ctrl=%v startupErr=%q", ready.Ctrl, ready.StartupErr)
	}
}
...
func TestV05LegacyEventSessionsImportIntoGlobalTopic(t *testing.T) {
	...
	legacyDir := filepath.Join(home, ".WorkGround2", "sessions")
...
func TestBuildTabControllerIgnoresStaleSessionModelWhenTabModelResolves(t *testing.T) {
	isolateDesktopUserDirs(t)
	t.Setenv("WorkGround2_TEST_KEY", "sk-test")
...
model = "default-model"
api_key_env = "WorkGround2_TEST_KEY"
...
func TestLoadPinnedTabSessionFallsBackToMigratedBasename(t *testing.T) {
	...
	oldPath := filepath.Join(t.TempDir(), "old-WorkGround2", "projects", "slug", "sessions", filepath.Base(path))
...
func TestTrashGlobalTopicKeepsEmptyGlobalWithProjects(t *testing.T) {
	isolateDesktopUserDirs(t)
	...
	sessionPath := writeLegacySession(t, dir, "global-with-project.jsonl", "global history", time.Now().Add(-time.Hour))
	topicID := legacySessionTopicID(sessionPath)
	app := NewApp()

	initialNodes := app.ListProjectTree()
	if len(initialNodes) == 0 {
		t.Fatalf("initial project tree = %#v, want Global topic before trash", initialNodes)
	}
	...
	for i := range nodes {
		...
		case "global_folder":
			global = &nodes[i]
		case "project":
			projectFound = projectFound || nodes[i].Root == projectRoot
		}
	}
	if global == nil {
		t.Fatalf("project tree after trash = %#v, want empty Global folder alongside project", nodes)
	}
	if len(global.Children) != 0 {
		t.Fatalf("global children after trash = %#v, want empty", global.Children)
	}
	if !projectFound {
		t.Fatalf("project tree after trash = %#v, want project %q to remain", nodes, projectRoot)
	}
}
...
func TestSwitchWorkspaceReusesExistingWorkspace(t *testing.T) {
	isolateDesktopUserDirs(t)

	projectRoot := t.TempDir()
	app := NewApp()
	firstRoot, err := app.SwitchWorkspace(projectRoot)
	if err != nil {
		t.Fatalf("first SwitchWorkspace: %v", err)
	}
	if firstRoot != projectRoot {
		t.Fatalf("first root = %q, want %q", firstRoot, projectRoot)
	}
	firstTabs := app.ListTabs()
	if len(firstTabs) != 1 {
		t.Fatalf("first tabs len = %d, want 1: %+v", len(firstTabs), firstTabs)
	}
	firstTabID := firstTabs[0].ID
	firstTopicID := firstTabs[0].TopicID
	if strings.TrimSpace(firstTopicID) == "" {
		t.Fatalf("first switch should create a reusable default topic: %+v", firstTabs[0])
	}

	secondRoot, err := app.SwitchWorkspace(projectRoot)
	if err != nil {
		t.Fatalf("second SwitchWorkspace: %v", err)
	}
	if secondRoot != projectRoot {
		t.Fatalf("second root = %q, want %q", secondRoot, projectRoot)
	}

	tabs := app.ListTabs()
	if len(tabs) != 1 {
		t.Fatalf("tabs len after second switch = %d, want 1: %+v", len(tabs), tabs)
	}
	if tabs[0].ID != firstTabID || tabs[0].TopicID != firstTopicID {
		t.Fatalf("second switch should reuse first tab/topic, got %+v want tab %q topic %q", tabs[0], firstTabID, firstTopicID)
	}
	...
	if !found {
		t.Fatalf("project %q not persisted: %+v", projectRoot, f.Projects)
	}
	if got := project.Topics; len(got) != 1 || got[0] != firstTopicID {
		t.Fatalf("project topics after repeated switch = %+v, want only %q", got, firstTopicID)
	}
}

func TestSwitchWorkspaceUsesExistingWorkspaceTopic(t *testing.T) {
	isolateDesktopUserDirs(t)

	projectRoot := t.TempDir()
	app := NewApp()
	topic, err := app.CreateTopic("project", projectRoot, "Existing")
	if err != nil {
		t.Fatalf("create topic: %v", err)
	}
	...
	tabs := app.ListTabs()
	if len(tabs) != 1 || tabs[0].TopicID != topic.ID {
		t.Fatalf("SwitchWorkspace should open existing topic, tabs=%+v topic=%+v", tabs, topic)
	}
	f := loadProjectsFile()
	var topics []string
	...
	if len(topics) != 1 || topics[0] != topic.ID {
		t.Fatalf("project topics after switch = %+v, want only existing topic %q", topics, topic.ID)
	}
}
...
func TestAutoTitleTopicFromSubmittedTextUpdatesOpenTabImmediately(t *testing.T) {
	isolateDesktopUserDirs(t)
	...
	app.activeTabID = tab.ID
	changes := 0
	app.projectTreeChangedHook = func() { changes++ }

	if !app.maybeAutoTitleTopicFromText(tab, "新会话在列表的名字没有及时修改") {
		t.Fatal("submitted text should update the automatic topic title")
	}
	const want = "新会话在列表的名字没有及时修改"
	if tab.TopicTitle != want {
		t.Fatalf("open tab title = %q, want %q", tab.TopicTitle, want)
	}
	if got := loadTopicTitle(projectRoot, topic.ID); got != want {
		t.Fatalf("stored title = %q, want %q", got, want)
	}
	nodes := app.ListProjectTree()
	if len(nodes) != 1 || len(nodes[0].Children) != 1 || nodes[0].Children[0].Label != want {
		t.Fatalf("project tree did not expose the new title immediately: %#v", nodes)
	}
	if changes != 1 {
		t.Fatalf("project tree change events = %d, want 1", changes)
	}

	if app.maybeAutoTitleTopicFromText(tab, "第二条消息不应覆盖首条标题") {
		t.Fatal("later submitted text should not replace the first automatic title")
	}
	if tab.TopicTitle != want {
		t.Fatalf("title after second message = %q, want %q", tab.TopicTitle, want)
	}
}

func TestAutoTitleTopicFromSubmittedTextIgnoresCommands(t *testing.T) {
	isolateDesktopUserDirs(t)
	...
	if app.maybeAutoTitleTopicFromText(tab, "/compact") {
		t.Fatal("slash command should not become a topic title")
	}
	if got := loadTopicTitle(projectRoot, topic.ID); got != defaultTopicTitle {
		t.Fatalf("stored title = %q, want %q", got, defaultTopicTitle)
	}

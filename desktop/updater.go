...
const (
	r2Base         = "https://dl.WorkGround2.io"
	ghReleasesBase = "https://github.com/KiddPhenix/WorkGround2/releases"
...
func defaultUpdateCacheBaseDir() (string, error) {
	...
	return filepath.Join(base, "WorkGround2", "updates"), nil
}
...
func assetFileName(asset update.Asset, version string) string {
	...
	return "WorkGround2-" + clean + "-" + update.CurrentPlatform() + ".update"
}
...
func applyLinux(targz []byte) error {
	bin, err := extractBinary(targz, "WorkGround2-desktop")
	if err != nil {
		return err
	}
